...
fn main() {
    ...
    // Ensure GPMF files exist (create stubs if missing)
    ensure_gpmf_files_exist();

    // Ensure writable tags file exists (create stub if missing)
    ensure_writable_tags_exists();
    println!("cargo:rerun-if-changed=third-party/exiftool/lib/Image/ExifTool/Exif.pm");
    println!("cargo:rerun-if-changed=third-party/exiftool/lib/Image/ExifTool/Canon.pm");
    println!("cargo:rerun-if-changed=third-party/exiftool/lib/Image/ExifTool/Olympus.pm");
    ...
    // Parse Canon tags
    let canon_pm_path = "third-party/exiftool/lib/Image/ExifTool/Canon.pm";
...
fn ensure_gpmf_files_exist() {
    let gpmf_files = [
        ("src/gpmf/mod.rs", "mod"),
        ("src/gpmf/tags.rs", "tags"),
        ("src/gpmf/format.rs", "format"),
    ];

    for (file_path, file_type) in &gpmf_files {
        if !Path::new(file_path).exists() {
            // Create directory if it doesn't exist
            ...
            let stub_content = match *file_type {
                "mod" => generate_gpmf_mod_stub(),
                "tags" => generate_gpmf_tags_stub(),
                "format" => generate_gpmf_format_stub(),
                _ => String::new(),
            };
...
fn generate_gpmf_mod_stub() -> String {
    r#"//! STUB: Auto-generated by build.rs to ensure compilation
//! Regenerate with: cargo run --bin exiftool_sync extract gpmf-tags gpmf-format
...
/// GPMF module stub (regenerate to populate)
pub fn parse_gpmf(_data: &[u8]) -> Result<(), String> {
...
fn generate_gpmf_tags_stub() -> String {
    r#"//! STUB: Auto-generated by build.rs to ensure compilation
...
pub const GPMF_TAGS: &[GpmfTag] = &[
    // Stub - real GPMF tags will be generated by extractor
];
...
fn generate_gpmf_format_stub() -> String {
    r#"//! STUB: Auto-generated by build.rs to ensure compilation
...
/// Ensure writable_tags.rs file exists, creating a stub if missing
fn ensure_writable_tags_exists() {
    let file_path = "src/write/writable_tags.rs";

    if !Path::new(file_path).exists() {
        // Create directory if needed
        let _ = fs::create_dir_all("src/write");
...
/// Safety level for write operations (stub)
...
/// Writable format types from ExifTool (stub)
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Information about a writable tag (stub)
#[derive(Debug, Clone)]
pub struct WritableTagInfo {
    pub tag_name: String,
    pub tag_id: Option<u32>,
    pub tag_id_hex: Option<String>,
    pub group: String,
    pub writable_format: WritableFormat,
    pub write_proc: Option<String>,
    pub safety_level: SafetyLevel,
    pub source_file: String,
}

lazy_static! {
    /// Registry of all writable tags from ExifTool (stub - empty)
    pub static ref WRITABLE_TAGS: HashMap<String, WritableTagInfo> = HashMap::new();
}

/// Check if a tag is writable (stub implementation)
pub fn is_tag_writable(_tag_name: &str) -> bool {
    false // Stub implementation
}

/// Get writable tag information (stub implementation)
pub fn get_writable_tag_info(_tag_name: &str) -> Option<&WritableTagInfo> {
    None // Stub implementation
}

/// Get tag safety level (stub implementation)
pub fn get_tag_safety_level(_tag_name: &str) -> Option<&SafetyLevel> {
    None // Stub implementation
}

/// Get all writable tags by safety level (stub implementation)
pub fn get_tags_by_safety_level(_level: &SafetyLevel) -> Vec<&WritableTagInfo> {
    Vec::new() // Stub implementation
}

/// Get writable tags count (stub implementation)
pub fn get_writable_tags_count() -> usize {
