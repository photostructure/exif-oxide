...
fn main() {
    ...
    match Regex::new(claude_pattern) {
        Ok(regex) => {
            ...
            println!("\\x89P matches first 2 bytes: {}", regex.is_match(&alt1_data));
            println!("\\x8aM matches first 2 bytes: {}", regex.is_match(&alt2_data));
            println!("\\x8bJ matches first 2 bytes: {}", regex.is_match(&alt3_data));
