...
impl PrintConvAnalyzer {
    pub fn new(manufacturer_file: &str) -> Self {
        let manufacturer = manufacturer_file
            .trim_end_matches(".pm")
            .to_string();
    ...
    fn parse_main_table_patterns(&mut self, content: &str) -> Result<(), String> {
        ...
        let inline_printconv_re = Regex::new(r"(?s)(0x[0-9a-fA-F]+)\s*=>\s*\{([^}]*PrintConv[^}]*)\}")
    ...
    fn parse_subdirectory_patterns(&mut self, content: &str) -> Result<(), String> {
        ...
        let table_def_re = Regex::new(&format!(r"%Image::ExifTool::{}::([A-Za-z0-9_]+)\s*=\s*\(", self.manufacturer))
        ...
        for table_cap in table_def_re.captures_iter(content) {
            ...
            let full_table_name = format!("%Image::ExifTool::{}::{}", self.manufacturer, table_name);
    ...
    fn parse_table_printconv_patterns(&mut self, table_content: &str, table_name: &str) -> Result<(), String> {
        // Look for entries with PrintConv in subdirectory tables
        let entry_with_printconv_re = Regex::new(r"(?s)(\d+|0x[0-9a-fA-F]+)\s*=>\s*\{([^}]*PrintConv[^}]*)\}")
    ...
    fn parse_printconv_hash(&self, printconv_content: &str) -> Result<Vec<(String, String)>, String> {
        ...
        for pattern in &patterns {
            let pair_re = Regex::new(pattern)
                .map_err(|e| format!("Regex error for {}: {}", pattern, e))?;
    ...
    fn classify_patterns(&mut self) {
        ...
        let universal_patterns = [
            ...
            ("Quality", vec!["1", "Best", "2", "Better", "3", "Good", "4", "Normal"]),
            ("FlashMode", vec!["0", "Auto", "1", "On", "2", "Off"]),
            ("WhiteBalance", vec!["0", "Auto", "1", "Daylight", "2", "Cloudy", "3", "Tungsten"]),
            ("MeteringMode", vec!["0", "Multi", "1", "Center", "2", "Spot"]),
        ];
    ...
    fn pattern_matches_universal(pattern_values: &[(String, String)], universal_values: &[&str]) -> bool {
    ...
    fn is_simple_lookup(values: &[(String, String)]) -> bool {
        ...
        values.iter().all(|(_, value)| {
            !value.contains('$') && !value.contains('(') && value.len() < 50
        })
    }
    ...
    fn print_analysis(&self) {
        ...
        if !shared_lookup_groups.is_empty() {
            ...
            for (shared_name, tag_patterns) in &shared_lookup_groups {
                println!("- {} → {} tags can share single implementation:", shared_name, tag_patterns.len());
        ...
        for pattern in &self.patterns {
            match &pattern.pattern_type {
                ...
                PrintConvType::Complex(name) => {
                    println!("- {} '{}' → NEW: PrintConvId::{} (complex)", 
                             pattern.tag_id, pattern.tag_name, name);
                    complex_count += 1;
                }
                _ => {}
        ...
        let reusable_pct = if total > 0 { (universal_count * 100) / total } else { 0 };
        let shared_count = shared_lookup_groups.values().map(|v| v.len()).sum::<usize>();
        let optimization_pct = if total > 0 { (shared_count * 100) / total } else { 0 };
        
        println!("\nOptimization summary:");
        println!("- {}% patterns reuse existing universal functions", reusable_pct);
        println!("- {}% patterns benefit from shared lookup table optimization", optimization_pct);
        println!("- {} shared lookup tables eliminate {} duplicate implementations", 
