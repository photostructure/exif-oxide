...
//!
//! Generated from: PanasonicRaw.pm table: CameraIFD
//!
...
pub fn apply_print_conv(
    ...
    match tag_id {
        4866 => crate::implementations::print_conv::exposuretime_print_conv(value),
        _ => {
...
pub fn apply_value_conv(
    ...
    match tag_id {
        ...
        2 => {
            // Compiled arithmetic: $val / 32768
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 32768.0_f64)),
                None => Ok(value.clone()),
            }
        }
        4 => {
        ...
        5 => {
            // Compiled arithmetic: 1 / (1 + $val/32768)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(1.0_f64 / (1.0_f64 + (val / 32768.0_f64)))),
                None => Ok(value.clone()),
            }
        }
        8 => {
        ...
        284 => {
            // Compiled arithmetic: $val / ($val >= 1024 ? 1024 : ($val >= 256 ? 256 : 100))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    val / if val >= 1024.0_f64 {
                        1024.0_f64
                    } else if val >= 256.0_f64 {
                        256.0_f64
                    } else {
                        100.0_f64
                    },
        ...
        4865 => {
            // Compiled arithmetic: 2 ** ($val / 512)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(val / 512.0_f64))),
                None => Ok(value.clone()),
            }
        }
        4867 => {
