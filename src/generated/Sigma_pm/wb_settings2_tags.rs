//! Generated tag table definitions from Sigma::WBSettings2
//!
//! This file is auto-generated by codegen/src/strategies/tag_kit.rs. Do not edit manually.

use crate::types::{PrintConv, TagInfo, ValueConv};
use std::collections::HashMap;
use std::sync::LazyLock;

/// Tag definitions for Sigma::WBSettings2 table
pub static SIGMA_WBSETTINGS2_TAGS: LazyLock<HashMap<u16, TagInfo>> = LazyLock::new(|| {
    HashMap::from([
        (
            0,
            TagInfo {
                name: "WB_RGBLevelsUnknown0",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            3,
            TagInfo {
                name: "WB_RGBLevelsUnknown1",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            6,
            TagInfo {
                name: "WB_RGBLevelsUnknown2",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            9,
            TagInfo {
                name: "WB_RGBLevelsUnknown3",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            12,
            TagInfo {
                name: "WB_RGBLevelsUnknown4",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            15,
            TagInfo {
                name: "WB_RGBLevelsUnknown5",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            18,
            TagInfo {
                name: "WB_RGBLevelsUnknown6",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            21,
            TagInfo {
                name: "WB_RGBLevelsUnknown7",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            24,
            TagInfo {
                name: "WB_RGBLevelsUnknown8",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            27,
            TagInfo {
                name: "WB_RGBLevelsUnknown9",
                format: "float[3]",
                print_conv: None,
                value_conv: None,
            },
        ),
    ])
});

/// Apply ValueConv transformation for tags in this table
pub fn apply_value_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
) -> Result<crate::types::TagValue, String> {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = SIGMA_WBSETTINGS2_TAGS.get(&tag_id_u16) {
        if let Some(ref value_conv) = tag_def.value_conv {
            match value_conv {
                ValueConv::None => Ok(value.clone()),
                ValueConv::Function(func) => func(value).map_err(|e| e.to_string()),
                ValueConv::Expression(expr) => {
                    // Use runtime expression evaluator for dynamic evaluation
                    let mut evaluator = crate::expressions::ExpressionEvaluator::new();
                    evaluator
                        .evaluate_expression(expr, value)
                        .map_err(|e| e.to_string())
                }
                _ => Ok(value.clone()),
            }
        } else {
            Ok(value.clone())
        }
    } else {
        Err(format!("Tag 0x{:04x} not found in table", tag_id))
    }
}

/// Apply PrintConv transformation for tags in this table
pub fn apply_print_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _evaluator: &mut crate::expressions::ExpressionEvaluator,
    _errors: &mut Vec<String>,
    _warnings: &mut Vec<String>,
) -> crate::types::TagValue {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = SIGMA_WBSETTINGS2_TAGS.get(&tag_id_u16) {
        if let Some(ref print_conv) = tag_def.print_conv {
            match print_conv {
                PrintConv::None => value.clone(),
                PrintConv::Function(func) => func(value),
                PrintConv::Expression(expr) => {
                    // Use runtime expression evaluator for dynamic evaluation
                    _evaluator
                        .evaluate_expression(expr, value)
                        .unwrap_or_else(|_| value.clone())
                }
                _ => value.clone(),
            }
        } else {
            value.clone()
        }
    } else {
        value.clone()
    }
}
