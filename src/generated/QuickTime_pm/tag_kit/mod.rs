...
//!
//! Generated from: QuickTime.pm table: AV1Config
//!
...
fn process_quicktime_facerec(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FaceItem at offset cits
...
fn process_quicktime_genmediainfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // GenFlags at offset 1
    if data.len() >= 5 {
        // TODO: Handle format int8u
    }

    // GenBalance at offset 12
    if data.len() >= 26 {
        // TODO: Handle format fixed16s
    }

    // GenGraphicsMode at offset 4
    if data.len() >= 10 {
        // TODO: Handle format int16u
    }

    // GenOpColor at offset 6
    if data.len() >= 18 {
        if let Ok(values) = read_int16u_array(&data[12..18], byte_order, 3) {
            let value_str = values
                ...
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("GenOpColor".to_string(), TagValue::String(value_str)));
        }
    }
...
fn process_quicktime_timecode(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // TCMediaInfo at offset tcmi
...
fn process_quicktime_metarelation(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
...
fn process_quicktime_schemeinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // Certificate at offset cert

    // InitializationVector at offset iviv

    // KeyID at offset key

    // UserName at offset name

    // Rights at offset righ

    // UserID at offset user

    Ok(tags)
}

fn process_quicktime_schemetype(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // SchemeURL at offset 10

    // SchemeType at offset 4
    if data.len() >= 16 {
        // TODO: Handle format undef
    }

    // SchemeVersion at offset 8
    if data.len() >= 18 {
        // TODO: Handle format int16u
    }
...
fn process_quicktime_rights(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ItemTool at offset tool
    if data.len() >= 2 {
        // TODO: Handle format string
    }
...
fn process_quicktime_tcmediainfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // TextColor at offset 12
    if data.len() >= 30 {
        if let Ok(values) = read_int16u_array(&data[24..30], byte_order, 3) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("TextColor".to_string(), TagValue::String(value_str)));
        }
    }

    // BackgroundColor at offset 18
    if data.len() >= 42 {
        if let Ok(values) = read_int16u_array(&data[36..42], byte_order, 3) {
            let value_str = values
                ...
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("BackgroundColor".to_string(), TagValue::String(value_str)));
        }
    }

    // FontName at offset 24
    if data.len() >= 50 {
        // TODO: Handle format pstring
    }

    // TextFont at offset 4
    if data.len() >= 10 {
        // TODO: Handle format int16u
    }

    // TextFace at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int16u
    }

    // TextSize at offset 8
    if data.len() >= 18 {
        // TODO: Handle format int16u
    }
...
fn process_quicktime_encodingparams(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AudioComponentVersion at offset cdcv
...
pub fn apply_print_conv(
    ...
    match tag_id {
        0 => crate::implementations::print_conv::gpsaltituderef_print_conv(value),
        _ => {
