//! Generated tag table definitions from CanonCustom::Functions2
//!
//! This file is auto-generated by codegen/src/strategies/tag_kit.rs. Do not edit manually.

use crate::types::{PrintConv, TagInfo, ValueConv};
use std::collections::HashMap;
use std::sync::LazyLock;

/// Tag definitions for CanonCustom::Functions2 table
pub static CANON_CUSTOM_FUNCTIONS2_TAGS: LazyLock<HashMap<u16, TagInfo>> = LazyLock::new(|| {
    HashMap::from([
        (
            257,
            TagInfo {
                name: "ExposureLevelIncrements",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "1/3 Stop"),
                    ("1".to_string(), "1/2 Stop"),
                ]))),
                value_conv: None,
            },
        ),
        (
            258,
            TagInfo {
                name: "ISOSpeedIncrements",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "1/3 Stop"),
                    ("1".to_string(), "1 Stop"),
                ]))),
                value_conv: None,
            },
        ),
        (
            259,
            TagInfo {
                name: "ISOExpansion",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "On"),
                ]))),
                value_conv: None,
            },
        ),
        (
            260,
            TagInfo {
                name: "AEBAutoCancel",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "On"),
                    ("1".to_string(), "Off"),
                ]))),
                value_conv: None,
            },
        ),
        (
            261,
            TagInfo {
                name: "AEBSequence",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "0,-,+"),
                    ("1".to_string(), "-,0,+"),
                    ("2".to_string(), "+,0,-"),
                ]))),
                value_conv: None,
            },
        ),
        (
            262,
            TagInfo {
                name: "AEBShotCount",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("2 1".to_string(), "2 shots"),
                    ("3 0".to_string(), "3 shots"),
                    ("5 2".to_string(), "5 shots"),
                    ("7 3".to_string(), "7 shots"),
                ]))),
                value_conv: None,
            },
        ),
        (
            263,
            TagInfo {
                name: "SpotMeterLinkToAFPoint",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable (use center AF point)"),
                    ("1".to_string(), "Enable (use active AF point)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            264,
            TagInfo {
                name: "SafetyShift",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable (Tv/Av)"),
                    ("2".to_string(), "Enable (ISO speed)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            265,
            TagInfo {
                name: "UsableShootingModes",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            266,
            TagInfo {
                name: "UsableMeteringModes",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            267,
            TagInfo {
                name: "ExposureModeInManual",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Specified metering mode"),
                    ("1".to_string(), "Evaluative metering"),
                    ("2".to_string(), "Partial metering"),
                    ("3".to_string(), "Spot metering"),
                    ("4".to_string(), "Center-weighted average"),
                ]))),
                value_conv: None,
            },
        ),
        (
            268,
            TagInfo {
                name: "ShutterSpeedRange",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            269,
            TagInfo {
                name: "ApertureRange",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            270,
            TagInfo {
                name: "ApplyShootingMeteringMode",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            271,
            TagInfo {
                name: "FlashSyncSpeedAv",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Auto"),
                    ("1".to_string(), "1/200-1/60 Auto"),
                    ("2".to_string(), "1/200 Fixed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            272,
            TagInfo {
                name: "AEMicroadjustment",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            273,
            TagInfo {
                name: "FEMicroadjustment",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            274,
            TagInfo {
                name: "SameExposureForNewAperture",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "ISO Speed"),
                    ("2".to_string(), "ISO Speed/Shutter Speed"),
                    ("3".to_string(), "Shutter Speed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            275,
            TagInfo {
                name: "ExposureCompAutoCancel",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            276,
            TagInfo {
                name: "AELockMeterModeAfterFocus",
                format: "unknown",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            513,
            TagInfo {
                name: "LongExposureNoiseReduction",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Auto"),
                    ("2".to_string(), "On"),
                ]))),
                value_conv: None,
            },
        ),
        (
            514,
            TagInfo {
                name: "HighISONoiseReduction",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "On"),
                ]))),
                value_conv: None,
            },
        ),
        (
            515,
            TagInfo {
                name: "HighlightTonePriority",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            516,
            TagInfo {
                name: "AutoLightingOptimizer",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            772,
            TagInfo {
                name: "ETTLII",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Evaluative"),
                    ("1".to_string(), "Average"),
                ]))),
                value_conv: None,
            },
        ),
        (
            773,
            TagInfo {
                name: "ShutterCurtainSync",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "1st-curtain sync"),
                    ("1".to_string(), "2nd-curtain sync"),
                ]))),
                value_conv: None,
            },
        ),
        (
            774,
            TagInfo {
                name: "FlashFiring",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Fires"),
                    ("1".to_string(), "Does not fire"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1031,
            TagInfo {
                name: "ViewInfoDuringExposure",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1032,
            TagInfo {
                name: "LCDIlluminationDuringBulb",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "On"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1033,
            TagInfo {
                name: "InfoButtonWhenShooting",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Displays shooting functions"),
                    ("1".to_string(), "Displays camera settings"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1034,
            TagInfo {
                name: "ViewfinderWarnings",
                format: "unknown",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            1035,
            TagInfo {
                name: "LVShootingAreaDisplay",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Masked"),
                    ("1".to_string(), "Outlined"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1036,
            TagInfo {
                name: "LVShootingAreaDisplay",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Masked"),
                    ("1".to_string(), "Outlined"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1281,
            TagInfo {
                name: "USMLensElectronicMF",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable after one-shot AF"),
                    ("1".to_string(), "Disable after one-shot AF"),
                    ("2".to_string(), "Disable in AF mode"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1282,
            TagInfo {
                name: "AIServoTrackingSensitivity",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("-1".to_string(), "Medium Slow"),
                    ("-2".to_string(), "Slow"),
                    ("0".to_string(), "Standard"),
                    ("1".to_string(), "Medium Fast"),
                    ("2".to_string(), "Fast"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1283,
            TagInfo {
                name: "AIServoImagePriority",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "1: AF, 2: Tracking"),
                    ("1".to_string(), "1: AF, 2: Drive speed"),
                    ("2".to_string(), "1: Release, 2: Drive speed"),
                    ("3".to_string(), "1: Release, 2: Tracking"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1284,
            TagInfo {
                name: "AIServoTrackingMethod",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Main focus point priority"),
                    ("1".to_string(), "Continuous AF track priority"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1285,
            TagInfo {
                name: "LensDriveNoAF",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Focus search on"),
                    ("1".to_string(), "Focus search off"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1286,
            TagInfo {
                name: "LensAFStopButton",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "AF stop"),
                    ("1".to_string(), "AF start"),
                    ("2".to_string(), "AE lock"),
                    ("3".to_string(), "AF point: M->Auto/Auto->ctr"),
                    ("4".to_string(), "One Shot <-> AI servo"),
                    ("5".to_string(), "IS start"),
                    ("6".to_string(), "Switch to registered AF point"),
                    ("7".to_string(), "Spot AF"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1287,
            TagInfo {
                name: "AFMicroadjustment",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1288,
            TagInfo {
                name: "AFPointAreaExpansion",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Left/right AF points"),
                    ("2".to_string(), "Surrounding AF points"),
                    ("3".to_string(), "All 45 points area"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1289,
            TagInfo {
                name: "SelectableAFPoint",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "19 points"),
                    ("1".to_string(), "Inner 9 points"),
                    ("2".to_string(), "Outer 9 points"),
                    ("3".to_string(), "19 Points, Multi-controller selectable"),
                    (
                        "4".to_string(),
                        "Inner 9 Points, Multi-controller selectable",
                    ),
                    (
                        "5".to_string(),
                        "Outer 9 Points, Multi-controller selectable",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            1290,
            TagInfo {
                name: "SwitchToRegisteredAFPoint",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1291,
            TagInfo {
                name: "AFPointAutoSelection",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Control-direct:disable/Main:enable"),
                    ("1".to_string(), "Control-direct:disable/Main:disable"),
                    ("2".to_string(), "Control-direct:enable/Main:enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1292,
            TagInfo {
                name: "AFPointDisplayDuringFocus",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Selected (constant)"),
                    ("1".to_string(), "All (constant)"),
                    ("2".to_string(), "Selected (pre-AF, focused)"),
                    ("3".to_string(), "Selected (focused)"),
                    ("4".to_string(), "Disable display"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1293,
            TagInfo {
                name: "AFPointBrightness",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal"),
                    ("1".to_string(), "Brighter"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1294,
            TagInfo {
                name: "AFAssistBeam",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Emits"),
                    ("1".to_string(), "Does not emit"),
                    ("2".to_string(), "Only ext. flash emits"),
                    ("3".to_string(), "IR AF assist beam only"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1295,
            TagInfo {
                name: "AFPointSelectionMethod",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    (
                        "0".to_string(),
                        "AF point button: Activate AF Sel; Rear dial: Select AF points",
                    ),
                    (
                        "1".to_string(),
                        "AF point button: Auto selection; Rear dial: Manual selection",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            1296,
            TagInfo {
                name: "SuperimposedDisplay",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "On"),
                    ("1".to_string(), "Off"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1297,
            TagInfo {
                name: "AFDuringLiveView",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Quick mode"),
                    ("2".to_string(), "Live mode"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1298,
            TagInfo {
                name: "SelectAFAreaSelectMode",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1299,
            TagInfo {
                name: "ManualAFPointSelectPattern",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Stops at AF area edges"),
                    ("1".to_string(), "Continuous"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1300,
            TagInfo {
                name: "DisplayAllAFPoints",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1301,
            TagInfo {
                name: "FocusDisplayAIServoAndMF",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1302,
            TagInfo {
                name: "OrientationLinkedAFPoint",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Same for vertical and horizontal"),
                    ("1".to_string(), "Select different AF points"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1303,
            TagInfo {
                name: "MultiControllerWhileMetering",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "AF point selection"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1304,
            TagInfo {
                name: "AccelerationTracking",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1305,
            TagInfo {
                name: "AIServoFirstImagePriority",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("-1".to_string(), "Release priority"),
                    ("0".to_string(), "Equal priority"),
                    ("1".to_string(), "Focus priority"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1306,
            TagInfo {
                name: "AIServoSecondImagePriority",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("-1".to_string(), "Shooting speed priority"),
                    ("0".to_string(), "Equal priority"),
                    ("1".to_string(), "Focus priority"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1307,
            TagInfo {
                name: "AFAreaSelectMethod",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "AF area selection button"),
                    ("1".to_string(), "Main dial"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1308,
            TagInfo {
                name: "AutoAFPointColorTracking",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "On-Shot AF only"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1309,
            TagInfo {
                name: "VFDisplayIllumination",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1310,
            TagInfo {
                name: "InitialAFPointAIServoAF",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Auto"),
                    ("1".to_string(), "Initial AF point selected"),
                    ("2".to_string(), "Manual AF point"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1551,
            TagInfo {
                name: "MirrorLockup",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                    ("2".to_string(), "Enable: Down with Set"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1552,
            TagInfo {
                name: "ContinuousShootingSpeed",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1553,
            TagInfo {
                name: "ContinuousShotLimit",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1554,
            TagInfo {
                name: "RestrictDriveModes",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1793,
            TagInfo {
                name: "ShutterButtonAFOnButton",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Metering + AF start"),
                    ("1".to_string(), "Metering + AF start/AF stop"),
                    ("2".to_string(), "Metering start/Meter + AF start"),
                    ("3".to_string(), "AE lock/Metering + AF start"),
                    ("4".to_string(), "Metering + AF start/disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1794,
            TagInfo {
                name: "AFOnAELockButtonSwitch",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1795,
            TagInfo {
                name: "QuickControlDialInMeter",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Exposure comp/Aperture"),
                    ("1".to_string(), "AF point selection"),
                    ("2".to_string(), "ISO speed"),
                    (
                        "3".to_string(),
                        "AF point selection swapped with Exposure comp",
                    ),
                    ("4".to_string(), "ISO speed swapped with Exposure comp"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1796,
            TagInfo {
                name: "SetButtonWhenShooting",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal (disabled)"),
                    ("1".to_string(), "White balance"),
                    ("2".to_string(), "Image size"),
                    ("3".to_string(), "ISO speed"),
                    ("4".to_string(), "Picture style"),
                    ("5".to_string(), "Record func. + media/folder"),
                    ("6".to_string(), "Menu display"),
                    ("7".to_string(), "Image playback"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1797,
            TagInfo {
                name: "ManualTv",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Tv=Main/Av=Control"),
                    ("1".to_string(), "Tv=Control/Av=Main"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1798,
            TagInfo {
                name: "DialDirectionTvAv",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal"),
                    ("1".to_string(), "Reversed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1799,
            TagInfo {
                name: "AvSettingWithoutLens",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1800,
            TagInfo {
                name: "WBMediaImageSizeSetting",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Rear LCD panel"),
                    ("1".to_string(), "LCD monitor"),
                    ("2".to_string(), "Off (disable button)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1801,
            TagInfo {
                name: "LockMicrophoneButton",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1802,
            TagInfo {
                name: "ButtonFunctionControlOff",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal (enable)"),
                    ("1".to_string(), "Disable main, Control, Multi-control"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1803,
            TagInfo {
                name: "AssignFuncButton",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "LCD brightness"),
                    ("1".to_string(), "Image quality"),
                    ("2".to_string(), "Exposure comp./AEB setting"),
                    ("3".to_string(), "Image jump with main dial"),
                    ("4".to_string(), "Live view function settings"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1804,
            TagInfo {
                name: "CustomControls",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1805,
            TagInfo {
                name: "StartMovieShooting",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Default (from LV)"),
                    ("1".to_string(), "Quick start (FEL button)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1806,
            TagInfo {
                name: "FlashButtonFunction",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Raise built-in flash"),
                    ("1".to_string(), "ISO speed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1807,
            TagInfo {
                name: "MultiFunctionLock",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            1808,
            TagInfo {
                name: "TrashButtonFunction",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal (set center AF point)"),
                    ("1".to_string(), "Depth-of-field preview"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1809,
            TagInfo {
                name: "ShutterReleaseWithoutLens",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1810,
            TagInfo {
                name: "ControlRingRotation",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal"),
                    ("1".to_string(), "Reversed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1811,
            TagInfo {
                name: "FocusRingRotation",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Normal"),
                    ("1".to_string(), "Reversed"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1812,
            TagInfo {
                name: "RFLensMFFocusRingSensitivity",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Varies With Rotation Speed"),
                    ("1".to_string(), "Linked To Rotation Angle"),
                ]))),
                value_conv: None,
            },
        ),
        (
            1813,
            TagInfo {
                name: "CustomizeDials",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            2059,
            TagInfo {
                name: "FocusingScreen",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Ec-CIV"),
                    ("1".to_string(), "Ec-A,B,C,CII,CIII,D,H,I,L"),
                    ("2".to_string(), "Ec-S"),
                    ("3".to_string(), "Ec-N,R"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2060,
            TagInfo {
                name: "TimerLength",
                format: "unknown",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            2061,
            TagInfo {
                name: "ShortReleaseTimeLag",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2062,
            TagInfo {
                name: "AddAspectRatioInfo",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "6:6"),
                    ("2".to_string(), "3:4"),
                    ("3".to_string(), "4:5"),
                    ("4".to_string(), "6:7"),
                    ("5".to_string(), "10:12"),
                    ("6".to_string(), "5:7"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2063,
            TagInfo {
                name: "AddOriginalDecisionData",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "On"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2064,
            TagInfo {
                name: "LiveViewExposureSimulation",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable (LCD auto adjust)"),
                    ("1".to_string(), "Enable (simulates exposure)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2065,
            TagInfo {
                name: "LCDDisplayAtPowerOn",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Display"),
                    ("1".to_string(), "Retain power off status"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2066,
            TagInfo {
                name: "MemoAudioQuality",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "High (48 kHz)"),
                    ("1".to_string(), "Low (8 kHz)"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2067,
            TagInfo {
                name: "DefaultEraseOption",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Cancel selected"),
                    ("1".to_string(), "Erase selected"),
                    ("2".to_string(), "Erase RAW selected"),
                    ("3".to_string(), "Erase non-RAW selected"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2068,
            TagInfo {
                name: "RetractLensOnPowerOff",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2069,
            TagInfo {
                name: "AddIPTCInformation",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Disable"),
                    ("1".to_string(), "Enable"),
                ]))),
                value_conv: None,
            },
        ),
        (
            2070,
            TagInfo {
                name: "AudioCompression",
                format: "unknown",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Enable"),
                    ("1".to_string(), "Disable"),
                ]))),
                value_conv: None,
            },
        ),
    ])
});

/// Apply ValueConv transformation for tags in this table
pub fn apply_value_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
) -> Result<crate::types::TagValue, crate::types::ExifError> {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = CANON_CUSTOM_FUNCTIONS2_TAGS.get(&tag_id_u16) {
        if let Some(ref value_conv) = tag_def.value_conv {
            match value_conv {
                ValueConv::None => Ok(value.clone()),
                ValueConv::Function(func) => func(value, None),
                ValueConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    Err(crate::types::ExifError::NotImplemented("Runtime expression evaluation not supported - should be handled by PPI at build time".to_string()))
                }
                _ => Ok(value.clone()),
            }
        } else {
            Ok(value.clone())
        }
    } else {
        Err(crate::types::ExifError::ParseError(format!(
            "Tag 0x{:04x} not found in table",
            tag_id
        )))
    }
}

/// Apply PrintConv transformation for tags in this table
pub fn apply_print_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
    _warnings: &mut Vec<String>,
) -> crate::types::TagValue {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = CANON_CUSTOM_FUNCTIONS2_TAGS.get(&tag_id_u16) {
        if let Some(ref print_conv) = tag_def.print_conv {
            match print_conv {
                PrintConv::None => value.clone(),
                PrintConv::Function(func) => func(value, None),
                PrintConv::Simple(lookup) => {
                    // Look up value in the hash map
                    // ExifTool uses the stringified value as the key
                    let key = value.to_string();
                    if let Some(display_value) = lookup.get(&key) {
                        crate::types::TagValue::String(display_value.to_string())
                    } else {
                        // Key not found - return original value
                        value.clone()
                    }
                }
                PrintConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    value.clone() // Fallback to original value when expression not handled by PPI
                }
                _ => value.clone(),
            }
        } else {
            value.clone()
        }
    } else {
        value.clone()
    }
}
