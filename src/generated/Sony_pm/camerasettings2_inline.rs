...
pub static CAMERA_SETTINGS2_DRIVE_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_DRIVE_MODE_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FLASH_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_FLASH_MODE_DATA.iter().cloned().collect());
...
/// Raw data (2 entries)
static CAMERA_SETTINGS2_COLOR_SPACE_DATA: &[(u8, &'static str)] = &[(5, "Adobe RGB"), (6, "sRGB")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_COLOR_SPACE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_COLOR_SPACE_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_WHITE_BALANCE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_WHITE_BALANCE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FOCUS_MODE_SETTING: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_FOCUS_MODE_SETTING_DATA
            .iter()
            .cloned()
            .collect()
    });
...
static CAMERA_SETTINGS2_A_F_AREA_MODE_DATA: &[(u8, &'static str)] =
    &[(0, "Wide"), (1, "Local"), (2, "Spot")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_A_F_AREA_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_A_F_AREA_MODE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_A_F_POINT_SETTING: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_A_F_POINT_SETTING_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_METERING_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_METERING_MODE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Raw data (2 entries)
static CAMERA_SETTINGS2_HIGH_SPEED_SYNC_DATA: &[(u8, &'static str)] = &[(0, "Off"), (1, "On")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_HIGH_SPEED_SYNC: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_HIGH_SPEED_SYNC_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_DYNAMIC_RANGE_OPTIMIZER_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_DYNAMIC_RANGE_OPTIMIZER_MODE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
pub fn lookup_camera_settings2__dynamic_range_optimizer_mode(key: u8) -> Option<&'static str> {
    CAMERA_SETTINGS2_DYNAMIC_RANGE_OPTIMIZER_MODE
        .get(&key)
        .copied()
}
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_CREATIVE_STYLE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_CREATIVE_STYLE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
static CAMERA_SETTINGS2_FLASH_CONTROL_DATA: &[(u8, &'static str)] =
    &[(0, "ADI"), (1, "Pre-flash TTL"), (2, "Manual")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FLASH_CONTROL: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_FLASH_CONTROL_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_LONG_EXPOSURE_NOISE_REDUCTION: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_LONG_EXPOSURE_NOISE_REDUCTION_DATA
            .iter()
            .cloned()
            .collect()
    });
...
pub fn lookup_camera_settings2__long_exposure_noise_reduction(key: u8) -> Option<&'static str> {
    CAMERA_SETTINGS2_LONG_EXPOSURE_NOISE_REDUCTION
        .get(&key)
        .copied()
}
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_HIGH_I_S_O_NOISE_REDUCTION: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_HIGH_I_S_O_NOISE_REDUCTION_DATA
            .iter()
            .cloned()
            .collect()
    });
...
pub fn lookup_camera_settings2__high_i_s_o_noise_reduction(key: u8) -> Option<&'static str> {
    CAMERA_SETTINGS2_HIGH_I_S_O_NOISE_REDUCTION
        .get(&key)
        .copied()
}
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_IMAGE_STYLE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_IMAGE_STYLE_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_WHITE_BALANCE_SETTING: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_WHITE_BALANCE_SETTING_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_EXPOSURE_PROGRAM: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_EXPOSURE_PROGRAM_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_IMAGE_STABILIZATION_SETTING: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_IMAGE_STABILIZATION_SETTING_DATA
            .iter()
            .cloned()
            .collect()
    });
...
pub fn lookup_camera_settings2__image_stabilization_setting(key: u8) -> Option<&'static str> {
    CAMERA_SETTINGS2_IMAGE_STABILIZATION_SETTING
        .get(&key)
        .copied()
}
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FLASH_ACTION: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_FLASH_ACTION_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_ROTATION: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_ROTATION_DATA.iter().cloned().collect());
...
/// Raw data (2 entries)
static CAMERA_SETTINGS2_A_E_LOCK_DATA: &[(u8, &'static str)] = &[(1, "Off"), (2, "On")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_A_E_LOCK: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_A_E_LOCK_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FLASH_ACTION2: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_FLASH_ACTION2_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FOCUS_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_FOCUS_MODE_DATA.iter().cloned().collect());
...
static CAMERA_SETTINGS2_FOCUS_STATUS_DATA: &[(u8, &'static str)] =
    &[(0, "Not confirmed"), (4, "Not confirmed, Tracking")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_FOCUS_STATUS: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_FOCUS_STATUS_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_SONY_IMAGE_SIZE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_SONY_IMAGE_SIZE_DATA
            .iter()
            .cloned()
            .collect()
    });
...
/// Raw data (2 entries)
static CAMERA_SETTINGS2_ASPECT_RATIO_DATA: &[(u8, &'static str)] = &[(1, "3:2"), (2, "16:9")];

/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_ASPECT_RATIO: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_ASPECT_RATIO_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_QUALITY: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| CAMERA_SETTINGS2_QUALITY_DATA.iter().cloned().collect());
...
/// Lookup table (lazy-initialized)
pub static CAMERA_SETTINGS2_EXPOSURE_LEVEL_INCREMENTS: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| {
        CAMERA_SETTINGS2_EXPOSURE_LEVEL_INCREMENTS_DATA
            .iter()
            .cloned()
            .collect()
    });
...
pub fn lookup_camera_settings2__exposure_level_increments(key: u8) -> Option<&'static str> {
    CAMERA_SETTINGS2_EXPOSURE_LEVEL_INCREMENTS
        .get(&key)
        .copied()
}
