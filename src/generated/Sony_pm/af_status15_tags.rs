//! Generated tag table definitions from Sony::AFStatus15
//!
//! This file is auto-generated by codegen/src/strategies/tag_kit.rs. Do not edit manually.

use crate::types::{PrintConv, TagInfo, ValueConv};
use std::collections::HashMap;
use std::sync::LazyLock;

/// Tag definitions for Sony::AFStatus15 table
pub static SONY_AFSTATUS15_TAGS: LazyLock<HashMap<u16, TagInfo>> = LazyLock::new(|| {
    HashMap::from([
        (
            0,
            TagInfo {
                name: "AFStatusUpper-left",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            2,
            TagInfo {
                name: "AFStatusLeft",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            4,
            TagInfo {
                name: "AFStatusLower-left",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            6,
            TagInfo {
                name: "AFStatusFarLeft",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            8,
            TagInfo {
                name: "AFStatusTopHorizontal",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            10,
            TagInfo {
                name: "AFStatusNearRight",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            12,
            TagInfo {
                name: "AFStatusCenterHorizontal",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            14,
            TagInfo {
                name: "AFStatusNearLeft",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            16,
            TagInfo {
                name: "AFStatusBottomHorizontal",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            18,
            TagInfo {
                name: "AFStatusTopVertical",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            20,
            TagInfo {
                name: "AFStatusCenterVertical",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            22,
            TagInfo {
                name: "AFStatusBottomVertical",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            24,
            TagInfo {
                name: "AFStatusFarRight",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            26,
            TagInfo {
                name: "AFStatusUpper-right",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            28,
            TagInfo {
                name: "AFStatusRight",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            30,
            TagInfo {
                name: "AFStatusLower-right",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            32,
            TagInfo {
                name: "AFStatusUpper-middle",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
        (
            34,
            TagInfo {
                name: "AFStatusLower-middle",
                format: "int16s",
                print_conv: Some(PrintConv::Complex),
                value_conv: None,
            },
        ),
    ])
});

/// Apply ValueConv transformation for tags in this table
pub fn apply_value_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
) -> Result<crate::types::TagValue, crate::types::ExifError> {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = SONY_AFSTATUS15_TAGS.get(&tag_id_u16) {
        if let Some(ref value_conv) = tag_def.value_conv {
            match value_conv {
                ValueConv::None => Ok(value.clone()),
                ValueConv::Function(func) => func(value, None),
                ValueConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    Err(crate::types::ExifError::NotImplemented("Runtime expression evaluation not supported - should be handled by PPI at build time".to_string()))
                }
                _ => Ok(value.clone()),
            }
        } else {
            Ok(value.clone())
        }
    } else {
        Err(crate::types::ExifError::ParseError(format!(
            "Tag 0x{:04x} not found in table",
            tag_id
        )))
    }
}

/// Apply PrintConv transformation for tags in this table
pub fn apply_print_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
    _warnings: &mut Vec<String>,
) -> crate::types::TagValue {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = SONY_AFSTATUS15_TAGS.get(&tag_id_u16) {
        if let Some(ref print_conv) = tag_def.print_conv {
            match print_conv {
                PrintConv::None => value.clone(),
                PrintConv::Function(func) => func(value, None),
                PrintConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    value.clone() // Fallback to original value when expression not handled by PPI
                }
                _ => value.clone(),
            }
        } else {
            value.clone()
        }
    } else {
        value.clone()
    }
}
