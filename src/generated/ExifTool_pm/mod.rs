...
pub static MIME_TYPES: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    ...
    map.insert("DOTM", "application/vnd.ms-word.template.macroEnabledTemplate");
    ...
    map.insert("PPAX", "application/vnd.openxmlformats-officedocument.presentationml.addin");
...
/// Look up MIME_TYPES value by key
...
/// Look up FILE_TYPE_EXTENSIONS value by key
pub fn lookup_file_type_extensions(key: &str) -> Option<&'static str> {
    FILE_TYPE_EXTENSIONS.get(&key).copied()
}

// Boolean sets
// File types with weak magic numbers that defer to extension
// Source: ExifTool ExifTool.pm %weakMagic (1 entries)
pub static WEAK_MAGIC_TYPES: LazyLock<HashSet<String>> = LazyLock::new(|| {
    let mut set = HashSet::new();
    set.insert("MP3".to_string());
    set
});

/// Check if key is in WEAK_MAGIC_TYPES
pub fn is_weak_magic_types(key: &str) -> bool {
    WEAK_MAGIC_TYPES.contains(key)
}
