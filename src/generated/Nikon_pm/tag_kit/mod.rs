//! Modular tag kits with embedded PrintConv for Nikon_pm
//!
//! This file is automatically generated by codegen.
//! DO NOT EDIT MANUALLY - changes will be overwritten.
//!
//! Generated from: Nikon.pm table: AFInfo
//!
#![allow(unused_imports)]
#![allow(unused_mut)]
#![allow(dead_code)]
#![allow(unused_variables)]

pub mod camera;
pub mod color;
pub mod core;
pub mod datetime;
pub mod document;
pub mod gps;
pub mod interop;
pub mod other;
pub mod thumbnail;

use crate::expressions::ExpressionEvaluator;
use crate::tiff_types::ByteOrder;
use crate::types::{Result, TagValue};
use std::collections::HashMap;
use std::sync::LazyLock;

#[derive(Debug, Clone)]
pub struct TagKitDef {
    pub id: u32,
    pub name: &'static str,
    pub format: &'static str,
    pub groups: HashMap<&'static str, &'static str>,
    pub writable: bool,
    pub notes: Option<&'static str>,
    pub print_conv: PrintConvType,
    pub value_conv: Option<&'static str>,
    pub subdirectory: Option<SubDirectoryType>,
}

#[derive(Debug, Clone)]
pub enum PrintConvType {
    None,
    Simple(&'static HashMap<String, &'static str>),
    Expression(&'static str),
    Manual(&'static str),
}

/// Type alias for subdirectory processor function
pub type SubDirectoryProcessor =
    fn(&[u8], ByteOrder, Option<&str>) -> Result<Vec<(String, TagValue)>>;

#[derive(Debug, Clone)]
pub enum SubDirectoryType {
    Binary { processor: SubDirectoryProcessor },
}

/// All tag kits for Nikon_pm
pub static NIKON_PM_TAG_KITS: LazyLock<HashMap<u32, TagKitDef>> = LazyLock::new(|| {
    let mut map: HashMap<u32, TagKitDef> = HashMap::new();

    // camera tags
    for (id, tag_def) in camera::get_camera_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // color tags
    for (id, tag_def) in color::get_color_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // core tags
    for (id, tag_def) in core::get_core_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // datetime tags
    for (id, tag_def) in datetime::get_datetime_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // document tags
    for (id, tag_def) in document::get_document_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // gps tags
    for (id, tag_def) in gps::get_gps_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // interop tags
    for (id, tag_def) in interop::get_interop_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // other tags
    for (id, tag_def) in other::get_other_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    // thumbnail tags
    for (id, tag_def) in thumbnail::get_thumbnail_tags() {
        // Priority insertion: preserve existing entries with subdirectory processors
        match map.get(&id) {
            Some(existing) if existing.subdirectory.is_some() => {
                // Keep existing tag if it has a subdirectory processor
                if tag_def.subdirectory.is_none() {
                    // Skip this tag - existing one is more important
                    continue;
                }
            }
            _ => {}
        }
        map.insert(id, tag_def);
    }

    map
});

// Helper functions for reading binary data
fn model_matches(model: &str, pattern: &str) -> bool {
    // ExifTool regexes are already in a compatible format
    // Just need to ensure proper escaping was preserved

    match regex::Regex::new(pattern) {
        Ok(re) => re.is_match(model),
        Err(e) => {
            tracing::warn!("Failed to compile model regex '{}': {}", pattern, e);
            false
        }
    }
}

fn format_matches(format: &str, pattern: &str) -> bool {
    if let Ok(re) = regex::Regex::new(pattern) {
        re.is_match(format)
    } else {
        tracing::warn!("Failed to compile format regex: {}", pattern);
        false
    }
}

fn read_int16s_array(data: &[u8], byte_order: ByteOrder, count: usize) -> Result<Vec<i16>> {
    if data.len() < count * 2 {
        return Err(crate::types::ExifError::ParseError(
            "Insufficient data for int16s array".to_string(),
        ));
    }
    let mut values = Vec::with_capacity(count);
    for i in 0..count {
        let offset = i * 2;
        let value = match byte_order {
            ByteOrder::LittleEndian => i16::from_le_bytes([data[offset], data[offset + 1]]),
            ByteOrder::BigEndian => i16::from_be_bytes([data[offset], data[offset + 1]]),
        };
        values.push(value);
    }
    Ok(values)
}

fn read_int16u_array(data: &[u8], byte_order: ByteOrder, count: usize) -> Result<Vec<u16>> {
    if data.len() < count * 2 {
        return Err(crate::types::ExifError::ParseError(
            "Insufficient data for int16u array".to_string(),
        ));
    }
    let mut values = Vec::with_capacity(count);
    for i in 0..count {
        let offset = i * 2;
        let value = match byte_order {
            ByteOrder::LittleEndian => u16::from_le_bytes([data[offset], data[offset + 1]]),
            ByteOrder::BigEndian => u16::from_be_bytes([data[offset], data[offset + 1]]),
        };
        values.push(value);
    }
    Ok(values)
}

fn read_int16s(data: &[u8], byte_order: ByteOrder) -> Result<i16> {
    if data.len() < 2 {
        return Err(crate::types::ExifError::ParseError(
            "Insufficient data for int16s".to_string(),
        ));
    }
    Ok(match byte_order {
        ByteOrder::LittleEndian => i16::from_le_bytes([data[0], data[1]]),
        ByteOrder::BigEndian => i16::from_be_bytes([data[0], data[1]]),
    })
}

// Subdirectory processing functions
fn process_nikon_avitags(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ExposureCompensation at offset 10
    if data.len() >= 22 {
        // TODO: Handle format rational64s
    }

    // MaxApertureValue at offset 11
    if data.len() >= 24 {
        // TODO: Handle format rational64u
    }

    // MeteringMode at offset 12
    if data.len() >= 26 {
        // TODO: Handle format int16u
    }

    // Nikon_AVITags_0x000d at offset 13
    if data.len() >= 28 {
        // TODO: Handle format int16u
    }

    // Nikon_AVITags_0x000e at offset 14
    if data.len() >= 30 {
        // TODO: Handle format int16u
    }

    // FocalLength at offset 15
    if data.len() >= 32 {
        // TODO: Handle format rational64u
    }

    // XResolution at offset 16
    if data.len() >= 34 {
        // TODO: Handle format rational64u
    }

    // YResolution at offset 17
    if data.len() >= 36 {
        // TODO: Handle format rational64u
    }

    // ResolutionUnit at offset 18
    if data.len() >= 38 {
        // TODO: Handle format int16u
    }

    // DateTimeOriginal at offset 19

    // CreateDate at offset 20

    // Nikon_AVITags_0x0015 at offset 21
    if data.len() >= 44 {
        // TODO: Handle format int16u
    }

    // Duration at offset 22
    if data.len() >= 46 {
        // TODO: Handle format rational64u
    }

    // Nikon_AVITags_0x0017 at offset 23
    if data.len() >= 48 {
        // TODO: Handle format int16u
    }

    // Nikon_AVITags_0x0019 at offset 25
    if data.len() >= 54 {
        // TODO: Handle format int32s
    }

    // DigitalZoom at offset 27
    if data.len() >= 56 {
        // TODO: Handle format rational64u
    }

    // Nikon_AVITags_0x001c at offset 28
    if data.len() >= 58 {
        // TODO: Handle format rational64u
    }

    // Sharpness at offset 30

    // WhiteBalance at offset 31

    // NoiseReduction at offset 32

    // Nikon_AVITags_0x801a at offset 32794
    if data.len() >= 65592 {
        // TODO: Handle format int32s
    }

    // Software at offset 5
    if data.len() >= 12 {
        // TODO: Handle format undef
    }

    // Orientation at offset 7
    if data.len() >= 16 {
        // TODO: Handle format int16u
    }

    // ExposureTime at offset 8
    if data.len() >= 18 {
        // TODO: Handle format rational64u
    }

    // FNumber at offset 9
    if data.len() >= 20 {
        // TODO: Handle format rational64u
    }

    Ok(tags)
}

fn process_nikon_avivers(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MakerNoteVersion at offset 2
    if data.len() >= 5 {
        // TODO: Handle format int8u
    }

    Ok(tags)
}

fn process_nikon_menusettingsz8(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MultipleExposureMode at offset 152

    // MultiExposureShots at offset 154

    // IntervalDurationHours at offset 184
    if data.len() >= 372 {
        // TODO: Handle format int32u
    }

    // IntervalDurationMinutes at offset 188
    if data.len() >= 380 {
        // TODO: Handle format int32u
    }

    // IntervalDurationSeconds at offset 192
    if data.len() >= 388 {
        // TODO: Handle format int32u
    }

    // Intervals at offset 200
    if data.len() >= 404 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 204
    if data.len() >= 412 {
        // TODO: Handle format int32u
    }

    // IntervalExposureSmoothing at offset 208
    if data.len() >= 417 {
        // TODO: Handle format int8u
    }

    // IntervalPriority at offset 210
    if data.len() >= 421 {
        // TODO: Handle format int8u
    }

    // FocusShiftNumberShots at offset 244

    // FocusShiftStepWidth at offset 248

    // FocusShiftInterval at offset 252

    // FocusShiftExposureLock at offset 256

    // PhotoShootingMenuBank at offset 286

    // ExtendedMenuBanks at offset 288

    // PhotoShootingMenuBankImageArea at offset 324

    // AutoISO at offset 338

    // ISOAutoHiLimit at offset 340
    if data.len() >= 682 {
        // TODO: Handle format int16u
    }

    // ISOAutoFlashLimit at offset 342
    if data.len() >= 686 {
        // TODO: Handle format int16u
    }

    // ISOAutoShutterTime at offset 350
    if data.len() >= 702 {
        if let Ok(value) = read_int16s(&data[700..702], byte_order) {
            tags.push(("ISOAutoShutterTime".to_string(), TagValue::I16(value)));
        }
    }

    // MovieVignetteControl at offset 432

    // DiffractionCompensation at offset 434

    // FlickerReductionShooting at offset 436

    // FlashControlMode at offset 440

    // AFAreaMode at offset 548

    // VRMode at offset 550

    // BracketSet at offset 554

    // BracketProgram at offset 556

    // BracketIncrement at offset 558

    // HDR at offset 570

    // SecondarySlotFunction at offset 576

    // HDRLevel at offset 582

    // Slot2JpgSize at offset 586

    // DXCropAlert at offset 592

    // SubjectDetection at offset 594

    // DynamicAFAreaSize at offset 596

    // ToneMap at offset 618

    // PortraitImpressionBalance at offset 622

    // HighFrequencyFlickerReduction at offset 636

    // HighFrameRate at offset 72

    // MovieImageArea at offset 730

    // MovieType at offset 740

    // MovieISOAutoHiLimit at offset 742
    if data.len() >= 1486 {
        // TODO: Handle format int16u
    }

    // MovieISOAutoControlManualMode at offset 744

    // MovieISOAutoManualMode at offset 746
    if data.len() >= 1494 {
        // TODO: Handle format int16u
    }

    // MovieActiveD-Lighting at offset 820

    // MovieHighISONoiseReduction at offset 822

    // MovieFlickerReduction at offset 828

    // MovieMeteringMode at offset 830

    // MovieFocusMode at offset 832

    // MovieAFAreaMode at offset 834

    // MovieVRMode at offset 836

    // MovieElectronicVR at offset 840

    // MovieSoundRecording at offset 842

    // MicrophoneSensitivity at offset 844

    // MicrophoneAttenuator at offset 846

    // MicrophoneFrequencyResponse at offset 848

    // WindNoiseReduction at offset 850

    // MovieFrameSize at offset 882

    // MovieFrameRate at offset 884

    // MicrophoneJackPower at offset 886

    // MovieDXCropAlert at offset 887

    // MovieSubjectDetection at offset 888

    // MovieHighResZoom at offset 896

    Ok(tags)
}

fn process_nikon_nineedits(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // TagfilterParametersBinary at offset filterParametersBinary

    // TagfilterParametersCustomCustomData at offset filterParametersCustomCustomData

    // TagfilterParametersExportExportData at offset filterParametersExportExportData

    Ok(tags)
}

fn process_nikon_distortioninfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // DistortionCorrectionVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // RadialDistortionCoefficient1 at offset 20
    if data.len() >= 42 {
        // TODO: Handle format rational64s
    }

    // RadialDistortionCoefficient2 at offset 28
    if data.len() >= 58 {
        // TODO: Handle format rational64s
    }

    // RadialDistortionCoefficient3 at offset 36
    if data.len() >= 74 {
        // TODO: Handle format rational64s
    }

    // DistortionCorrection at offset 4
    if data.len() >= 9 {
        // TODO: Handle format int8u
    }

    Ok(tags)
}

fn process_nikon_vignetteinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // VignetteCorrectionVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // VignetteCoefficient1 at offset 36
    if data.len() >= 74 {
        // TODO: Handle format rational64s
    }

    // VignetteCoefficient2 at offset 52
    if data.len() >= 106 {
        // TODO: Handle format rational64s
    }

    // VignetteCoefficient3 at offset 68
    if data.len() >= 138 {
        // TODO: Handle format rational64s
    }

    Ok(tags)
}

fn process_nikon_menusettingsz7ii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // IntervalDurationHours at offset 160
    if data.len() >= 324 {
        // TODO: Handle format int32u
    }

    // IntervalDurationMinutes at offset 164
    if data.len() >= 332 {
        // TODO: Handle format int32u
    }

    // IntervalDurationSeconds at offset 168
    if data.len() >= 340 {
        // TODO: Handle format int32u
    }

    // Intervals at offset 176
    if data.len() >= 356 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 180
    if data.len() >= 364 {
        // TODO: Handle format int32u
    }

    // IntervalExposureSmoothing at offset 184
    if data.len() >= 369 {
        // TODO: Handle format int8u
    }

    // IntervalPriority at offset 186
    if data.len() >= 373 {
        // TODO: Handle format int8u
    }

    // FocusShiftNumberShots at offset 220

    // FocusShiftStepWidth at offset 224

    // FocusShiftInterval at offset 228

    // FocusShiftExposureLock at offset 232

    // DiffractionCompensation at offset 322
    if data.len() >= 645 {
        // TODO: Handle format int8u
    }

    // AutoDistortionControl at offset 323
    if data.len() >= 647 {
        // TODO: Handle format int8u
    }

    // NikonMeteringMode at offset 326

    // FlashControlMode at offset 328

    // FlashGNDistance at offset 334

    // FlashOutput at offset 338

    // FlashWirelessOption at offset 346

    // FlashRemoteControl at offset 348

    // FlashMasterControlMode at offset 352

    // FlashMasterCompensation at offset 354
    if data.len() >= 709 {
        // TODO: Handle format int8s
    }

    // FlashMasterOutput at offset 358

    // MovieFrameSize at offset 502

    // MovieFrameRate at offset 504

    // MovieSlowMotion at offset 506

    // MovieType at offset 510

    // MovieISOAutoManualMode at offset 516
    if data.len() >= 1034 {
        // TODO: Handle format int16u
    }

    // MovieActiveD-Lighting at offset 568

    // MovieHighISONoiseReduction at offset 572

    // MovieVignetteControl at offset 574

    // MovieVignetteControlSameAsPhoto at offset 576

    // MovieDiffractionCompensation at offset 577

    // MovieAutoDistortionControl at offset 578

    // MovieFocusMode at offset 584

    // MovieVibrationReduction at offset 590

    // MovieVibrationReductionSameAsPhoto at offset 591

    // HDMIOutputN-Log at offset 858

    // SingleFrame at offset 90

    // ReleaseMode at offset 92

    Ok(tags)
}

fn process_nikon_menusettingsz8v1(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MenuSettingsZ8 at offset 0
    if data.len() >= 1886 {
        // TODO: Handle format undef
    }

    // TimeZone at offset 1684

    // MonitorBrightness at offset 1690

    // Language at offset 1698

    // AFFineTune at offset 1712

    // NonCPULens1FocalLength at offset 1716
    if data.len() >= 3434 {
        // TODO: Handle format int16u
    }

    // NonCPULens2FocalLength at offset 1718
    if data.len() >= 3438 {
        // TODO: Handle format int16u
    }

    // NonCPULens3FocalLength at offset 1720
    if data.len() >= 3442 {
        // TODO: Handle format int16u
    }

    // NonCPULens4FocalLength at offset 1722
    if data.len() >= 3446 {
        // TODO: Handle format int16u
    }

    // NonCPULens5FocalLength at offset 1724
    if data.len() >= 3450 {
        // TODO: Handle format int16u
    }

    // NonCPULens6FocalLength at offset 1726
    if data.len() >= 3454 {
        // TODO: Handle format int16u
    }

    // NonCPULens7FocalLength at offset 1728
    if data.len() >= 3458 {
        // TODO: Handle format int16u
    }

    // NonCPULens8FocalLength at offset 1730
    if data.len() >= 3462 {
        // TODO: Handle format int16u
    }

    // NonCPULens9FocalLength at offset 1732
    if data.len() >= 3466 {
        // TODO: Handle format int16u
    }

    // NonCPULens10FocalLength at offset 1734
    if data.len() >= 3470 {
        // TODO: Handle format int16u
    }

    // NonCPULens11FocalLength at offset 1736
    if data.len() >= 3474 {
        // TODO: Handle format int16u
    }

    // NonCPULens12FocalLength at offset 1738
    if data.len() >= 3478 {
        // TODO: Handle format int16u
    }

    // NonCPULens13FocalLength at offset 1740
    if data.len() >= 3482 {
        // TODO: Handle format int16u
    }

    // NonCPULens14FocalLength at offset 1742
    if data.len() >= 3486 {
        // TODO: Handle format int16u
    }

    // NonCPULens15FocalLength at offset 1744
    if data.len() >= 3490 {
        // TODO: Handle format int16u
    }

    // NonCPULens16FocalLength at offset 1746
    if data.len() >= 3494 {
        // TODO: Handle format int16u
    }

    // NonCPULens17FocalLength at offset 1748
    if data.len() >= 3498 {
        // TODO: Handle format int16u
    }

    // NonCPULens18FocalLength at offset 1750
    if data.len() >= 3502 {
        // TODO: Handle format int16u
    }

    // NonCPULens19FocalLength at offset 1752
    if data.len() >= 3506 {
        // TODO: Handle format int16u
    }

    // NonCPULens20FocalLength at offset 1754
    if data.len() >= 3510 {
        // TODO: Handle format int16u
    }

    // NonCPULens1MaxAperture at offset 1756
    if data.len() >= 3514 {
        // TODO: Handle format int16u
    }

    // NonCPULens2MaxAperture at offset 1758
    if data.len() >= 3518 {
        // TODO: Handle format int16u
    }

    // NonCPULens3MaxAperture at offset 1760
    if data.len() >= 3522 {
        // TODO: Handle format int16u
    }

    // NonCPULens4MaxAperture at offset 1762
    if data.len() >= 3526 {
        // TODO: Handle format int16u
    }

    // NonCPULens5MaxAperture at offset 1764
    if data.len() >= 3530 {
        // TODO: Handle format int16u
    }

    // NonCPULens6MaxAperture at offset 1766
    if data.len() >= 3534 {
        // TODO: Handle format int16u
    }

    // NonCPULens7MaxAperture at offset 1768
    if data.len() >= 3538 {
        // TODO: Handle format int16u
    }

    // NonCPULens8MaxAperture at offset 1770
    if data.len() >= 3542 {
        // TODO: Handle format int16u
    }

    // NonCPULens9MaxAperture at offset 1772
    if data.len() >= 3546 {
        // TODO: Handle format int16u
    }

    // NonCPULens10MaxAperture at offset 1774
    if data.len() >= 3550 {
        // TODO: Handle format int16u
    }

    // NonCPULens11MaxAperture at offset 1776
    if data.len() >= 3554 {
        // TODO: Handle format int16u
    }

    // NonCPULens12MaxAperture at offset 1778
    if data.len() >= 3558 {
        // TODO: Handle format int16u
    }

    // NonCPULens13MaxAperture at offset 1780
    if data.len() >= 3562 {
        // TODO: Handle format int16u
    }

    // NonCPULens14MaxAperture at offset 1782
    if data.len() >= 3566 {
        // TODO: Handle format int16u
    }

    // NonCPULens15MaxAperture at offset 1784
    if data.len() >= 3570 {
        // TODO: Handle format int16u
    }

    // NonCPULens16MaxAperture at offset 1786
    if data.len() >= 3574 {
        // TODO: Handle format int16u
    }

    // NonCPULens17MaxAperture at offset 1788
    if data.len() >= 3578 {
        // TODO: Handle format int16u
    }

    // NonCPULens18MaxAperture at offset 1790
    if data.len() >= 3582 {
        // TODO: Handle format int16u
    }

    // NonCPULens19MaxAperture at offset 1792
    if data.len() >= 3586 {
        // TODO: Handle format int16u
    }

    // NonCPULens20MaxAperture at offset 1794
    if data.len() >= 3590 {
        // TODO: Handle format int16u
    }

    // HDMIOutputResolution at offset 1808

    // AirplaneMode at offset 1826

    // EmptySlotRelease at offset 1827

    // EnergySavingMode at offset 1862

    // USBPowerDelivery at offset 1890

    // SensorShield at offset 1899

    // CustomSettingsZ8 at offset 943
    if data.len() >= 3346 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_menusettingsz8v2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MenuSettingsZ8 at offset 0
    if data.len() >= 1886 {
        // TODO: Handle format undef
    }

    // Language at offset 1698

    // TimeZone at offset 1700

    // MonitorBrightness at offset 1706

    // AFFineTune at offset 1728

    // NonCPULens1FocalLength at offset 1732
    if data.len() >= 3466 {
        // TODO: Handle format int16u
    }

    // NonCPULens2FocalLength at offset 1734
    if data.len() >= 3470 {
        // TODO: Handle format int16u
    }

    // NonCPULens3FocalLength at offset 1736
    if data.len() >= 3474 {
        // TODO: Handle format int16u
    }

    // NonCPULens4FocalLength at offset 1738
    if data.len() >= 3478 {
        // TODO: Handle format int16u
    }

    // NonCPULens5FocalLength at offset 1740
    if data.len() >= 3482 {
        // TODO: Handle format int16u
    }

    // NonCPULens6FocalLength at offset 1742
    if data.len() >= 3486 {
        // TODO: Handle format int16u
    }

    // NonCPULens7FocalLength at offset 1744
    if data.len() >= 3490 {
        // TODO: Handle format int16u
    }

    // NonCPULens8FocalLength at offset 1746
    if data.len() >= 3494 {
        // TODO: Handle format int16u
    }

    // NonCPULens9FocalLength at offset 1748
    if data.len() >= 3498 {
        // TODO: Handle format int16u
    }

    // NonCPULens10FocalLength at offset 1750
    if data.len() >= 3502 {
        // TODO: Handle format int16u
    }

    // NonCPULens11FocalLength at offset 1752
    if data.len() >= 3506 {
        // TODO: Handle format int16u
    }

    // NonCPULens12FocalLength at offset 1754
    if data.len() >= 3510 {
        // TODO: Handle format int16u
    }

    // NonCPULens13FocalLength at offset 1756
    if data.len() >= 3514 {
        // TODO: Handle format int16u
    }

    // NonCPULens14FocalLength at offset 1758
    if data.len() >= 3518 {
        // TODO: Handle format int16u
    }

    // NonCPULens15FocalLength at offset 1760
    if data.len() >= 3522 {
        // TODO: Handle format int16u
    }

    // NonCPULens16FocalLength at offset 1762
    if data.len() >= 3526 {
        // TODO: Handle format int16u
    }

    // NonCPULens17FocalLength at offset 1764
    if data.len() >= 3530 {
        // TODO: Handle format int16u
    }

    // NonCPULens18FocalLength at offset 1766
    if data.len() >= 3534 {
        // TODO: Handle format int16u
    }

    // NonCPULens19FocalLength at offset 1768
    if data.len() >= 3538 {
        // TODO: Handle format int16u
    }

    // NonCPULens20FocalLength at offset 1770
    if data.len() >= 3542 {
        // TODO: Handle format int16u
    }

    // NonCPULens1MaxAperture at offset 1812
    if data.len() >= 3628 {
        // TODO: Handle format int32u
    }

    // NonCPULens2MaxAperture at offset 1816
    if data.len() >= 3636 {
        // TODO: Handle format int32u
    }

    // NonCPULens3MaxAperture at offset 1820
    if data.len() >= 3644 {
        // TODO: Handle format int32u
    }

    // NonCPULens4MaxAperture at offset 1824
    if data.len() >= 3652 {
        // TODO: Handle format int32u
    }

    // NonCPULens5MaxAperture at offset 1828
    if data.len() >= 3660 {
        // TODO: Handle format int32u
    }

    // NonCPULens6MaxAperture at offset 1832
    if data.len() >= 3668 {
        // TODO: Handle format int32u
    }

    // NonCPULens7MaxAperture at offset 1836
    if data.len() >= 3676 {
        // TODO: Handle format int32u
    }

    // NonCPULens8MaxAperture at offset 1840
    if data.len() >= 3684 {
        // TODO: Handle format int32u
    }

    // NonCPULens9MaxAperture at offset 1844
    if data.len() >= 3692 {
        // TODO: Handle format int32u
    }

    // NonCPULens10MaxAperture at offset 1848
    if data.len() >= 3700 {
        // TODO: Handle format int32u
    }

    // NonCPULens11MaxAperture at offset 1852
    if data.len() >= 3708 {
        // TODO: Handle format int32u
    }

    // NonCPULens12MaxAperture at offset 1856
    if data.len() >= 3716 {
        // TODO: Handle format int32u
    }

    // NonCPULens13MaxAperture at offset 1860
    if data.len() >= 3724 {
        // TODO: Handle format int32u
    }

    // NonCPULens14MaxAperture at offset 1864
    if data.len() >= 3732 {
        // TODO: Handle format int32u
    }

    // NonCPULens15MaxAperture at offset 1868
    if data.len() >= 3740 {
        // TODO: Handle format int32u
    }

    // NonCPULens16MaxAperture at offset 1872
    if data.len() >= 3748 {
        // TODO: Handle format int32u
    }

    // NonCPULens17MaxAperture at offset 1876
    if data.len() >= 3756 {
        // TODO: Handle format int32u
    }

    // NonCPULens18MaxAperture at offset 1880
    if data.len() >= 3764 {
        // TODO: Handle format int32u
    }

    // NonCPULens19MaxAperture at offset 1884
    if data.len() >= 3772 {
        // TODO: Handle format int32u
    }

    // NonCPULens20MaxAperture at offset 1888
    if data.len() >= 3780 {
        // TODO: Handle format int32u
    }

    // HDMIOutputResolution at offset 1904

    // AirplaneMode at offset 1922

    // EmptySlotRelease at offset 1923

    // EnergySavingMode at offset 1958

    // USBPowerDelivery at offset 1986

    // SensorShield at offset 1995

    // PixelShiftShooting at offset 2046

    // PixelShiftNumberShots at offset 2048

    // PixelShiftDelay at offset 2050

    // PlaybackButton at offset 2052

    // WBButton at offset 2054

    // BracketButton at offset 2056

    // LensFunc1ButtonPlaybackMode at offset 2058

    // LensFunc2ButtonPlaybackMode at offset 2060

    // PlaybackButtonPlaybackMode at offset 2062

    // BracketButtonPlaybackMode at offset 2064

    // MaximumApertureLV at offset 2206

    // ReleaseModeButton at offset 2208

    // ReleaseModeButtonPlaybackMode at offset 2216

    // CustomSettingsZ8 at offset 943
    if data.len() >= 3396 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_menusettingsz9(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MultipleExposureMode at offset 140

    // MultiExposureShots at offset 142

    // Language at offset 1426

    // TimeZone at offset 1428

    // MonitorBrightness at offset 1434

    // AFFineTune at offset 1456

    // HDMIOutputResolution at offset 1552

    // SetClockFromLocationData at offset 1565

    // AirplaneMode at offset 1572

    // EmptySlotRelease at offset 1573

    // EnergySavingMode at offset 1608

    // RecordLocationData at offset 1632

    // USBPowerDelivery at offset 1636

    // SensorShield at offset 1645

    // Intervals at offset 188
    if data.len() >= 380 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 192
    if data.len() >= 388 {
        // TODO: Handle format int32u
    }

    // FocusShiftNumberShots at offset 232

    // FocusShiftStepWidth at offset 236

    // FocusShiftInterval at offset 240

    // FocusShiftExposureLock at offset 244

    // PhotoShootingMenuBank at offset 274

    // ExtendedMenuBanks at offset 276

    // PhotoShootingMenuBankImageArea at offset 308

    // AutoISO at offset 322

    // ISOAutoHiLimit at offset 324
    if data.len() >= 650 {
        // TODO: Handle format int16u
    }

    // ISOAutoFlashLimit at offset 326
    if data.len() >= 654 {
        // TODO: Handle format int16u
    }

    // ISOAutoShutterTime at offset 334
    if data.len() >= 670 {
        if let Ok(value) = read_int16s(&data[668..670], byte_order) {
            tags.push(("ISOAutoShutterTime".to_string(), TagValue::I16(value)));
        }
    }

    // MovieVignetteControl at offset 416

    // DiffractionCompensation at offset 418

    // FlickerReductionShooting at offset 420

    // FlashControlMode at offset 424

    // FlashMasterCompensation at offset 426
    if data.len() >= 853 {
        // TODO: Handle format int8s
    }

    // FlashGNDistance at offset 430

    // FlashOutput at offset 434

    // FlashRemoteControl at offset 444

    // FlashWirelessOption at offset 456

    // AFAreaMode at offset 528

    // VRMode at offset 530

    // BracketSet at offset 534

    // BracketProgram at offset 536

    // BracketIncrement at offset 538

    // SecondarySlotFunction at offset 556

    // DXCropAlert at offset 572

    // SubjectDetection at offset 574

    // DynamicAFAreaSize at offset 576

    // MovieImageArea at offset 604

    // MovieType at offset 614

    // MovieISOAutoHiLimit at offset 616
    if data.len() >= 1234 {
        // TODO: Handle format int16u
    }

    // MovieISOAutoControlManualMode at offset 618

    // MovieISOAutoManualMode at offset 620
    if data.len() >= 1242 {
        // TODO: Handle format int16u
    }

    // MovieActiveD-Lighting at offset 696

    // MovieHighISONoiseReduction at offset 698

    // MovieFlickerReduction at offset 704

    // MovieMeteringMode at offset 706

    // MovieFocusMode at offset 708

    // MovieAFAreaMode at offset 710

    // MovieVRMode at offset 712

    // MovieElectronicVR at offset 716

    // MovieSoundRecording at offset 718

    // MicrophoneSensitivity at offset 720

    // MicrophoneAttenuator at offset 722

    // MicrophoneFrequencyResponse at offset 724

    // WindNoiseReduction at offset 726

    // MovieToneMap at offset 748

    // MovieFrameSize at offset 754

    // MovieFrameRate at offset 756

    // MicrophoneJackPower at offset 762

    // MovieDXCropAlert at offset 763

    // MovieSubjectDetection at offset 764

    // CustomSettingsZ9 at offset 799
    if data.len() >= 2814 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_menusettingsz9v3(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // Language at offset 1474

    // TimeZone at offset 1476

    // MonitorBrightness at offset 1482

    // AFFineTune at offset 1504

    // MultipleExposureMode at offset 154

    // MultiExposureShots at offset 156

    // HDMIOutputResolution at offset 1600

    // SetClockFromLocationData at offset 1613

    // AirplaneMode at offset 1620

    // EmptySlotRelease at offset 1621

    // EnergySavingMode at offset 1656

    // RecordLocationData at offset 1680

    // USBPowerDelivery at offset 1684

    // SensorShield at offset 1693

    // FocusShiftAutoReset at offset 1754

    // PreReleaseBurstLength at offset 1810

    // PostReleaseBurstLength at offset 1812

    // Intervals at offset 204
    if data.len() >= 412 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 208
    if data.len() >= 420 {
        // TODO: Handle format int32u
    }

    // FocusShiftNumberShots at offset 248

    // FocusShiftStepWidth at offset 252

    // FocusShiftInterval at offset 256

    // FocusShiftExposureLock at offset 260

    // PhotoShootingMenuBank at offset 290

    // ExtendedMenuBanks at offset 292

    // PhotoShootingMenuBankImageArea at offset 328

    // AutoISO at offset 342

    // ISOAutoHiLimit at offset 344
    if data.len() >= 690 {
        // TODO: Handle format int16u
    }

    // ISOAutoFlashLimit at offset 346
    if data.len() >= 694 {
        // TODO: Handle format int16u
    }

    // ISOAutoShutterTime at offset 354
    if data.len() >= 710 {
        if let Ok(value) = read_int16s(&data[708..710], byte_order) {
            tags.push(("ISOAutoShutterTime".to_string(), TagValue::I16(value)));
        }
    }

    // MovieVignetteControl at offset 436

    // DiffractionCompensation at offset 438

    // FlickerReductionShooting at offset 440

    // FlashControlMode at offset 444

    // FlashMasterCompensation at offset 446
    if data.len() >= 893 {
        // TODO: Handle format int8s
    }

    // FlashGNDistance at offset 450

    // FlashOutput at offset 454

    // AFAreaMode at offset 548

    // VRMode at offset 550

    // BracketSet at offset 554

    // BracketProgram at offset 556

    // BracketIncrement at offset 558

    // SecondarySlotFunction at offset 576

    // DXCropAlert at offset 592

    // SubjectDetection at offset 594

    // DynamicAFAreaSize at offset 596

    // HighFrequencyFlickerReduction at offset 636

    // MovieImageArea at offset 646

    // MovieType at offset 656

    // MovieISOAutoHiLimit at offset 658
    if data.len() >= 1318 {
        // TODO: Handle format int16u
    }

    // MovieISOAutoControlManualMode at offset 660

    // MovieISOAutoManualMode at offset 662
    if data.len() >= 1326 {
        // TODO: Handle format int16u
    }

    // HighFrameRate at offset 72

    // MovieActiveD-Lighting at offset 736

    // MovieHighISONoiseReduction at offset 738

    // MovieFlickerReduction at offset 744

    // MovieMeteringMode at offset 746

    // MovieFocusMode at offset 748

    // MovieAFAreaMode at offset 750

    // MovieVRMode at offset 752

    // MovieElectronicVR at offset 756

    // MovieSoundRecording at offset 758

    // MicrophoneSensitivity at offset 760

    // MicrophoneAttenuator at offset 762

    // MicrophoneFrequencyResponse at offset 764

    // WindNoiseReduction at offset 766

    // MovieToneMap at offset 788

    // MovieFrameSize at offset 794

    // MovieFrameRate at offset 796

    // MicrophoneJackPower at offset 802

    // MovieDXCropAlert at offset 803

    // MovieSubjectDetection at offset 804

    // MovieHighResZoom at offset 812

    // CustomSettingsZ9 at offset 847
    if data.len() >= 2910 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_menusettingsz9v4(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // Language at offset 1498

    // TimeZone at offset 1500

    // MonitorBrightness at offset 1506

    // AFFineTune at offset 1528

    // NonCPULens1FocalLength at offset 1532
    if data.len() >= 3066 {
        if let Ok(value) = read_int16s(&data[3064..3066], byte_order) {
            tags.push(("NonCPULens1FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens2FocalLength at offset 1536
    if data.len() >= 3074 {
        if let Ok(value) = read_int16s(&data[3072..3074], byte_order) {
            tags.push(("NonCPULens2FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // MultipleExposureMode at offset 154

    // NonCPULens3FocalLength at offset 1540
    if data.len() >= 3082 {
        if let Ok(value) = read_int16s(&data[3080..3082], byte_order) {
            tags.push(("NonCPULens3FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens4FocalLength at offset 1544
    if data.len() >= 3090 {
        if let Ok(value) = read_int16s(&data[3088..3090], byte_order) {
            tags.push(("NonCPULens4FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens5FocalLength at offset 1548
    if data.len() >= 3098 {
        if let Ok(value) = read_int16s(&data[3096..3098], byte_order) {
            tags.push(("NonCPULens5FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens6FocalLength at offset 1552
    if data.len() >= 3106 {
        if let Ok(value) = read_int16s(&data[3104..3106], byte_order) {
            tags.push(("NonCPULens6FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens7FocalLength at offset 1556
    if data.len() >= 3114 {
        if let Ok(value) = read_int16s(&data[3112..3114], byte_order) {
            tags.push(("NonCPULens7FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // MultiExposureShots at offset 156

    // NonCPULens8FocalLength at offset 1560
    if data.len() >= 3122 {
        if let Ok(value) = read_int16s(&data[3120..3122], byte_order) {
            tags.push(("NonCPULens8FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens9FocalLength at offset 1564
    if data.len() >= 3130 {
        if let Ok(value) = read_int16s(&data[3128..3130], byte_order) {
            tags.push(("NonCPULens9FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens10FocalLength at offset 1568
    if data.len() >= 3138 {
        if let Ok(value) = read_int16s(&data[3136..3138], byte_order) {
            tags.push(("NonCPULens10FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens11FocalLength at offset 1572
    if data.len() >= 3146 {
        if let Ok(value) = read_int16s(&data[3144..3146], byte_order) {
            tags.push(("NonCPULens11FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens12FocalLength at offset 1576
    if data.len() >= 3154 {
        if let Ok(value) = read_int16s(&data[3152..3154], byte_order) {
            tags.push(("NonCPULens12FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens13FocalLength at offset 1580
    if data.len() >= 3162 {
        if let Ok(value) = read_int16s(&data[3160..3162], byte_order) {
            tags.push(("NonCPULens13FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens14FocalLength at offset 1584
    if data.len() >= 3170 {
        if let Ok(value) = read_int16s(&data[3168..3170], byte_order) {
            tags.push(("NonCPULens14FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens15FocalLength at offset 1588
    if data.len() >= 3178 {
        if let Ok(value) = read_int16s(&data[3176..3178], byte_order) {
            tags.push(("NonCPULens15FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens16FocalLength at offset 1592
    if data.len() >= 3186 {
        if let Ok(value) = read_int16s(&data[3184..3186], byte_order) {
            tags.push(("NonCPULens16FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens17FocalLength at offset 1596
    if data.len() >= 3194 {
        if let Ok(value) = read_int16s(&data[3192..3194], byte_order) {
            tags.push(("NonCPULens17FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens18FocalLength at offset 1600
    if data.len() >= 3202 {
        if let Ok(value) = read_int16s(&data[3200..3202], byte_order) {
            tags.push(("NonCPULens18FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens19FocalLength at offset 1604
    if data.len() >= 3210 {
        if let Ok(value) = read_int16s(&data[3208..3210], byte_order) {
            tags.push(("NonCPULens19FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens20FocalLength at offset 1608
    if data.len() >= 3218 {
        if let Ok(value) = read_int16s(&data[3216..3218], byte_order) {
            tags.push(("NonCPULens20FocalLength".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens1MaxAperture at offset 1612
    if data.len() >= 3226 {
        if let Ok(value) = read_int16s(&data[3224..3226], byte_order) {
            tags.push(("NonCPULens1MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens2MaxAperture at offset 1616
    if data.len() >= 3234 {
        if let Ok(value) = read_int16s(&data[3232..3234], byte_order) {
            tags.push(("NonCPULens2MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens3MaxAperture at offset 1620
    if data.len() >= 3242 {
        if let Ok(value) = read_int16s(&data[3240..3242], byte_order) {
            tags.push(("NonCPULens3MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens4MaxAperture at offset 1624
    if data.len() >= 3250 {
        if let Ok(value) = read_int16s(&data[3248..3250], byte_order) {
            tags.push(("NonCPULens4MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens5MaxAperture at offset 1628
    if data.len() >= 3258 {
        if let Ok(value) = read_int16s(&data[3256..3258], byte_order) {
            tags.push(("NonCPULens5MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens6MaxAperture at offset 1632
    if data.len() >= 3266 {
        if let Ok(value) = read_int16s(&data[3264..3266], byte_order) {
            tags.push(("NonCPULens6MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens7MaxAperture at offset 1636
    if data.len() >= 3274 {
        if let Ok(value) = read_int16s(&data[3272..3274], byte_order) {
            tags.push(("NonCPULens7MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens8MaxAperture at offset 1640
    if data.len() >= 3282 {
        if let Ok(value) = read_int16s(&data[3280..3282], byte_order) {
            tags.push(("NonCPULens8MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens9MaxAperture at offset 1644
    if data.len() >= 3290 {
        if let Ok(value) = read_int16s(&data[3288..3290], byte_order) {
            tags.push(("NonCPULens9MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens10MaxAperture at offset 1648
    if data.len() >= 3298 {
        if let Ok(value) = read_int16s(&data[3296..3298], byte_order) {
            tags.push(("NonCPULens10MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens11MaxAperture at offset 1652
    if data.len() >= 3306 {
        if let Ok(value) = read_int16s(&data[3304..3306], byte_order) {
            tags.push(("NonCPULens11MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens12MaxAperture at offset 1656
    if data.len() >= 3314 {
        if let Ok(value) = read_int16s(&data[3312..3314], byte_order) {
            tags.push(("NonCPULens12MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens13MaxAperture at offset 1660
    if data.len() >= 3322 {
        if let Ok(value) = read_int16s(&data[3320..3322], byte_order) {
            tags.push(("NonCPULens13MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens14MaxAperture at offset 1664
    if data.len() >= 3330 {
        if let Ok(value) = read_int16s(&data[3328..3330], byte_order) {
            tags.push(("NonCPULens14MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens15MaxAperture at offset 1668
    if data.len() >= 3338 {
        if let Ok(value) = read_int16s(&data[3336..3338], byte_order) {
            tags.push(("NonCPULens15MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens16MaxAperture at offset 1672
    if data.len() >= 3346 {
        if let Ok(value) = read_int16s(&data[3344..3346], byte_order) {
            tags.push(("NonCPULens16MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens17MaxAperture at offset 1676
    if data.len() >= 3354 {
        if let Ok(value) = read_int16s(&data[3352..3354], byte_order) {
            tags.push(("NonCPULens17MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens18MaxAperture at offset 1680
    if data.len() >= 3362 {
        if let Ok(value) = read_int16s(&data[3360..3362], byte_order) {
            tags.push(("NonCPULens18MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens19MaxAperture at offset 1684
    if data.len() >= 3370 {
        if let Ok(value) = read_int16s(&data[3368..3370], byte_order) {
            tags.push(("NonCPULens19MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // NonCPULens20MaxAperture at offset 1688
    if data.len() >= 3378 {
        if let Ok(value) = read_int16s(&data[3376..3378], byte_order) {
            tags.push(("NonCPULens20MaxAperture".to_string(), TagValue::I16(value)));
        }
    }

    // HDMIOutputResolution at offset 1704

    // SetClockFromLocationData at offset 1717

    // AirplaneMode at offset 1724

    // EmptySlotRelease at offset 1725

    // EnergySavingMode at offset 1760

    // RecordLocationData at offset 1784

    // USBPowerDelivery at offset 1788

    // SensorShield at offset 1797

    // AutoCapturePreset at offset 1862

    // FocusShiftAutoReset at offset 1864

    // PreReleaseBurstLength at offset 1922

    // PostReleaseBurstLength at offset 1924

    // VerticalISOButton at offset 1938

    // ExposureCompensationButton at offset 1940

    // ISOButton at offset 1942

    // ViewModeShowEffectsOfSettings at offset 2002

    // DispButton at offset 2004

    // Intervals at offset 204
    if data.len() >= 412 {
        // TODO: Handle format int32u
    }

    // ExposureDelay at offset 2048
    if data.len() >= 4098 {
        // TODO: Handle format fixed32u
    }

    // CommandDialFrameAdvanceZoom at offset 2052

    // SubCommandDialFrameAdvanceZoom at offset 2054

    // PlaybackButton at offset 2056

    // WBButton at offset 2058

    // BracketButton at offset 2060

    // FlashModeButton at offset 2062

    // LensFunc1ButtonPlaybackMode at offset 2064

    // LensFunc2ButtonPlaybackMode at offset 2066

    // PlaybackButtonPlaybackMode at offset 2068

    // BracketButtonPlaybackMode at offset 2070

    // FlashModeButtonPlaybackMode at offset 2072

    // ShotsPerInterval at offset 208
    if data.len() >= 420 {
        // TODO: Handle format int32u
    }

    // FocusShiftNumberShots at offset 248

    // FocusShiftStepWidth at offset 252

    // FocusShiftInterval at offset 256

    // FocusShiftExposureLock at offset 260

    // PhotoShootingMenuBank at offset 290

    // ExtendedMenuBanks at offset 292

    // PhotoShootingMenuBankImageArea at offset 328

    // AutoISO at offset 342

    // ISOAutoHiLimit at offset 344
    if data.len() >= 690 {
        // TODO: Handle format int16u
    }

    // ISOAutoFlashLimit at offset 346
    if data.len() >= 694 {
        // TODO: Handle format int16u
    }

    // ISOAutoShutterTime at offset 354
    if data.len() >= 710 {
        if let Ok(value) = read_int16s(&data[708..710], byte_order) {
            tags.push(("ISOAutoShutterTime".to_string(), TagValue::I16(value)));
        }
    }

    // MovieVignetteControl at offset 436

    // DiffractionCompensation at offset 438

    // FlickerReductionShooting at offset 440

    // FlashControlMode at offset 444

    // FlashMasterCompensation at offset 446
    if data.len() >= 893 {
        // TODO: Handle format int8s
    }

    // FlashGNDistance at offset 450

    // FlashOutput at offset 454

    // AFAreaMode at offset 548

    // VRMode at offset 550

    // BracketSet at offset 554

    // BracketProgram at offset 556

    // BracketIncrement at offset 558

    // HDR at offset 570

    // SecondarySlotFunction at offset 576

    // HDRLevel at offset 582

    // Slot2JpgSize at offset 586

    // DXCropAlert at offset 592

    // SubjectDetection at offset 594

    // DynamicAFAreaSize at offset 596

    // HighFrequencyFlickerReduction at offset 636

    // MovieImageArea at offset 646

    // MovieType at offset 656

    // MovieISOAutoHiLimit at offset 658
    if data.len() >= 1318 {
        // TODO: Handle format int16u
    }

    // MovieISOAutoControlManualMode at offset 660

    // MovieISOAutoManualMode at offset 662
    if data.len() >= 1326 {
        // TODO: Handle format int16u
    }

    // HighFrameRate at offset 72

    // MovieActiveD-Lighting at offset 736

    // MovieHighISONoiseReduction at offset 738

    // MovieFlickerReduction at offset 744

    // MovieMeteringMode at offset 746

    // MovieFocusMode at offset 748

    // MovieAFAreaMode at offset 750

    // MovieVRMode at offset 752

    // MovieElectronicVR at offset 756

    // MovieSoundRecording at offset 758

    // MicrophoneSensitivity at offset 760

    // MicrophoneAttenuator at offset 762

    // MicrophoneFrequencyResponse at offset 764

    // WindNoiseReduction at offset 766

    // MovieToneMap at offset 788

    // MovieFrameSize at offset 794

    // MovieFrameRate at offset 796

    // MicrophoneJackPower at offset 802

    // MovieDXCropAlert at offset 803

    // MovieSubjectDetection at offset 804

    // MovieHighResZoom at offset 812

    // CustomSettingsZ9v4 at offset 847
    if data.len() >= 2958 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_rotationinfod500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlickerReductionIndicator at offset 1330

    // Rotation at offset 26

    // Interval at offset 32

    // IntervalFrame at offset 36

    Ok(tags)
}

fn process_nikon_settingsinfod810(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // SecondarySlotFunction at offset 316

    Ok(tags)
}

fn process_nikon_menusettingsd850(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PhotoShootingMenuBankImageArea at offset 1757

    Ok(tags)
}

fn process_nikon_colorbalancea(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_RBLevels at offset 624
    if data.len() >= 1252 {
        if let Ok(values) = read_int16u_array(&data[1248..1252], byte_order, 2) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBLevels".to_string(), TagValue::String(value_str)));
        }
    }

    // WB_RBLevelsAuto at offset 626
    if data.len() >= 1256 {
        if let Ok(values) = read_int16u_array(&data[1252..1256], byte_order, 2) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBLevelsAuto".to_string(), TagValue::String(value_str)));
        }
    }

    // WB_RBLevelsDaylight at offset 628
    if data.len() >= 1284 {
        if let Ok(values) = read_int16u_array(&data[1256..1284], byte_order, 14) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push((
                "WB_RBLevelsDaylight".to_string(),
                TagValue::String(value_str),
            ));
        }
    }

    // WB_RBLevelsIncandescent at offset 642
    if data.len() >= 1312 {
        if let Ok(values) = read_int16u_array(&data[1284..1312], byte_order, 14) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push((
                "WB_RBLevelsIncandescent".to_string(),
                TagValue::String(value_str),
            ));
        }
    }

    // WB_RBLevelsFluorescent at offset 656
    if data.len() >= 1324 {
        if let Ok(values) = read_int16u_array(&data[1312..1324], byte_order, 6) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push((
                "WB_RBLevelsFluorescent".to_string(),
                TagValue::String(value_str),
            ));
        }
    }

    // WB_RBLevelsCloudy at offset 662
    if data.len() >= 1352 {
        if let Ok(values) = read_int16u_array(&data[1324..1352], byte_order, 14) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBLevelsCloudy".to_string(), TagValue::String(value_str)));
        }
    }

    // WB_RBLevelsFlash at offset 676
    if data.len() >= 1380 {
        if let Ok(values) = read_int16u_array(&data[1352..1380], byte_order, 14) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBLevelsFlash".to_string(), TagValue::String(value_str)));
        }
    }

    // WB_RBLevelsShade at offset 690
    if data.len() >= 1408 {
        if let Ok(values) = read_int16u_array(&data[1380..1408], byte_order, 14) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBLevelsShade".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_colorbalanceb(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ColorBalanceVersion at offset 4
    if data.len() >= 16 {
        // TODO: Handle format undef
    }

    // WB_RGGBLevels at offset 5096
    if data.len() >= 10208 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsDaylight at offset 5112
    if data.len() >= 10240 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsCloudy at offset 5128
    if data.len() >= 10272 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsTungsten at offset 5160
    if data.len() >= 10336 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsFluorescentW at offset 5176
    if data.len() >= 10368 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsFlash at offset 5192
    if data.len() >= 10400 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsCustom at offset 5224
    if data.len() >= 10464 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsAuto at offset 5240
    if data.len() >= 10496 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_colorbalancec(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_RGGBLevelsShade at offset 116
    if data.len() >= 248 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsTungsten at offset 136
    if data.len() >= 288 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsFluorescentW at offset 156
    if data.len() >= 328 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsFluorescentN at offset 176
    if data.len() >= 368 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsFluorescentD at offset 196
    if data.len() >= 408 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsHTMercury at offset 216
    if data.len() >= 448 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsCustom at offset 256
    if data.len() >= 528 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsAuto at offset 276
    if data.len() >= 568 {
        // TODO: Handle format int32u
    }

    // BlackLevel at offset 32
    if data.len() >= 66 {
        // TODO: Handle format int16u
    }

    // ColorBalanceVersion at offset 4
    if data.len() >= 16 {
        // TODO: Handle format undef
    }

    // WB_RGGBLevels at offset 56
    if data.len() >= 128 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsDaylight at offset 76
    if data.len() >= 168 {
        // TODO: Handle format int32u
    }

    // WB_RGGBLevelsCloudy at offset 96
    if data.len() >= 208 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_jpginfod500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // JPGCompression at offset 36

    Ok(tags)
}

fn process_nikon_vrinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // VRInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // VibrationReduction at offset 4

    // VRType at offset 8

    Ok(tags)
}

fn process_nikon_facedetect(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FaceDetectFrameSize at offset 1
    if data.len() >= 6 {
        if let Ok(values) = read_int16u_array(&data[2..6], byte_order, 2) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push((
                "FaceDetectFrameSize".to_string(),
                TagValue::String(value_str),
            ));
        }
    }

    // Face3Position at offset 12
    if data.len() >= 32 {
        if let Ok(values) = read_int16u_array(&data[24..32], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face3Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face4Position at offset 16
    if data.len() >= 40 {
        if let Ok(values) = read_int16u_array(&data[32..40], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face4Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face5Position at offset 20
    if data.len() >= 48 {
        if let Ok(values) = read_int16u_array(&data[40..48], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face5Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face6Position at offset 24
    if data.len() >= 56 {
        if let Ok(values) = read_int16u_array(&data[48..56], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face6Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face7Position at offset 28
    if data.len() >= 64 {
        if let Ok(values) = read_int16u_array(&data[56..64], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face7Position".to_string(), TagValue::String(value_str)));
        }
    }

    // FacesDetected at offset 3

    // Face8Position at offset 32
    if data.len() >= 72 {
        if let Ok(values) = read_int16u_array(&data[64..72], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face8Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face9Position at offset 36
    if data.len() >= 80 {
        if let Ok(values) = read_int16u_array(&data[72..80], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face9Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face1Position at offset 4
    if data.len() >= 16 {
        if let Ok(values) = read_int16u_array(&data[8..16], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face1Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face10Position at offset 40
    if data.len() >= 88 {
        if let Ok(values) = read_int16u_array(&data[80..88], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face10Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face11Position at offset 44
    if data.len() >= 96 {
        if let Ok(values) = read_int16u_array(&data[88..96], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face11Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face12Position at offset 48
    if data.len() >= 104 {
        if let Ok(values) = read_int16u_array(&data[96..104], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face12Position".to_string(), TagValue::String(value_str)));
        }
    }

    // Face2Position at offset 8
    if data.len() >= 24 {
        if let Ok(values) = read_int16u_array(&data[16..24], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("Face2Position".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_picturecontrol(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PictureControlVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // PictureControlBase at offset 24
    if data.len() >= 88 {
        // TODO: Handle format string
    }

    // PictureControlName at offset 4
    if data.len() >= 48 {
        // TODO: Handle format string
    }

    // PictureControlAdjust at offset 48

    // PictureControlQuickAdjust at offset 49

    // Sharpness at offset 50

    // Contrast at offset 51

    // Brightness at offset 52

    // Saturation at offset 53

    // HueAdjustment at offset 54

    // FilterEffect at offset 55

    // ToningEffect at offset 56

    // ToningSaturation at offset 57

    Ok(tags)
}

fn process_nikon_picturecontrol2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PictureControlVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // PictureControlBase at offset 24
    if data.len() >= 88 {
        // TODO: Handle format string
    }

    // PictureControlName at offset 4
    if data.len() >= 48 {
        // TODO: Handle format string
    }

    // PictureControlAdjust at offset 48

    // PictureControlQuickAdjust at offset 49

    // Sharpness at offset 51

    // Clarity at offset 53

    // Contrast at offset 55

    // Brightness at offset 57

    // Saturation at offset 59

    // Hue at offset 61

    // FilterEffect at offset 63

    // ToningEffect at offset 64

    // ToningSaturation at offset 65

    Ok(tags)
}

fn process_nikon_picturecontrol3(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PictureControlVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // PictureControlBase at offset 28
    if data.len() >= 96 {
        // TODO: Handle format string
    }

    // PictureControlAdjust at offset 54

    // PictureControlQuickAdjust at offset 55

    // Sharpness at offset 57

    // MidRangeSharpness at offset 59

    // Clarity at offset 61

    // Contrast at offset 63

    // Brightness at offset 65

    // Saturation at offset 67

    // Hue at offset 69

    // FilterEffect at offset 71

    // ToningEffect at offset 72

    // ToningSaturation at offset 73

    // PictureControlName at offset 8
    if data.len() >= 56 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_picturecontrolunknown(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PictureControlVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_worldtime(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // TimeZone at offset 0
    if data.len() >= 2 {
        if let Ok(value) = read_int16s(&data[0..2], byte_order) {
            tags.push(("TimeZone".to_string(), TagValue::I16(value)));
        }
    }

    // DaylightSavings at offset 2

    // DateDisplayFormat at offset 3

    Ok(tags)
}

fn process_nikon_bracketinginfod810(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AEBracketingSteps at offset 15

    // WBBracketingSteps at offset 16

    // NikonMeteringMode at offset 23

    Ok(tags)
}

fn process_nikon_isoinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ISO at offset 0

    // ISOExpansion2 at offset 10
    if data.len() >= 22 {
        // TODO: Handle format int16u
    }

    // ISOExpansion at offset 4
    if data.len() >= 10 {
        // TODO: Handle format int16u
    }

    // ISO2 at offset 6

    Ok(tags)
}

fn process_nikon_distortinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // DistortionVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AutoDistortionControl at offset 4

    Ok(tags)
}

fn process_nikon_unknowninfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // UnknownInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_bracketinginfod500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AEBracketingSteps at offset 15

    // WBBracketingSteps at offset 16

    // ADLBracketingStep at offset 23

    // ADLBracketingType at offset 24

    Ok(tags)
}

fn process_nikon_seqinfod6(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // IntervalShooting at offset 36
    if data.len() >= 74 {
        // TODO: Handle format int16u
    }

    // IntervalFrame at offset 40
    if data.len() >= 82 {
        // TODO: Handle format int16u
    }

    // ImageArea at offset 43

    Ok(tags)
}

fn process_nikon_intervalinfoz7ii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // IntervalShooting at offset 36
    if data.len() >= 74 {
        // TODO: Handle format int16u
    }

    // IntervalFrame at offset 40
    if data.len() >= 82 {
        // TODO: Handle format int16u
    }

    // ImageArea at offset 43

    Ok(tags)
}

fn process_nikon_seqinfoz9(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FocusShiftShooting at offset 32

    // IntervalShooting at offset 40
    if data.len() >= 82 {
        // TODO: Handle format int16u
    }

    // IntervalFrame at offset 42
    if data.len() >= 86 {
        // TODO: Handle format int16u
    }

    Ok(tags)
}

fn process_nikon_unknowninfo2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // UnknownInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_hdrinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // HDRInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // HDR at offset 4

    // HDRLevel at offset 5

    // HDRSmoothing at offset 6

    // HDRLevel2 at offset 7

    Ok(tags)
}

fn process_nikon_hdrinfo2(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // HDRInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // HDR at offset 4

    // HDRLevel at offset 5

    Ok(tags)
}

fn process_nikon_isoautoinfod810(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ISOAutoShutterTime at offset 4

    // ISOAutoHiLimit at offset 5

    Ok(tags)
}

fn process_nikon_portraitinfoz7ii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PortraitImpressionBalance at offset 160
    if data.len() >= 322 {
        // TODO: Handle format int8u
    }

    Ok(tags)
}

fn process_nikon_locationinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LocationInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // TextEncoding at offset 4

    // CountryCode at offset 5
    if data.len() >= 16 {
        // TODO: Handle format undef
    }

    // Location at offset 9
    if data.len() >= 158 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_moresettingsd850(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PhotoShootingMenuBank at offset 36

    // PrimarySlot at offset 37

    Ok(tags)
}

fn process_nikon_shootingmenud500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // PhotoShootingMenuBank at offset 0

    // PrimarySlot at offset 2

    // ISOAutoShutterTime at offset 4

    // ISOAutoHiLimit at offset 5

    // FlickerReduction at offset 7

    // PhotoShootingMenuBankImageArea at offset 7.1

    Ok(tags)
}

fn process_nikon_makernotes0x51(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FirmwareVersion51 at offset 0
    if data.len() >= 16 {
        // TODO: Handle format string
    }

    // NEFCompression at offset 10
    if data.len() >= 22 {
        // TODO: Handle format int16u
    }

    Ok(tags)
}

fn process_nikon_makernotes0x56(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FirmwareVersion56 at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // PixelShiftID at offset 12

    // BurstGroupID at offset 4
    if data.len() >= 10 {
        // TODO: Handle format int16u
    }

    Ok(tags)
}

fn process_nikon_customsettingsd500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    Ok(tags)
}

fn process_nikon_offset13infoz9(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFAreaInitialXPosition at offset 3048
    if data.len() >= 6097 {
        // TODO: Handle format int8s
    }

    // AFAreaInitialYPosition at offset 3049
    if data.len() >= 6099 {
        // TODO: Handle format int8s
    }

    // AFAreaInitialWidth at offset 3050

    // AFAreaInitialHeight at offset 3051

    Ok(tags)
}

fn process_nikon_autocaptureinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AutoCapturedFrame at offset 0

    // AutoCaptureCriteria at offset 1

    // AutoCaptureCriteriaMotionSize at offset 100

    // AutoCaptureCriteriaSubjectSize at offset 105

    // AutoCaptureCriteriaSubjectType at offset 106

    // AutoCaptureRecordingTime at offset 55

    // AutoCaptureWaitTime at offset 56

    // AutoCaptureDistanceFar at offset 74

    // AutoCaptureDistanceNear at offset 78

    // AutoCaptureCriteriaMotionDirection at offset 95

    // AutoCaptureCriteriaMotionSpeed at offset 99

    Ok(tags)
}

fn process_nikon_orientationinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // RollAngle at offset 0
    if data.len() >= 2 {
        // TODO: Handle format fixed32u
    }

    // PitchAngle at offset 4
    if data.len() >= 10 {
        // TODO: Handle format fixed32u
    }

    // YawAngle at offset 8
    if data.len() >= 18 {
        // TODO: Handle format fixed32u
    }

    Ok(tags)
}

fn process_nikon_afinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFAreaMode at offset 0

    // AFPoint at offset 1

    // AFPointsInFocus at offset 2
    if data.len() >= 6 {
        // TODO: Handle format int16u
    }

    Ok(tags)
}

fn process_nikon_menuinfoz8(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    Ok(tags)
}

fn process_nikon_menuinfoz9(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    Ok(tags)
}

fn process_nikon_menusettingsz6iii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // SubjectDetectionAreaMF at offset 1002

    // LinkVRToFocusPoint at offset 1004

    // MovieSlowMotion at offset 1046

    // MovieType at offset 1050

    // MovieFrameSize at offset 1162

    // MovieFrameRate at offset 1164

    // CustomSettingsZ6III at offset 1255
    if data.len() >= 3910 {
        // TODO: Handle format undef
    }

    // Language at offset 2300

    // TimeZone at offset 2302

    // MonitorBrightness at offset 2308

    // EmptySlotRelease at offset 2444

    // EnergySavingMode at offset 2450

    // USBPowerDelivery at offset 2476

    // SaveFocusPosition at offset 2480

    // SilentPhotography at offset 2487

    // AirplaneMode at offset 2496

    // SingleFrame at offset 360

    // HighFrameRate at offset 364

    // MultipleExposureMode at offset 444

    // MultiExposureShots at offset 446

    // IntervalDurationHours at offset 476
    if data.len() >= 956 {
        // TODO: Handle format int32u
    }

    // IntervalDurationMinutes at offset 480
    if data.len() >= 964 {
        // TODO: Handle format int32u
    }

    // IntervalDurationSeconds at offset 484
    if data.len() >= 972 {
        // TODO: Handle format int32u
    }

    // Intervals at offset 492
    if data.len() >= 988 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 496
    if data.len() >= 996 {
        // TODO: Handle format int32u
    }

    // IntervalExposureSmoothing at offset 500
    if data.len() >= 1001 {
        // TODO: Handle format int8u
    }

    // IntervalPriority at offset 502
    if data.len() >= 1005 {
        // TODO: Handle format int8u
    }

    // FocusShiftNumberShots at offset 536

    // FocusShiftStepWidth at offset 540

    // FocusShiftInterval at offset 544

    // FocusShiftExposureLock at offset 548

    // AutoISO at offset 648

    // ISOAutoHiLimit at offset 650
    if data.len() >= 1302 {
        // TODO: Handle format int16u
    }

    // DiffractionCompensation at offset 718
    if data.len() >= 1437 {
        // TODO: Handle format int8u
    }

    // AutoDistortionControl at offset 719
    if data.len() >= 1439 {
        // TODO: Handle format int8u
    }

    // FlickerReductionShooting at offset 720

    // NikonMeteringMode at offset 722

    // FlashControlMode at offset 724

    // FlashGNDistance at offset 730

    // FlashOutput at offset 734

    // FlashWirelessOption at offset 742

    // FlashRemoteControl at offset 744

    // FlashMasterControlMode at offset 748

    // FlashMasterCompensation at offset 750
    if data.len() >= 1501 {
        // TODO: Handle format int8s
    }

    // FlashMasterOutput at offset 754

    // AFAreaMode at offset 832

    // VRMode at offset 834

    // BracketSet at offset 838

    // BracketProgram at offset 840

    // BracketIncrement at offset 842

    // HDR at offset 852

    // SecondarySlotFunction at offset 858

    // HDRLevel at offset 864

    // Slot2JpgSize at offset 868

    // SubjectDetection at offset 878

    // DynamicAFAreaSize at offset 880

    // ToneMap at offset 884

    // PortraitImpressionBalance at offset 888

    // HighFrequencyFlickerReduction at offset 902

    // PixelShiftShooting at offset 904

    // PixelShiftNumberShots at offset 906

    // PixelShiftDelay at offset 908

    // PixelShiftInterval at offset 910

    Ok(tags)
}

fn process_nikon_shotinfod40(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ShutterCount at offset 582
    if data.len() >= 1168 {
        // TODO: Handle format int32u
    }

    // VibrationReduction at offset 586.1

    // CustomSettingsD40 at offset 729
    if data.len() >= 1482 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod80(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ShutterCount at offset 586
    if data.len() >= 1176 {
        // TODO: Handle format int32u
    }

    // Rotation at offset 590.1

    // VibrationReduction at offset 590.2

    // FlashFired at offset 590.3

    // NikonImageSize at offset 708

    // ImageQuality at offset 708.1

    // CustomSettingsD80 at offset 748
    if data.len() >= 1530 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod90(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 693

    // ShutterCount at offset 725
    if data.len() >= 1454 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD90 at offset 884
    if data.len() >= 1840 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod3a(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ISO2 at offset 598

    // ShutterCount at offset 630
    if data.len() >= 1264 {
        // TODO: Handle format int32u
    }

    // NikonImageSize at offset 723.1

    // ImageQuality at offset 723.2

    // CustomSettingsD3 at offset 769
    if data.len() >= 1586 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod3b(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ImageArea at offset 16

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 605

    // ShutterCount at offset 637
    if data.len() >= 1278 {
        // TODO: Handle format int32u
    }

    // ShutterCount at offset 639
    if data.len() >= 1282 {
        // TODO: Handle format int32u
    }

    // PreFlashReturnStrength at offset 650

    // NikonImageSize at offset 732.1

    // ImageQuality at offset 732.2

    // CustomSettingsD3 at offset 778
    if data.len() >= 1604 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod3x(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 605

    // ShutterCount at offset 640
    if data.len() >= 1284 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD3X at offset 779
    if data.len() >= 1606 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod3s(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ImageArea at offset 16

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 545

    // ShutterCount at offset 578
    if data.len() >= 1160 {
        // TODO: Handle format int32u
    }

    // JPGCompression at offset 671.1

    // CustomSettingsD3S at offset 718
    if data.len() >= 1490 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod300a(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ISO2 at offset 604

    // ShutterCount at offset 633
    if data.len() >= 1270 {
        // TODO: Handle format int32u
    }

    // AFFineTuneAdj at offset 721
    if data.len() >= 1444 {
        // TODO: Handle format int16u
    }

    // CustomSettingsD300 at offset 790
    if data.len() >= 1628 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod300b(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 613

    // ShutterCount at offset 644
    if data.len() >= 1292 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD300 at offset 802
    if data.len() >= 1652 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod300s(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 613

    // ShutterCount at offset 646
    if data.len() >= 1296 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD300S at offset 804
    if data.len() >= 1656 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod700(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 613

    // ShutterCount at offset 647
    if data.len() >= 1298 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD700 at offset 804
    if data.len() >= 1704 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod780(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientOffset at offset 156
    if data.len() >= 316 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfod7500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientOffset at offset 160
    if data.len() >= 324 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfod800(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // RepeatingFlashOutputExternal at offset 1216

    // RepeatingFlashRateExternal at offset 1218

    // RepeatingFlashCountExternal at offset 1219

    // FlashExposureComp2 at offset 1234
    if data.len() >= 2469 {
        // TODO: Handle format int8s
    }

    // RepeatingFlashRateBuilt-in at offset 1242

    // RepeatingFlashCountBuilt-in at offset 1243

    // ShutterCount at offset 1531
    if data.len() >= 3066 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD800 at offset 1772
    if data.len() >= 3640 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfod810(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientationOffset at offset 132
    if data.len() >= 268 {
        // TODO: Handle format int32u
    }

    // SettingsOffset at offset 16
    if data.len() >= 36 {
        // TODO: Handle format int32u
    }

    // BracketingOffset at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISOAutoOffset at offset 56
    if data.len() >= 116 {
        // TODO: Handle format int32u
    }

    // CustomSettingsOffset at offset 64
    if data.len() >= 132 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod850(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // MenuSettingsOffset at offset 16
    if data.len() >= 36 {
        // TODO: Handle format int32u
    }

    // OrientationOffset at offset 160
    if data.len() >= 324 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // MoreSettingsOffset at offset 76
    if data.len() >= 156 {
        // TODO: Handle format int32u
    }

    // CustomSettingsOffset at offset 88
    if data.len() >= 180 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod5000(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ISO2 at offset 693

    // ShutterCount at offset 726
    if data.len() >= 1456 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD5000 at offset 888
    if data.len() >= 1844 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod5100(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // CustomSettingsD5100 at offset 1031
    if data.len() >= 2130 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ShutterCount at offset 801
    if data.len() >= 1606 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod5200(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // ShutterCount at offset 3032
    if data.len() >= 6068 {
        // TODO: Handle format int32u
    }

    // CustomSettingsD5200 at offset 3285
    if data.len() >= 6638 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfod7000(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // CustomSettingsD7000 at offset 1028
    if data.len() >= 2152 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // ShutterCount at offset 800
    if data.len() >= 1604 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod4(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // CustomSettingsD4 at offset 1873
    if data.len() >= 3858 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfod4s(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientationInfo at offset 13579
    if data.len() >= 27182 {
        // TODO: Handle format undef
    }

    // Rotation at offset 13971

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // SecondarySlotFunction at offset 464

    // AEBracketingSteps at offset 5964

    // WBBracketingSteps at offset 5965

    // ReleaseMode at offset 6221

    // CustomSettingsD4S at offset 6301
    if data.len() >= 12714 {
        // TODO: Handle format undef
    }

    // MultiSelectorLiveViewMode at offset 6338

    // ISOAutoShutterTime at offset 6378

    // ISOAutoHiLimit at offset 6379

    // CustomSettingsD4S at offset 6461
    if data.len() >= 13034 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_shotinfod500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // RotationInfoOffset at offset 16
    if data.len() >= 36 {
        // TODO: Handle format int32u
    }

    // OrientationOffset at offset 160
    if data.len() >= 324 {
        // TODO: Handle format int32u
    }

    // OtherOffset at offset 168
    if data.len() >= 340 {
        // TODO: Handle format int32u
    }

    // JPGInfoOffset at offset 20
    if data.len() >= 44 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // BracketingOffset at offset 44
    if data.len() >= 92 {
        // TODO: Handle format int32u
    }

    // ShootingMenuOffset at offset 80
    if data.len() >= 164 {
        // TODO: Handle format int32u
    }

    // CustomSettingsOffset at offset 88
    if data.len() >= 180 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod6(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientationOffset at offset 156
    if data.len() >= 316 {
        // TODO: Handle format int32u
    }

    // IntervalOffset at offset 164
    if data.len() >= 332 {
        // TODO: Handle format int32u
    }

    // NumberOffsets at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 24 {
        // TODO: Handle format string
    }

    // SequenceOffset at offset 48
    if data.len() >= 100 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfod610(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // CustomSettingsD610 at offset 1999
    if data.len() >= 4094 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfoz6iii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientationOffset at offset 136
    if data.len() >= 276 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion2 at offset 14
    if data.len() >= 44 {
        // TODO: Handle format string
    }

    // MenuOffset at offset 144
    if data.len() >= 292 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion3 at offset 24
    if data.len() >= 64 {
        // TODO: Handle format string
    }

    // NumberOffsets at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 24 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_shotinfoz7ii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // OrientationOffset at offset 136
    if data.len() >= 276 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion2 at offset 14
    if data.len() >= 44 {
        // TODO: Handle format string
    }

    // OrientationOffset at offset 152
    if data.len() >= 308 {
        // TODO: Handle format int32u
    }

    // MenuOffset at offset 160
    if data.len() >= 324 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion3 at offset 24
    if data.len() >= 64 {
        // TODO: Handle format string
    }

    // NumberOffsets at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 24 {
        // TODO: Handle format string
    }

    // IntervalOffset at offset 48
    if data.len() >= 100 {
        // TODO: Handle format int32u
    }

    // PortraitOffset at offset 56
    if data.len() >= 116 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfoz8(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // AutoCaptureOffset at offset 128
    if data.len() >= 260 {
        // TODO: Handle format int32u
    }

    // OrientOffset at offset 132
    if data.len() >= 268 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion2 at offset 14
    if data.len() >= 44 {
        // TODO: Handle format string
    }

    // MenuOffset at offset 140
    if data.len() >= 284 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion3 at offset 24
    if data.len() >= 64 {
        // TODO: Handle format string
    }

    // NumberOffsets at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 24 {
        // TODO: Handle format string
    }

    // SequenceOffset at offset 48
    if data.len() >= 100 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfoz9(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // AutoCaptureOffset at offset 128
    if data.len() >= 260 {
        // TODO: Handle format int32u
    }

    // OrientOffset at offset 132
    if data.len() >= 268 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion2 at offset 14
    if data.len() >= 44 {
        // TODO: Handle format string
    }

    // MenuOffset at offset 140
    if data.len() >= 284 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion3 at offset 24
    if data.len() >= 64 {
        // TODO: Handle format string
    }

    // NumberOffsets at offset 36
    if data.len() >= 76 {
        // TODO: Handle format int32u
    }

    // FirmwareVersion at offset 4
    if data.len() >= 24 {
        // TODO: Handle format string
    }

    // SequenceOffset at offset 48
    if data.len() >= 100 {
        // TODO: Handle format int32u
    }

    // Offset13 at offset 88
    if data.len() >= 180 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_shotinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ShotInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // VR_0x66 at offset 102
    if data.len() >= 205 {
        // TODO: Handle format int8u
    }

    // ShutterCount at offset 106
    if data.len() >= 216 {
        // TODO: Handle format int32u
    }

    // DeletedImageCount at offset 110
    if data.len() >= 224 {
        // TODO: Handle format int32u
    }

    // VibrationReduction at offset 117
    if data.len() >= 235 {
        // TODO: Handle format int8u
    }

    // VibrationReduction at offset 130
    if data.len() >= 261 {
        // TODO: Handle format int8u
    }

    // DistortionControl at offset 16

    // ShutterCount at offset 343
    if data.len() >= 690 {
        // TODO: Handle format undef
    }

    // FirmwareVersion at offset 4
    if data.len() >= 18 {
        // TODO: Handle format string
    }

    // VibrationReduction at offset 430
    if data.len() >= 861 {
        // TODO: Handle format int8u
    }

    // ShutterCount at offset 589
    if data.len() >= 1182 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_colorbalance1(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_RBGGLevels at offset 0
    if data.len() >= 8 {
        if let Ok(values) = read_int16u_array(&data[0..8], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RBGGLevels".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_colorbalance2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_RGGBLevels at offset 0
    if data.len() >= 8 {
        if let Ok(values) = read_int16u_array(&data[0..8], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RGGBLevels".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_colorbalance3(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_RGBGLevels at offset 0
    if data.len() >= 8 {
        if let Ok(values) = read_int16u_array(&data[0..8], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_RGBGLevels".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_colorbalance4(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // WB_GRBGLevels at offset 0
    if data.len() >= 8 {
        if let Ok(values) = read_int16u_array(&data[0..8], byte_order, 4) {
            let value_str = values
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<_>>()
                .join(" ");
            tags.push(("WB_GRBGLevels".to_string(), TagValue::String(value_str)));
        }
    }

    Ok(tags)
}

fn process_nikon_colorbalanceunknown(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ColorBalanceVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_colorbalanceunknown2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // ColorBalanceVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    Ok(tags)
}

fn process_nikon_lensdata00(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // MaxApertureAtMinFocal at offset 10

    // MaxApertureAtMaxFocal at offset 11

    // LensIDNumber at offset 6

    // LensFStops at offset 7

    // MinFocalLength at offset 8

    // MaxFocalLength at offset 9

    Ok(tags)
}

fn process_nikon_lensdata01(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FocalLength at offset 10

    // LensIDNumber at offset 11

    // LensFStops at offset 12

    // MinFocalLength at offset 13

    // MaxFocalLength at offset 14

    // MaxApertureAtMinFocal at offset 15

    // MaxApertureAtMaxFocal at offset 16

    // EffectiveMaxAperture at offset 18

    // ExitPupilPosition at offset 4

    // AFAperture at offset 5

    // FocusPosition at offset 8

    // FocusDistance at offset 9

    Ok(tags)
}

fn process_nikon_lensdata0204(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FocusDistance at offset 10

    // FocalLength at offset 11

    // LensIDNumber at offset 12

    // LensFStops at offset 13

    // MinFocalLength at offset 14

    // MaxFocalLength at offset 15

    // MaxApertureAtMinFocal at offset 16

    // MaxApertureAtMaxFocal at offset 17

    // EffectiveMaxAperture at offset 19

    // ExitPupilPosition at offset 4

    // AFAperture at offset 5

    // FocusPosition at offset 8

    Ok(tags)
}

fn process_nikon_lensdata0400(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // LensModel at offset 394
    if data.len() >= 916 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_lensdata0402(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // LensModel at offset 395
    if data.len() >= 918 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_lensdata0403(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // LensModel at offset 684
    if data.len() >= 1496 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_lensdata0800(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FocusDistance at offset 11

    // FocalLength at offset 12

    // LensIDNumber at offset 13

    // LensFStops at offset 14

    // MinFocalLength at offset 15

    // MaxFocalLength at offset 16

    // MaxApertureAtMinFocal at offset 17

    // MaxApertureAtMaxFocal at offset 18

    // MCUVersion at offset 19

    // EffectiveMaxAperture at offset 20

    // OldLensData at offset 3
    if data.len() >= 40 {
        // TODO: Handle format undef
    }

    // ExitPupilPosition at offset 4

    // NewLensData at offset 47
    if data.len() >= 128 {
        // TODO: Handle format undef
    }

    // LensID at offset 48
    if data.len() >= 98 {
        // TODO: Handle format int16u
    }

    // AFAperture at offset 5

    // LensFirmwareVersion at offset 52
    if data.len() >= 106 {
        // TODO: Handle format int16u
    }

    // MaxAperture at offset 54
    if data.len() >= 110 {
        // TODO: Handle format int16u
    }

    // FNumber at offset 56
    if data.len() >= 114 {
        // TODO: Handle format int16u
    }

    // FocalLength at offset 60
    if data.len() >= 122 {
        // TODO: Handle format int16u
    }

    // FocusDistanceRangeWidth at offset 76
    if data.len() >= 153 {
        // TODO: Handle format int8u
    }

    // FocusDistance at offset 78
    if data.len() >= 158 {
        // TODO: Handle format int16u
    }

    // LensDriveEnd at offset 86
    if data.len() >= 173 {
        // TODO: Handle format int8u
    }

    // FocusStepsFromInfinity at offset 88
    if data.len() >= 177 {
        // TODO: Handle format int8u
    }

    // LensPositionAbsolute at offset 90
    if data.len() >= 184 {
        // TODO: Handle format int32s
    }

    // LensMountType at offset 95
    if data.len() >= 191 {
        // TODO: Handle format int8u
    }

    Ok(tags)
}

fn process_nikon_lensdataunknown(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // LensDataVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_menuinfoz7ii(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MenuSettingsOffsetZ7II at offset 16
    if data.len() >= 36 {
        // TODO: Handle format int32u
    }

    Ok(tags)
}

fn process_nikon_intervalinfod6(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // Intervals at offset 380
    if data.len() >= 764 {
        // TODO: Handle format int32u
    }

    // ShotsPerInterval at offset 384
    if data.len() >= 772 {
        // TODO: Handle format int32u
    }

    // IntervalExposureSmoothing at offset 388
    if data.len() >= 777 {
        // TODO: Handle format int8u
    }

    // IntervalPriority at offset 390
    if data.len() >= 781 {
        // TODO: Handle format int8u
    }

    // FocusShiftNumberShots at offset 424

    // FocusShiftStepWidth at offset 428

    // FocusShiftInterval at offset 432

    // FocusShiftExposureLock at offset 436

    // DiffractionCompensation at offset 526
    if data.len() >= 1053 {
        // TODO: Handle format int8u
    }

    // FlashControlMode at offset 532

    // FlashGNDistance at offset 538

    // FlashOutput at offset 542

    // FlashRemoteControl at offset 552

    // FlashMasterControlMode at offset 556

    // FlashMasterCompensation at offset 558
    if data.len() >= 1117 {
        // TODO: Handle format int8s
    }

    // FlashMasterOutput at offset 562

    // FlashWirelessOption at offset 564

    // MovieType at offset 714

    Ok(tags)
}

fn process_nikon_flashinfo0100(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashFocalLength at offset 11

    // RepeatingFlashRate at offset 12

    // RepeatingFlashCount at offset 13

    // FlashGNDistance at offset 14

    // FlashGroupAControlMode at offset 15

    // FlashGroupBControlMode at offset 16

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashFlags at offset 8

    // FlashCommanderMode at offset 9.1

    // FlashControlMode at offset 9.2

    Ok(tags)
}

fn process_nikon_flashinfo0102(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashFocalLength at offset 12

    // RepeatingFlashRate at offset 13

    // RepeatingFlashCount at offset 14

    // FlashGNDistance at offset 15

    // FlashGroupAControlMode at offset 16.1

    // FlashGroupBControlMode at offset 17.1

    // FlashGroupCControlMode at offset 17.2

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashFlags at offset 8

    // FlashCommanderMode at offset 9.1

    // FlashControlMode at offset 9.2

    Ok(tags)
}

fn process_nikon_flashinfo0103(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashFocalLength at offset 12

    // RepeatingFlashRate at offset 13

    // RepeatingFlashCount at offset 14

    // FlashGNDistance at offset 15

    // FlashColorFilter at offset 16

    // FlashGroupAControlMode at offset 17.1

    // FlashGroupBControlMode at offset 18.1

    // FlashGroupCControlMode at offset 18.2

    // ExternalFlashCompensation at offset 27
    if data.len() >= 55 {
        // TODO: Handle format int8s
    }

    // FlashExposureComp3 at offset 29
    if data.len() >= 59 {
        // TODO: Handle format int8s
    }

    // FlashExposureComp4 at offset 39
    if data.len() >= 79 {
        // TODO: Handle format int8s
    }

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashFlags at offset 8

    // FlashCommanderMode at offset 9.1

    // FlashControlMode at offset 9.2

    Ok(tags)
}

fn process_nikon_flashinfo0106(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashFocalLength at offset 12

    // RepeatingFlashRate at offset 13

    // RepeatingFlashCount at offset 14

    // FlashGNDistance at offset 15

    // FlashColorFilter at offset 16

    // FlashGroupAControlMode at offset 17.1

    // FlashGroupBControlMode at offset 18.1

    // FlashGroupCControlMode at offset 18.2

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashFlags at offset 8

    // FlashCommanderMode at offset 9.1

    // FlashControlMode at offset 9.2

    Ok(tags)
}

fn process_nikon_flashinfo0107(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashCompensation at offset 10
    if data.len() >= 21 {
        // TODO: Handle format int8s
    }

    // FlashFocalLength at offset 12

    // RepeatingFlashRate at offset 13

    // RepeatingFlashCount at offset 14

    // FlashGNDistance at offset 15

    // FlashGroupAControlMode at offset 17.1

    // FlashGroupBControlMode at offset 18.1

    // FlashGroupCControlMode at offset 18.2

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashZoomOverride at offset 8.1

    // ExternalFlashStatus at offset 8.2

    // ExternalFlashReadyState at offset 9.1

    Ok(tags)
}

fn process_nikon_flashinfo0300(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // FlashCompensation at offset 10
    if data.len() >= 21 {
        // TODO: Handle format int8s
    }

    // RepeatingFlashRate at offset 13

    // RepeatingFlashCount at offset 14

    // FlashGNDistance at offset 15

    // FlashColorFilter at offset 16

    // FlashGroupAControlMode at offset 17.1

    // FlashGroupBControlMode at offset 18.1

    // FlashGroupCControlMode at offset 18.2

    // FlashOutput at offset 33

    // FlashIlluminationPattern at offset 37

    // FlashFocalLength at offset 38

    // FlashSource at offset 4

    // ExternalFlashFirmware at offset 6
    if data.len() >= 14 {
        // TODO: Handle format int8u
    }

    // ExternalFlashFlags at offset 8

    // FlashCommanderMode at offset 9.1

    // FlashControlMode at offset 9.2

    Ok(tags)
}

fn process_nikon_flashinfounknown(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FlashInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    Ok(tags)
}

fn process_nikon_otherinfod500(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // NikonMeteringMode at offset 532

    Ok(tags)
}

fn process_nikon_multiexposure(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MultiExposureVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // MultiExposureMode at offset 1

    // MultiExposureAutoGain at offset 3

    Ok(tags)
}

fn process_nikon_multiexposure2(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // MultiExposureVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format string
    }

    // MultiExposureMode at offset 1

    // MultiExposureOverlayMode at offset 3

    Ok(tags)
}

fn process_nikon_afinfo2v0100(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AFImageWidth at offset 16
    if data.len() >= 34 {
        // TODO: Handle format int16u
    }

    // AFImageHeight at offset 18
    if data.len() >= 38 {
        // TODO: Handle format int16u
    }

    // AFAreaXPosition at offset 20
    if data.len() >= 42 {
        // TODO: Handle format int16u
    }

    // AFAreaYPosition at offset 22
    if data.len() >= 46 {
        // TODO: Handle format int16u
    }

    // AFAreaWidth at offset 24
    if data.len() >= 50 {
        // TODO: Handle format int16u
    }

    // AFAreaHeight at offset 26
    if data.len() >= 54 {
        // TODO: Handle format int16u
    }

    // AFDetectionMethod at offset 4

    // FocusPointSchema at offset 6

    Ok(tags)
}

fn process_nikon_afinfo2v0101(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AFDetectionMethod at offset 4

    // FocusPointSchema at offset 6

    // AFImageWidth at offset 70
    if data.len() >= 142 {
        // TODO: Handle format int16u
    }

    // AFImageHeight at offset 72
    if data.len() >= 146 {
        // TODO: Handle format int16u
    }

    // AFAreaXPosition at offset 74
    if data.len() >= 150 {
        // TODO: Handle format int16u
    }

    // AFAreaYPosition at offset 76
    if data.len() >= 154 {
        // TODO: Handle format int16u
    }

    // AFAreaWidth at offset 78
    if data.len() >= 158 {
        // TODO: Handle format int16u
    }

    // AFAreaHeight at offset 80
    if data.len() >= 162 {
        // TODO: Handle format int16u
    }

    // ContrastDetectAFInFocus at offset 82

    Ok(tags)
}

fn process_nikon_afinfo2v0200(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AFAreaMode at offset 5

    // PhaseDetectAF at offset 6

    Ok(tags)
}

fn process_nikon_afinfo2v0300(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AFDetectionMethod at offset 4

    // AFImageWidth at offset 42
    if data.len() >= 86 {
        // TODO: Handle format int16u
    }

    // AFImageHeight at offset 44
    if data.len() >= 90 {
        // TODO: Handle format int16u
    }

    // AFAreaXPosition at offset 46
    if data.len() >= 94 {
        // TODO: Handle format int16u
    }

    // AFAreaYPosition at offset 48
    if data.len() >= 98 {
        // TODO: Handle format int16u
    }

    // AFAreaWidth at offset 50
    if data.len() >= 102 {
        // TODO: Handle format int16u
    }

    // AFAreaHeight at offset 52
    if data.len() >= 106 {
        // TODO: Handle format int16u
    }

    // FocusPointSchema at offset 6

    // AFCoordinatesAvailable at offset 7

    Ok(tags)
}

fn process_nikon_afinfo2v0400(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFInfo2Version at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // AFDetectionMethod at offset 4

    // AFAreaMode at offset 5

    // AFImageWidth at offset 62
    if data.len() >= 126 {
        // TODO: Handle format int16u
    }

    // AFImageHeight at offset 64
    if data.len() >= 130 {
        // TODO: Handle format int16u
    }

    // AFAreaXPosition at offset 66
    if data.len() >= 134 {
        // TODO: Handle format int16u
    }

    // AFAreaYPosition at offset 68
    if data.len() >= 138 {
        // TODO: Handle format int16u
    }

    // AFCoordinatesAvailable at offset 7

    // AFAreaWidth at offset 70
    if data.len() >= 142 {
        // TODO: Handle format int16u
    }

    // AFAreaHeight at offset 72
    if data.len() >= 146 {
        // TODO: Handle format int16u
    }

    // FocusResult at offset 74

    Ok(tags)
}

fn process_nikon_fileinfo(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // FileInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // DirectoryNumber at offset 3

    // FileNumber at offset 4

    Ok(tags)
}

fn process_nikon_aftune(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // AFFineTune at offset 0

    // AFFineTuneIndex at offset 1

    // AFFineTuneAdj at offset 2
    if data.len() >= 5 {
        // TODO: Handle format int8s
    }

    // AFFineTuneAdjTele at offset 3
    if data.len() >= 7 {
        // TODO: Handle format int8s
    }

    Ok(tags)
}

fn process_nikon_retouchinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // RetouchInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // RetouchNEFProcessing at offset 5

    Ok(tags)
}

fn process_nikon_barometerinfo(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // BarometerInfoVersion at offset 0
    if data.len() >= 8 {
        // TODO: Handle format undef
    }

    // Altitude at offset 6
    if data.len() >= 16 {
        // TODO: Handle format int32s
    }

    Ok(tags)
}

fn process_nikon_roc(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // DigitalROC at offset 0

    Ok(tags)
}

fn process_nikon_gem(data: &[u8], byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    // DigitalGEM at offset 0

    Ok(tags)
}

fn process_nikon_captureoffsets(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    Ok(tags)
}

fn process_nikon_captureoutput(
    data: &[u8],
    byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    let mut tags = Vec::new();
    Ok(tags)
}

// Functions for tables not extracted by tag kit
fn process_nikon_previewifd(
    data: &[u8],
    _byte_order: ByteOrder,
) -> Result<Vec<(String, TagValue)>> {
    // TODO: Implement when this table is extracted
    tracing::debug!("Stub function called for {}", data.len());
    Ok(vec![])
}

fn process_nikon_scan(data: &[u8], _byte_order: ByteOrder) -> Result<Vec<(String, TagValue)>> {
    // TODO: Implement when this table is extracted
    tracing::debug!("Stub function called for {}", data.len());
    Ok(vec![])
}

pub fn process_tag_0x0_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x0_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module model condition: $$self{Model} =~ /\\bD5\\b/
    // Would dispatch to: Image::ExifTool::NikonCustom::SettingsD5
    Ok(vec![])
}

pub fn process_tag_0x5_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x5_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_distortioninfo(data, byte_order)
}

pub fn process_tag_0x6_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x6_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_vignetteinfo(data, byte_order)
}

pub fn process_tag_0x10_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x10_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} lt \"02.00\"
    // Would dispatch to: Image::ExifTool::Nikon::MenuSettingsZ8v1
    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} ge \"02.00\"
    // Would dispatch to: Image::ExifTool::Nikon::MenuSettingsZ8v2
    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} lt \"03.00\"
    // Would dispatch to: Image::ExifTool::Nikon::MenuSettingsZ9
    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} lt \"04.00\"
    // Would dispatch to: Image::ExifTool::Nikon::MenuSettingsZ9v3
    Ok(vec![])
}

pub fn process_tag_0x11_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x11_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_previewifd(data, byte_order)
}

pub fn process_tag_0x14_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x14_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Format condition not yet supported: $format eq \"undef\" and $count == 2560
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalanceA
    // Runtime condition not yet supported: $$valPt =~ /^NRW 0100/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalanceB
    // Runtime condition not yet supported: $$valPt =~ /^NRW /
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalanceC
    Ok(vec![])
}

pub fn process_tag_0x1f_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x1f_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_vrinfo(data, byte_order)
}

pub fn process_tag_0x21_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x21_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_facedetect(data, byte_order)
}

pub fn process_tag_0x23_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x23_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^01/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl
    // Runtime condition not yet supported: $$valPt =~ /^02/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl2
    // Runtime condition not yet supported: $$valPt =~ /^03/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl3
    Ok(vec![])
}

pub fn process_tag_0x24_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x24_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x25_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x25_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_isoinfo(data, byte_order)
}

pub fn process_tag_0x2b_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2b_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_distortinfo(data, byte_order)
}

pub fn process_tag_0x2c_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2c_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x30_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x30_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x32_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x32_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_unknowninfo2(data, byte_order)
}

pub fn process_tag_0x35_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x35_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $count != 6
    // Would dispatch to: Image::ExifTool::Nikon::HDRInfo
    Ok(vec![])
}

pub fn process_tag_0x38_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x38_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x39_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x39_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_locationinfo(data, byte_order)
}

pub fn process_tag_0x40_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x40_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD810
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x4c_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x4c_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_moresettingsd850(data, byte_order)
}

pub fn process_tag_0x4e_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x4e_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonSettings::Main
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x50_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x50_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_shootingmenud500(data, byte_order)
}

pub fn process_tag_0x51_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x51_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_makernotes0x51(data, byte_order)
}

pub fn process_tag_0x56_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x56_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_makernotes0x56(data, byte_order)
}

pub fn process_tag_0x58_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x58_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} ge \"03.01\"
    // Would dispatch to: Image::ExifTool::Nikon::Offset13InfoZ9
    Ok(vec![])
}

pub fn process_tag_0x80_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x80_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} ge \"02.00\"
    // Would dispatch to: Image::ExifTool::Nikon::AutoCaptureInfo
    // Runtime condition not yet supported: $$self{FirmwareVersion} and $$self{FirmwareVersion} ge \"04.00\"
    // Would dispatch to: Image::ExifTool::Nikon::AutoCaptureInfo
    Ok(vec![])
}

pub fn process_tag_0x84_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x84_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{ShutterMode} and $$self{ShutterMode} ne 96
    // Would dispatch to: Image::ExifTool::Nikon::OrientationInfo
    // Runtime condition not yet supported: $$self{ShutterMode} and $$self{ShutterMode} ne 96
    // Would dispatch to: Image::ExifTool::Nikon::OrientationInfo
    Ok(vec![])
}

pub fn process_tag_0x88_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x88_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Model condition: $$self{Model} =~ /^NIKON D/i
    if let Some(model) = &model {
        let mut evaluator = ExpressionEvaluator::new();
        let processor_context = ProcessorContext::default().with_model(model.to_string());
        if evaluator
            .evaluate_context_condition(&processor_context, "$$self{Model} =~ /^NIKON D/i")
            .unwrap_or(false)
        {
            debug!("Model condition matched: $$self{{Model}} =~ /^NIKON D/i for nikon_afinfo");
            return process_nikon_afinfo(data, byte_order);
        }
    }
    // Model condition: $$self{Model} =~ /^NIKON Z f\\b/i
    if let Some(model) = &model {
        let mut evaluator = ExpressionEvaluator::new();
        let processor_context = ProcessorContext::default().with_model(model.to_string());
        if evaluator
            .evaluate_context_condition(&processor_context, "$$self{Model} =~ /^NIKON Z f\\b/i")
            .unwrap_or(false)
        {
            debug!("Model condition matched: $$self{{Model}} =~ /^NIKON Z f\\b/i for nikon_orientationinfo");
            return process_nikon_orientationinfo(data, byte_order);
        }
    }
    // Runtime condition not yet supported: $$self{ShutterMode} and $$self{ShutterMode} ne 96
    // Would dispatch to: Image::ExifTool::Nikon::OrientationInfo
    Ok(vec![])
}

pub fn process_tag_0x8c_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x8c_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x90_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x90_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Model condition: $$self{Model} =~ /^NIKON Z6_3\\b/i
    if let Some(model) = &model {
        let mut evaluator = ExpressionEvaluator::new();
        let processor_context = ProcessorContext::default().with_model(model.to_string());
        if evaluator
            .evaluate_context_condition(&processor_context, "$$self{Model} =~ /^NIKON Z6_3\\b/i")
            .unwrap_or(false)
        {
            debug!("Model condition matched: $$self{{Model}} =~ /^NIKON Z6_3\\b/i for nikon_menusettingsz6iii");
            return process_nikon_menusettingsz6iii(data, byte_order);
        }
    }
    Ok(vec![])
}

pub fn process_tag_0x91_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x91_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0209/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD40
    // Runtime condition not yet supported: $$valPt =~ /^0208/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD80
    // Runtime condition not yet supported: $$valPt =~ /^0213/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD90
    // Runtime condition not yet supported: $$valPt =~ /^0245/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD780
    // Runtime condition not yet supported: $$valPt =~ /^0242/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD7500
    // Runtime condition not yet supported: $$valPt =~ /^0222/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD800
    // Runtime condition not yet supported: $$valPt =~ /^0233/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD810
    // Runtime condition not yet supported: $$valPt =~ /^0243/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD850
    // Runtime condition not yet supported: $$valPt =~ /^0220/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD7000
    // Runtime condition not yet supported: $$valPt =~ /^0223/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD4
    // Runtime condition not yet supported: $$valPt =~ /^0231/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD4S
    // Runtime condition not yet supported: $$valPt =~ /^023[89]/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD500
    // Runtime condition not yet supported: $$valPt =~ /^0246/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD6
    // Runtime condition not yet supported: $$valPt =~ /^0232/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoD610
    // Runtime condition not yet supported: $$valPt =~ /^08(09|10|11)/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoZ6III
    // Runtime condition not yet supported: $$valPt =~ /^080[0123478]/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoZ7II
    // Runtime condition not yet supported: $$valPt =~ /^0806/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoZ8
    // Runtime condition not yet supported: $$valPt =~ /^0805/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfoZ9
    // Runtime condition not yet supported: $$valPt =~ /^0[28]/
    // Would dispatch to: Image::ExifTool::Nikon::ShotInfo
    match count {
        5399 => {
            debug!("Matched count 5399 for variant nikon_shotinfod3a");
            process_nikon_shotinfod3a(data, byte_order)
        }
        5408 => {
            debug!("Matched count 5408 for variant nikon_shotinfod3b");
            process_nikon_shotinfod3b(data, byte_order)
        }
        5409 => {
            debug!("Matched count 5409 for variant nikon_shotinfod3x");
            process_nikon_shotinfod3x(data, byte_order)
        }
        5356 => {
            debug!("Matched count 5356 for variant nikon_shotinfod3s");
            process_nikon_shotinfod3s(data, byte_order)
        }
        5291 => {
            debug!("Matched count 5291 for variant nikon_shotinfod300a");
            process_nikon_shotinfod300a(data, byte_order)
        }
        5303 => {
            debug!("Matched count 5303 for variant nikon_shotinfod300b");
            process_nikon_shotinfod300b(data, byte_order)
        }
        5311 => {
            debug!("Matched count 5311 for variant nikon_shotinfod300s");
            process_nikon_shotinfod300s(data, byte_order)
        }
        5312 => {
            debug!("Matched count 5312 for variant nikon_shotinfod700");
            process_nikon_shotinfod700(data, byte_order)
        }
        6745 => {
            debug!("Matched count 6745 for variant nikon_shotinfod5000");
            process_nikon_shotinfod5000(data, byte_order)
        }
        8902 => {
            debug!("Matched count 8902 for variant nikon_shotinfod5100");
            process_nikon_shotinfod5100(data, byte_order)
        }
        11587 => {
            debug!("Matched count 11587 for variant nikon_shotinfod5200");
            process_nikon_shotinfod5200(data, byte_order)
        }
        _ => Ok(vec![]), // No matching variant
    }
}

pub fn process_tag_0x97_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x97_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance1
    // Runtime condition not yet supported: $$valPt =~ /^0102/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance2
    // Runtime condition not yet supported: $$valPt =~ /^0103/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance3
    // Runtime condition not yet supported: $$valPt =~ /^0205/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance2
    // Runtime condition not yet supported: $$valPt =~ /^02(09|12|14)/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance4
    // Runtime condition not yet supported: $$valPt =~ /^02(\\d{2})/ and $1 < 11
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance2
    // Runtime condition not yet supported: $$valPt =~ /^0211/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance4
    // Runtime condition not yet supported: $$valPt =~ /^0213/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance2
    // Runtime condition not yet supported: $$valPt =~ /^021[567]/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance4
    // Runtime condition not yet supported: $$valPt =~ /^02(19|2[1234])/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalance2
    // Runtime condition not yet supported: $$valPt =~ /^0(220|6)/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalanceUnknown
    // Runtime condition not yet supported: $$valPt =~ /^0(18|[248])/
    // Would dispatch to: Image::ExifTool::Nikon::ColorBalanceUnknown2
    Ok(vec![])
}

pub fn process_tag_0x98_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x98_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Model condition: $$self{Model} =~ /^NIKON Z (30|5|50|6|6_2|7|7_2|8|fc)\\b/i
    if let Some(model) = &model {
        let mut evaluator = ExpressionEvaluator::new();
        let processor_context = ProcessorContext::default().with_model(model.to_string());
        if evaluator
            .evaluate_context_condition(
                &processor_context,
                "$$self{Model} =~ /^NIKON Z (30|5|50|6|6_2|7|7_2|8|fc)\\b/i",
            )
            .unwrap_or(false)
        {
            debug!("Model condition matched: $$self{{Model}} =~ /^NIKON Z (30|5|50|6|6_2|7|7_2|8|fc)\\b/i for nikon_orientationinfo");
            return process_nikon_orientationinfo(data, byte_order);
        }
    }
    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::LensData00
    // Runtime condition not yet supported: $$valPt =~ /^0101/
    // Would dispatch to: Image::ExifTool::Nikon::LensData01
    // Runtime condition not yet supported: $$valPt =~ /^020[1-3]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData01
    // Runtime condition not yet supported: $$valPt =~ /^0204/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0204
    // Runtime condition not yet supported: $$valPt =~ /^040[01]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0400
    // Runtime condition not yet supported: $$valPt =~ /^0402/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0402
    // Runtime condition not yet supported: $$valPt =~ /^0403/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0403
    // Runtime condition not yet supported: $$valPt =~ /^080[012]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0800
    Ok(vec![])
}

pub fn process_tag_0x9c_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x9c_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0xa0_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xa0_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0xa4_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xa4_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_intervalinfod6(data, byte_order)
}

pub fn process_tag_0xa8_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xa8_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^010[01]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0100
    // Runtime condition not yet supported: $$valPt =~ /^0102/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0102
    // Runtime condition not yet supported: $$valPt =~ /^010[345]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0103
    // Runtime condition not yet supported: $$valPt =~ /^0106/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0106
    // Runtime condition not yet supported: $$valPt =~ /^010[78]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0107
    // Runtime condition not yet supported: $$valPt =~ /^030[01]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0300
    Ok(vec![])
}

pub fn process_tag_0xb0_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xb0_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::MultiExposure
    // Runtime condition not yet supported: $$valPt =~ /^0101/
    // Would dispatch to: Image::ExifTool::Nikon::MultiExposure
    // Runtime condition not yet supported: $$valPt =~ /^010[23]/
    // Would dispatch to: Image::ExifTool::Nikon::MultiExposure2
    Ok(vec![])
}

pub fn process_tag_0xb7_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xb7_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0100
    // Runtime condition not yet supported: $$valPt =~ /^0101/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0101
    // Runtime condition not yet supported: $$valPt =~ /^020[01]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0200
    // Runtime condition not yet supported: $$valPt =~ /^030[01]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0300
    // Runtime condition not yet supported: $$valPt =~ /^040[012]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0400
    Ok(vec![])
}

pub fn process_tag_0xb8_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xb8_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Model condition: if (length($$valPt) >= 0) { my ($dir, $file) = unpack('x6vv', $$valPt); my $littleEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); ($dir, $file) = unpack('x6nn', $$valPt); my $bigEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); return $littleEndian if $littleEndian xor $bigEndian; } return $$self{Model} =~ /^NIKON (D4S|D750|D810|D3300|D5200|D5300|D5500|D7100)$/;
    if let Some(model) = &model {
        let mut evaluator = ExpressionEvaluator::new();
        let processor_context = ProcessorContext::default().with_model(model.to_string());
        if evaluator.evaluate_context_condition(&processor_context, "if (length($$valPt) >= 0) { my ($dir, $file) = unpack('x6vv', $$valPt); my $littleEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); ($dir, $file) = unpack('x6nn', $$valPt); my $bigEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); return $littleEndian if $littleEndian xor $bigEndian; } return $$self{Model} =~ /^NIKON (D4S|D750|D810|D3300|D5200|D5300|D5500|D7100)$/;").unwrap_or(false) {
            debug!("Model condition matched: if (length($$valPt) >= 0) {{ my ($dir, $file) = unpack('x6vv', $$valPt); my $littleEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); ($dir, $file) = unpack('x6nn', $$valPt); my $bigEndian = ($dir >= 100 and $dir <= 999 and $file <= 9999); return $littleEndian if $littleEndian xor $bigEndian; }} return $$self{{Model}} =~ /^NIKON (D4S|D750|D810|D3300|D5200|D5300|D5500|D7100)$/; for nikon_fileinfo");
            return process_nikon_fileinfo(data, byte_order);
        }
    }
    Ok(vec![])
}

pub fn process_tag_0xb9_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xb9_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_aftune(data, byte_order)
}

pub fn process_tag_0xbb_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xbb_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_retouchinfo(data, byte_order)
}

pub fn process_tag_0xbd_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xbd_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_picturecontrol(data, byte_order)
}

pub fn process_tag_0xc3_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xc3_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_barometerinfo(data, byte_order)
}

pub fn process_tag_0x110_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x110_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_roc(data, byte_order)
}

pub fn process_tag_0x120_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x120_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_gem(data, byte_order)
}

pub fn process_tag_0x2ce_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2ce_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x2d9_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2d9_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD40
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x2ec_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2ec_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD80
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x301_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x301_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x30a_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x30a_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x30b_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x30b_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x316_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x316_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x31f_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x31f_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsZ9
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x322_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x322_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD3
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x324_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x324_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x34f_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x34f_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x374_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x374_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD90
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x378_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x378_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD5000
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x3af_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x3af_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    Ok(vec![])
}

pub fn process_tag_0x404_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x404_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD7000
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x407_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x407_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD5100
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x4e7_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x4e7_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsZ6III
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x6ec_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x6ec_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD800
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x751_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x751_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD4
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x7cf_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x7cf_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD610
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0xcd5_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xcd5_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCustom::SettingsD5200
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0xe00_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe00_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::PrintIM::Main
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0xe01_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe01_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonCapture::Main
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0xe0e_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe0e_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_captureoffsets(data, byte_order)
}

pub fn process_tag_0xe10_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe10_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_scan(data, byte_order)
}

pub fn process_tag_0xe13_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe13_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $self->Options(\"ExtractEmbedded\")
    // Would dispatch to: Image::ExifTool::NikonCapture::Main
    Ok(vec![])
}

pub fn process_tag_0xe1d_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe1d_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::ICC_Profile::Main
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0xe1e_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0xe1e_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_captureoutput(data, byte_order)
}

pub fn process_tag_0x189d_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x189d_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{FirmwareVersion} =~ /^1\\.00/
    // Would dispatch to: Image::ExifTool::NikonCustom::SettingsD4
    Ok(vec![])
}

pub fn process_tag_0x193d_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x193d_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$self{FirmwareVersion} !~ /^1\\.00/
    // Would dispatch to: Image::ExifTool::NikonCustom::SettingsD4
    Ok(vec![])
}

pub fn process_tag_0x350b_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x350b_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_orientationinfo(data, byte_order)
}

pub fn process_tag_0x200001f_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x200001f_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_vrinfo(data, byte_order)
}

pub fn process_tag_0x2000023_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000023_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^01/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl
    // Runtime condition not yet supported: $$valPt =~ /^02/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl2
    // Runtime condition not yet supported: $$valPt =~ /^03/
    // Would dispatch to: Image::ExifTool::Nikon::PictureControl3
    Ok(vec![])
}

pub fn process_tag_0x2000024_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000024_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_worldtime(data, byte_order)
}

pub fn process_tag_0x2000025_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000025_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_isoinfo(data, byte_order)
}

pub fn process_tag_0x200002c_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x200002c_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_unknowninfo(data, byte_order)
}

pub fn process_tag_0x2000032_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000032_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_unknowninfo2(data, byte_order)
}

pub fn process_tag_0x2000039_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000039_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_locationinfo(data, byte_order)
}

pub fn process_tag_0x200004e_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x200004e_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Cross-module reference to Image::ExifTool::NikonSettings::Main
    // TODO: Implement cross-module subdirectory support
    Ok(vec![])
}

pub fn process_tag_0x2000098_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x2000098_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::LensData00
    // Runtime condition not yet supported: $$valPt =~ /^0101/
    // Would dispatch to: Image::ExifTool::Nikon::LensData01
    // Runtime condition not yet supported: $$valPt =~ /^020[1-3]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData01
    // Runtime condition not yet supported: $$valPt =~ /^0204/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0204
    // Runtime condition not yet supported: $$valPt =~ /^040[01]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0400
    // Runtime condition not yet supported: $$valPt =~ /^0402/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0402
    // Runtime condition not yet supported: $$valPt =~ /^0403/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0403
    // Runtime condition not yet supported: $$valPt =~ /^080[012]/
    // Would dispatch to: Image::ExifTool::Nikon::LensData0800
    Ok(vec![])
}

pub fn process_tag_0x20000a8_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x20000a8_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^010[01]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0100
    // Runtime condition not yet supported: $$valPt =~ /^0102/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0102
    // Runtime condition not yet supported: $$valPt =~ /^010[345]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0103
    // Runtime condition not yet supported: $$valPt =~ /^0106/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0106
    // Runtime condition not yet supported: $$valPt =~ /^010[78]/
    // Would dispatch to: Image::ExifTool::Nikon::FlashInfo0107
    Ok(vec![])
}

pub fn process_tag_0x20000b7_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x20000b7_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Runtime condition not yet supported: $$valPt =~ /^0100/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0100
    // Runtime condition not yet supported: $$valPt =~ /^0101/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0101
    // Runtime condition not yet supported: $$valPt =~ /^020[01]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0200
    // Runtime condition not yet supported: $$valPt =~ /^030[01]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0300
    // Runtime condition not yet supported: $$valPt =~ /^040[012]/
    // Would dispatch to: Image::ExifTool::Nikon::AFInfo2V0400
    Ok(vec![])
}

pub fn process_tag_0x20000c3_subdirectory(
    data: &[u8],
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<Vec<(String, TagValue)>> {
    use crate::expressions::ExpressionEvaluator;
    use crate::processor_registry::ProcessorContext;
    use tracing::debug;
    let count = data.len() / 2;
    debug!(
        "process_tag_0x20000c3_subdirectory called with {} bytes, count={}",
        data.len(),
        count
    );

    // Single unconditional subdirectory
    process_nikon_barometerinfo(data, byte_order)
}

/// Apply PrintConv for a tag from this module
pub fn apply_print_conv(
    tag_id: u32,
    value: &TagValue,
    _evaluator: &mut ExpressionEvaluator,
    _errors: &mut Vec<String>,
    warnings: &mut Vec<String>,
) -> TagValue {
    match tag_id {
        8 => crate::implementations::print_conv::exposuretime_print_conv(value),
        10 => crate::implementations::print_conv::print_fraction(value),
        13 => crate::implementations::print_conv::print_fraction(value),
        18 => crate::implementations::print_conv::print_fraction(value),
        23 => crate::implementations::print_conv::print_fraction(value),
        27 => crate::implementations::print_conv::print_fraction(value),
        28 => crate::implementations::print_conv::print_fraction(value),
        29 => crate::implementations::print_conv::print_fraction(value),
        38 => crate::implementations::print_conv::exposuretime_print_conv(value),
        39 => crate::implementations::print_conv::print_fraction(value),
        50 => crate::implementations::print_conv::print_fraction(value),
        1216 => crate::implementations::print_conv::exposuretime_print_conv(value),
        1234 => crate::implementations::print_conv::print_fraction(value),
        17859226 => crate::implementations::print_conv::exposuretime_print_conv(value),
        17859229 => crate::implementations::print_conv::fnumber_print_conv(value),
        17863172 => crate::implementations::print_conv::print_fraction(value),
        17867826 => crate::implementations::print_conv::lensinfo_print_conv(value),
        18874369 => crate::implementations::print_conv::gpslatituderef_print_conv(value),
        18874371 => crate::implementations::print_conv::gpslongituderef_print_conv(value),
        18874373 => crate::implementations::print_conv::gpsaltituderef_print_conv(value),
        18874374 => crate::implementations::print_conv::gpsaltitude_print_conv(value),
        _ => {
            // Fall back to shared handling
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                crate::implementations::generic::apply_fallback_print_conv(
                    tag_id,
                    &tag_kit.name,
                    "Nikon",
                    value,
                    crate::to_print_conv_ref!(&tag_kit.print_conv),
                )
            } else {
                value.clone()
            }
        }
    }
}

/// Apply ValueConv for a tag from this module
pub fn apply_value_conv(
    tag_id: u32,
    value: &TagValue,
    _errors: &mut Vec<String>,
) -> Result<TagValue> {
    match tag_id {
        0 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        1 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        2 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        8 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        10 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        11 => crate::implementations::value_conv::apex_aperture_value_conv(value),
        23 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        24 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        28 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        38 => crate::implementations::value_conv::reciprocal_10_value_conv(value),
        47 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        48 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        49 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        56 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        67 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        69 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        76 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        92 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        96 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        116 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        136 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        139 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        156 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        158 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        176 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        196 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        216 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        256 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        276 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        338 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        343 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        358 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        434 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        454 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        542 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        562 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        734 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        754 => crate::implementations::value_conv::power_neg_div_3_value_conv(value),
        5096 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5112 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5128 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5160 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5176 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5192 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5224 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        5240 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        18874370 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        18874372 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        18874375 => {
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
        18874397 => crate::implementations::value_conv::exif_date_value_conv(value),
        4 => {
            // Compiled arithmetic: $val ? 2048 / $val : $val
            match value.as_f64() {
                Some(val) => Ok(if val != 0.0 {
                    TagValue::F64(2048.0_f64 / val)
                } else {
                    TagValue::F64(val)
                }),
                None => Ok(value.clone()),
            }
        }
        6 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        7 => {
            // Compiled arithmetic: $val / 12
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 12.0_f64)),
                None => Ok(value.clone()),
            }
        }
        9 => {
            // Compiled arithmetic: 5 * 2**($val/24)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(5.0_f64 * 2.0_f64.powf(val / 24.0_f64))),
                None => Ok(value.clone()),
            }
        }
        12 => {
            // Compiled arithmetic: $val / 12
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 12.0_f64)),
                None => Ok(value.clone()),
            }
        }
        13 => {
            // Compiled arithmetic: 5 * 2**($val/24)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(5.0_f64 * 2.0_f64.powf(val / 24.0_f64))),
                None => Ok(value.clone()),
            }
        }
        14 => {
            // Compiled arithmetic: 5 * 2**($val/24)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(5.0_f64 * 2.0_f64.powf(val / 24.0_f64))),
                None => Ok(value.clone()),
            }
        }
        15 => {
            // Compiled arithmetic: 2**($val/24)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(val / 24.0_f64))),
                None => Ok(value.clone()),
            }
        }
        16 => {
            // Compiled arithmetic: 2**($val/24)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(val / 24.0_f64))),
                None => Ok(value.clone()),
            }
        }
        17 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        18 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        19 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        20 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        21 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        27 => {
            // Compiled arithmetic: -$val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(-val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        29 => {
            // Compiled arithmetic: -$val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(-val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        33 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        39 => {
            // Compiled arithmetic: -$val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(-val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        40 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        41 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        42 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        50 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        51 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        52 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        53 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        54 => {
            // Compiled arithmetic: 2**($val/384-1)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(val / 384.0_f64 - 1.0_f64))),
                None => Ok(value.clone()),
            }
        }
        55 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        57 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        59 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        61 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        63 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        65 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        73 => {
            // Compiled arithmetic: $val - 0x80
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 128.0_f64)),
                None => Ok(value.clone()),
            }
        }
        78 => {
            // Compiled arithmetic: 2**(($val-80)/12)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(val - 80.0_f64 / 12.0_f64))),
                None => Ok(value.clone()),
            }
        }
        324 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        326 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        334 => {
            // Compiled arithmetic: $val + 3
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val + 3.0_f64)),
                None => Ok(value.clone()),
            }
        }
        340 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        342 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        344 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        346 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        350 => {
            // Compiled arithmetic: $val / 8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        354 => {
            // Compiled arithmetic: $val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        426 => {
            // Compiled arithmetic: $val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        430 => {
            // Compiled arithmetic: $val + 3
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val + 3.0_f64)),
                None => Ok(value.clone()),
            }
        }
        446 => {
            // Compiled arithmetic: $val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        450 => {
            // Compiled arithmetic: $val + 3
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val + 3.0_f64)),
                None => Ok(value.clone()),
            }
        }
        516 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        538 => {
            // Compiled arithmetic: $val + 3
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val + 3.0_f64)),
                None => Ok(value.clone()),
            }
        }
        545 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        558 => {
            // Compiled arithmetic: $val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        598 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        604 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        605 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        613 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        616 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        620 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        650 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        658 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        662 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        693 => {
            // Compiled arithmetic: 100*exp(($val/12-5)*log(2))
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(
                    100.0_f64 * (((val / 12.0_f64) - 5.0_f64) * 2.0_f64.ln()).exp(),
                )),
                None => Ok(value.clone()),
            }
        }
        730 => {
            // Compiled arithmetic: $val + 3
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val + 3.0_f64)),
                None => Ok(value.clone()),
            }
        }
        742 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        746 => {
            // Compiled arithmetic: ($val-104)/8
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64((val - 104.0_f64) / 8.0_f64)),
                None => Ok(value.clone()),
            }
        }
        750 => {
            // Compiled arithmetic: $val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        1216 => {
            // Compiled arithmetic: 2 ** (-$val/6)
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(2.0_f64.powf(-val / 6.0_f64))),
                None => Ok(value.clone()),
            }
        }
        1234 => {
            // Compiled arithmetic: -$val/6
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(-val / 6.0_f64)),
                None => Ok(value.clone()),
            }
        }
        1434 => {
            // Compiled arithmetic: $val - 5
            match value.as_f64() {
                Some(val) => Ok(TagValue::F64(val - 5.0_f64)),
                None => Ok(value.clone()),
            }
        }
        _ => {
            // Fall back to missing handler for unknown expressions
            if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
                if let Some(expr) = tag_kit.value_conv {
                    Ok(crate::implementations::missing::missing_value_conv(
                        tag_id,
                        &tag_kit.name,
                        "Nikon",
                        expr,
                        value,
                    ))
                } else {
                    Ok(value.clone())
                }
            } else {
                Ok(value.clone())
            }
        }
    }
}

/// Check if a tag has subdirectory processing
pub fn has_subdirectory(tag_id: u32) -> bool {
    if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
        tag_kit.subdirectory.is_some()
    } else {
        false
    }
}

/// Process subdirectory tags and return multiple extracted tags
pub fn process_subdirectory(
    tag_id: u32,
    value: &TagValue,
    byte_order: ByteOrder,
    model: Option<&str>,
) -> Result<HashMap<String, TagValue>> {
    use tracing::debug;
    let mut result = HashMap::new();

    debug!("process_subdirectory called for tag_id: 0x{:04x}", tag_id);

    if let Some(tag_kit) = NIKON_PM_TAG_KITS.get(&tag_id) {
        if let Some(SubDirectoryType::Binary { processor }) = &tag_kit.subdirectory {
            debug!("Found subdirectory processor for tag_id: 0x{:04x}", tag_id);
            let bytes = match value {
                TagValue::U16Array(arr) => {
                    debug!("Converting U16Array with {} elements to bytes", arr.len());
                    // Convert U16 array to bytes based on byte order
                    let mut bytes = Vec::with_capacity(arr.len() * 2);
                    for val in arr {
                        match byte_order {
                            ByteOrder::LittleEndian => bytes.extend_from_slice(&val.to_le_bytes()),
                            ByteOrder::BigEndian => bytes.extend_from_slice(&val.to_be_bytes()),
                        }
                    }
                    bytes
                }
                TagValue::U8Array(arr) => arr.clone(),
                _ => return Ok(result), // Not array data
            };

            debug!("Calling processor with {} bytes", bytes.len());
            // Process subdirectory and collect all extracted tags
            match processor(&bytes, byte_order, model) {
                Ok(extracted_tags) => {
                    debug!("Processor returned {} tags", extracted_tags.len());
                    for (name, value) in extracted_tags {
                        result.insert(name, value);
                    }
                }
                Err(e) => {
                    debug!("Processor error: {:?}", e);
                }
            }
        } else {
            debug!(
                "No subdirectory processor found for tag_id: 0x{:04x}",
                tag_id
            );
        }
    } else {
        debug!("Tag not found in TAG_KITS: 0x{:04x}", tag_id);
    }

    Ok(result)
}
