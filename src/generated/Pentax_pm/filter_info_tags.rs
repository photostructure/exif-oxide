//! Generated tag table definitions from Pentax::FilterInfo
//!
//! This file is auto-generated by codegen/src/strategies/tag_kit.rs. Do not edit manually.

use crate::types::{PrintConv, TagInfo, ValueConv};
use std::collections::HashMap;
use std::sync::LazyLock;

/// Tag definitions for Pentax::FilterInfo table
pub static PENTAX_FILTERINFO_TAGS: LazyLock<HashMap<u16, TagInfo>> = LazyLock::new(|| {
    HashMap::from([
        (
            0,
            TagInfo {
                name: "SourceDirectoryIndex",
                format: "int16u",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            2,
            TagInfo {
                name: "SourceFileIndex",
                format: "int16u",
                print_conv: None,
                value_conv: None,
            },
        ),
        (
            5,
            TagInfo {
                name: "DigitalFilter01",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            22,
            TagInfo {
                name: "DigitalFilter02",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            39,
            TagInfo {
                name: "DigitalFilter03",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            56,
            TagInfo {
                name: "DigitalFilter04",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            73,
            TagInfo {
                name: "DigitalFilter05",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            90,
            TagInfo {
                name: "DigitalFilter06",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            107,
            TagInfo {
                name: "DigitalFilter07",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            124,
            TagInfo {
                name: "DigitalFilter08",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            141,
            TagInfo {
                name: "DigitalFilter09",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            158,
            TagInfo {
                name: "DigitalFilter10",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            175,
            TagInfo {
                name: "DigitalFilter11",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            192,
            TagInfo {
                name: "DigitalFilter12",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            209,
            TagInfo {
                name: "DigitalFilter13",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            226,
            TagInfo {
                name: "DigitalFilter14",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            243,
            TagInfo {
                name: "DigitalFilter15",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            260,
            TagInfo {
                name: "DigitalFilter16",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            277,
            TagInfo {
                name: "DigitalFilter17",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            294,
            TagInfo {
                name: "DigitalFilter18",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            311,
            TagInfo {
                name: "DigitalFilter19",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
        (
            328,
            TagInfo {
                name: "DigitalFilter20",
                format: "undef[17]",
                print_conv: Some(PrintConv::Simple(std::collections::HashMap::from([
                    ("0".to_string(), "Off"),
                    ("1".to_string(), "Base Parameter Adjust"),
                    ("10".to_string(), "Toy Camera"),
                    ("11".to_string(), "Retro"),
                    ("12".to_string(), "Pastel"),
                    ("13".to_string(), "Water Color"),
                    ("14".to_string(), "HDR"),
                    ("16".to_string(), "Miniature"),
                    ("17".to_string(), "Starburst"),
                    ("18".to_string(), "Posterization"),
                    ("19".to_string(), "Sketch Filter"),
                    ("2".to_string(), "Soft Focus"),
                    ("20".to_string(), "Shading"),
                    ("21".to_string(), "Invert Color"),
                    ("23".to_string(), "Tone Expansion"),
                    ("254".to_string(), "Custom Filter"),
                    ("27".to_string(), "Unicolor Bold"),
                    ("28".to_string(), "Bold Monochrome"),
                    ("29".to_string(), "Replace Color"),
                    ("3".to_string(), "High Contrast"),
                    ("4".to_string(), "Color Filter"),
                    ("5".to_string(), "Extract Color"),
                    ("6".to_string(), "Monochrome"),
                    ("7".to_string(), "Slim"),
                    ("9".to_string(), "Fisheye"),
                    (
                        "OTHER".to_string(),
                        "[Function: Image::ExifTool::Pentax::PrintFilter]",
                    ),
                ]))),
                value_conv: None,
            },
        ),
    ])
});

/// Apply ValueConv transformation for tags in this table
pub fn apply_value_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
) -> Result<crate::types::TagValue, crate::types::ExifError> {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = PENTAX_FILTERINFO_TAGS.get(&tag_id_u16) {
        if let Some(ref value_conv) = tag_def.value_conv {
            match value_conv {
                ValueConv::None => Ok(value.clone()),
                ValueConv::Function(func) => func(value, None),
                ValueConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    Err(crate::types::ExifError::NotImplemented("Runtime expression evaluation not supported - should be handled by PPI at build time".to_string()))
                }
                _ => Ok(value.clone()),
            }
        } else {
            Ok(value.clone())
        }
    } else {
        Err(crate::types::ExifError::ParseError(format!(
            "Tag 0x{:04x} not found in table",
            tag_id
        )))
    }
}

/// Apply PrintConv transformation for tags in this table
pub fn apply_print_conv(
    tag_id: u32,
    value: &crate::types::TagValue,
    _errors: &mut Vec<String>,
    _warnings: &mut Vec<String>,
) -> crate::types::TagValue {
    let tag_id_u16 = tag_id as u16;
    if let Some(tag_def) = PENTAX_FILTERINFO_TAGS.get(&tag_id_u16) {
        if let Some(ref print_conv) = tag_def.print_conv {
            match print_conv {
                PrintConv::None => value.clone(),
                PrintConv::Function(func) => func(value, None),
                PrintConv::Simple(lookup) => {
                    // Look up value in the hash map
                    // ExifTool uses the stringified value as the key
                    let key = value.to_string();
                    if let Some(display_value) = lookup.get(&key) {
                        crate::types::TagValue::String(display_value.to_string())
                    } else {
                        // Key not found - return original value
                        value.clone()
                    }
                }
                PrintConv::Expression(_expr) => {
                    // Runtime expression evaluation removed - all Perl interpretation happens via PPI at build time
                    value.clone() // Fallback to original value when expression not handled by PPI
                }
                _ => value.clone(),
            }
        } else {
            value.clone()
        }
    } else {
        value.clone()
    }
}
