...
//!
//! Auto-generated from third-party/exiftool/lib/Image/ExifTool/Canon.pm (table: Main)
...
static MAIN_CANON_MODEL_I_D_DATA: &[(u32, &'static str)] = &[
    (1042, "EOS M50 / Kiss M"),
...
pub static MAIN_CANON_MODEL_I_D: LazyLock<HashMap<u32, &'static str>> =
    LazyLock::new(|| MAIN_CANON_MODEL_I_D_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__canon_model_i_d(key: u32) -> Option<&'static str> {
    MAIN_CANON_MODEL_I_D.get(&key).copied()
}

/// Raw data (3 entries)
static MAIN_COLOR_SPACE_DATA: &[(i32, &'static str)] =
    &[(1, "sRGB"), (2, "Adobe RGB"), (65535, "n/a")];

/// Lookup table (lazy-initialized)
pub static MAIN_COLOR_SPACE: LazyLock<HashMap<i32, &'static str>> =
    LazyLock::new(|| MAIN_COLOR_SPACE_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__color_space(key: i32) -> Option<&'static str> {
    MAIN_COLOR_SPACE.get(&key).copied()
}

/// Raw data (2 entries)
static MAIN_SERIAL_NUMBER_FORMAT_DATA: &[(u32, &'static str)] =
    &[(2415919104, "Format 1"), (2684354560, "Format 2")];

/// Lookup table (lazy-initialized)
pub static MAIN_SERIAL_NUMBER_FORMAT: LazyLock<HashMap<u32, &'static str>> =
    LazyLock::new(|| MAIN_SERIAL_NUMBER_FORMAT_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__serial_number_format(key: u32) -> Option<&'static str> {
    MAIN_SERIAL_NUMBER_FORMAT.get(&key).copied()
}

/// Raw data (3 entries)
static MAIN_SUPER_MACRO_DATA: &[(u8, &'static str)] = &[(0, "Off"), (1, "On (1)"), (2, "On (2)")];

/// Lookup table (lazy-initialized)
pub static MAIN_SUPER_MACRO: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| MAIN_SUPER_MACRO_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__super_macro(key: u8) -> Option<&'static str> {
    MAIN_SUPER_MACRO.get(&key).copied()
}

/// Raw data (3 entries)
static MAIN_DATE_STAMP_MODE_DATA: &[(u8, &'static str)] =
    &[(0, "Off"), (1, "Date"), (2, "Date & Time")];

/// Lookup table (lazy-initialized)
pub static MAIN_DATE_STAMP_MODE: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| MAIN_DATE_STAMP_MODE_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__date_stamp_mode(key: u8) -> Option<&'static str> {
    MAIN_DATE_STAMP_MODE.get(&key).copied()
}

/// Raw data (1 entries)
static MAIN_CATEGORIES_DATA: &[(u8, &'static str)] = &[(0, "(none)")];

/// Lookup table (lazy-initialized)
pub static MAIN_CATEGORIES: LazyLock<HashMap<u8, &'static str>> =
    LazyLock::new(|| MAIN_CATEGORIES_DATA.iter().cloned().collect());

/// Look up value by key
pub fn lookup_main__categories(key: u8) -> Option<&'static str> {
    MAIN_CATEGORIES.get(&key).copied()
}
