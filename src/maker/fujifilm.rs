//! Fujifilm maker note parser using table-driven approach
//!
//! Fujifilm maker notes use a proprietary format that starts with "FUJIFILM"
//! signature followed by IFD-like structures. This format is consistent
//! across Fujifilm cameras and contains camera-specific settings including
...
use crate::maker::fujifilm::detection::{detect_fujifilm_maker_note, FUJIFILMDetectionResult};

pub mod detection;
use crate::maker::MakerNoteParser;
use crate::tables::fujifilm_tags::get_fujifilm_tag;
...
impl MakerNoteParser for FujifilmMakerNoteParser {
    fn parse(
        ...
        // Use generated detection logic to identify Fujifilm maker note format
        let detection = match detect_fujifilm_maker_note(data) {
            Some(detection) => detection,
            None => {
                // Fallback: assume standard IFD at start of data
                FUJIFILMDetectionResult {
                    version: None,
                    ifd_offset: 0,
                    description: "Fallback Fujifilm parser".to_string(),
                }
            }
        };

        // Fujifilm maker notes start with "FUJIFILM" signature (8 bytes)
        // The IFD starts immediately after the signature
        let ifd_offset = if data.len() >= 8 && data.starts_with(b"FUJIFILM") {
            8 // Skip "FUJIFILM" signature
        } else {
        ...
        parse_fujifilm_ifd_with_tables(ifd_data, byte_order)
    }
...
/// Parse Fujifilm IFD using generated tag tables and print conversion
fn parse_fujifilm_ifd_with_tables(
    data: &[u8],
    byte_order: Endian,
) -> Result<HashMap<u16, ExifValue>> {
    ...
    let parsed_ifd = match IfdParser::parse_ifd(&tiff_data, &header, 8) {
        Ok(parsed) => parsed,
        Err(e) => {
            eprintln!("Warning: Fujifilm IFD parsing failed: {}", e);
            return Ok(HashMap::new());
        }
    };

    // Convert raw IFD entries to Fujifilm tags with print conversion
    let mut result = HashMap::new();

    for (tag_id, raw_value) in parsed_ifd.entries() {
        if let Some(fujifilm_tag) = get_fujifilm_tag(*tag_id) {
            // Apply print conversion to create human-readable value
            let converted_value = apply_print_conv(raw_value, fujifilm_tag.print_conv);
...
/// Fujifilm-specific tag IDs
pub mod tags {
    // Main Fujifilm tags from ExifTool
    pub const FUJIFILM_VERSION: u16 = 0x0000;
...
mod tests {
    ...
    #[test]
    fn test_fujifilm_signature_detection() {
        let parser = FujifilmMakerNoteParser;

        // Create test data with FUJIFILM signature
        let mut test_data = vec![0u8; 20];
        test_data[0..8].copy_from_slice(b"FUJIFILM");
    ...
    #[test]
    fn test_fujifilm_detection_pattern() {
        // Test the detection function directly
        let test_data = b"FUJIFILM_test_data";
        let detection = detect_fujifilm_maker_note(test_data);

        assert!(detection.is_some());
        let detection = detection.unwrap();
        assert_eq!(detection.ifd_offset, 0);
        assert_eq!(detection.description, "fujifilm maker note signature");
    }
}
