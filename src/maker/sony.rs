...
use crate::maker::sony::detection::{detect_sony_maker_note, SONYDetectionResult};

pub mod detection;
use crate::maker::MakerNoteParser;
use crate::tables::sony_tags::get_sony_tag;
...
impl MakerNoteParser for SonyMakerNoteParser {
    fn parse(
        ...
        // Use generated detection logic to identify Sony maker note format
        let detection = match detect_sony_maker_note(data) {
            ...
            None => {
                ...
                SONYDetectionResult {
                    version: None,
                    ifd_offset: 0,
                    description: "Fallback Sony parser".to_string(),
                }
            }
        };
        ...
        parse_sony_ifd_with_tables(ifd_data, byte_order)
    }
...
/// Parse Sony IFD using generated tag tables and print conversion
fn parse_sony_ifd_with_tables(data: &[u8], byte_order: Endian) -> Result<HashMap<u16, ExifValue>> {
    ...
    let parsed_ifd = match IfdParser::parse_ifd(&tiff_data, &header, 8) {
        Ok(parsed) => parsed,
        Err(e) => {
            eprintln!("Warning: Sony IFD parsing failed: {}", e);
            return Ok(HashMap::new());
        }
    };

    // Convert raw IFD entries to Sony tags with print conversion
    let mut result = HashMap::new();

    for (tag_id, raw_value) in parsed_ifd.entries() {
        if let Some(sony_tag) = get_sony_tag(*tag_id) {
            // Apply print conversion to create human-readable value
            let converted_value = apply_print_conv(raw_value, sony_tag.print_conv);
...
/// Sony-specific tag IDs
pub mod tags {
    // Main Sony tags from ExifTool
    pub const SONY_VERSION: u16 = 0x0000;
...
mod tests {
    ...
    fn test_sony_signature_detection() {
        ...
        // Create test data with SONY signature
        let mut test_data = vec![0u8; 20];
        test_data[0..4].copy_from_slice(b"SONY");
