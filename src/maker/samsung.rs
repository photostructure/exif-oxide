//! Samsung maker note parser using table-driven approach
//!
//! Samsung maker notes use a proprietary format that starts with "SAMSUNG"
//! signature followed by IFD-like structures. This format is consistent
//! across Samsung cameras and contains camera-specific settings including
...
use crate::maker::samsung::detection::{detect_samsung_maker_note, SAMSUNGDetectionResult};

pub mod detection;
use crate::maker::MakerNoteParser;
use crate::tables::samsung_tags::get_samsung_tag;
...
impl MakerNoteParser for SamsungMakerNoteParser {
    fn parse(
        ...
        // Use generated detection logic to identify Samsung maker note format
        let detection = match detect_samsung_maker_note(data) {
            Some(detection) => detection,
            None => {
                // Fallback: assume standard IFD at start of data
                SAMSUNGDetectionResult {
                    version: None,
                    ifd_offset: 0,
                    description: "Fallback Samsung parser".to_string(),
                }
            }
        };

        // Samsung maker notes have different formats (EXIFTOOL-SOURCE: MakerNotes.pm:925-954):
        // 1. MakerNoteSamsung1a: STMN format WITHOUT PreviewImage - /^STMN\d{3}.\0{4}/s
        // 2. MakerNoteSamsung1b: STMN format WITH PreviewImage - /^STMN\d{3}/
        // 3. MakerNoteSamsung2: Type2 EXIF format - specific byte patterns

        // Check for STMN format first (EXIFTOOL-SOURCE: MakerNotes.pm:928,935)
        if data.len() >= 4 && data.starts_with(b"STMN") {
            // STMN format - binary format, not implemented yet
            // TODO: Implement ProcessBinaryData framework for STMN format
            eprintln!("Warning: Samsung STMN binary format not yet supported");
            return Ok(HashMap::new());
        }

        // Check for Type2 EXIF format (EXIFTOOL-SOURCE: MakerNotes.pm:944-945)
        // Pattern: /^(\0.\0\x01\0\x07\0{3}\x04|.\0\x01\0\x07\0\x04\0{3})0100/s
        let is_type2_format = data.len() >= 12
            && (
                // First pattern: \0.\0\x01\0\x07\0{3}\x04
                (data[0] == 0x00 && data[2] == 0x00 && data[3] == 0x01 &&
             data[4] == 0x00 && data[5] == 0x07 && data[6] == 0x00 &&
             data[7] == 0x00 && data[8] == 0x00 && data[9] == 0x04) ||
            // Second pattern: .\0\x01\0\x07\0\x04\0{3}
            (data[1] == 0x00 && data[2] == 0x01 && data[3] == 0x00 &&
             data[4] == 0x07 && data[5] == 0x00 && data[6] == 0x04 &&
             data[7] == 0x00 && data[8] == 0x00 && data[9] == 0x00)
            )
            && data.len() >= 14
            && &data[10..12] == b"01"
            && data[12..14] == [0x30, 0x30]; // "0100"

        let ifd_offset = if is_type2_format {
            // Type2 format starts directly with IFD
            0
        } else {
            // Fall back to detection offset
        ...
        parse_samsung_ifd_with_tables(ifd_data, byte_order)
    }
...
/// Parse Samsung IFD using generated tag tables and print conversion
fn parse_samsung_ifd_with_tables(
    data: &[u8],
    byte_order: Endian,
) -> Result<HashMap<u16, ExifValue>> {
    // Create a fake TIFF header for IFD parsing
    // (Samsung maker notes don't have a TIFF header, they start directly with IFD)
    ...
    let parsed_ifd = match IfdParser::parse_ifd(&tiff_data, &header, 8) {
        Ok(parsed) => parsed,
        Err(e) => {
            eprintln!("Warning: Samsung IFD parsing failed: {}", e);
            return Ok(HashMap::new());
        }
    };

    // Convert raw IFD entries to Samsung tags with print conversion
    let mut result = HashMap::new();

    for (tag_id, raw_value) in parsed_ifd.entries() {
        if let Some(samsung_tag) = get_samsung_tag(*tag_id) {
            // Apply print conversion to create human-readable value
            let converted_value = apply_print_conv(raw_value, samsung_tag.print_conv);
...
/// Samsung-specific tag IDs
pub mod tags {
    // Main Samsung tags from ExifTool
    pub const SAMSUNG_VERSION: u16 = 0x0000;
...
mod tests {
    ...
    #[test]
    fn test_samsung_stmn_signature_detection() {
        let parser = SamsungMakerNoteParser;

        // Create test data with STMN signature (EXIFTOOL-SOURCE: MakerNotes.pm:928,935)
        let mut test_data = vec![0u8; 20];
        test_data[0..4].copy_from_slice(b"STMN");
        test_data[4..7].copy_from_slice(b"123"); // version digits
        test_data[7] = 0x01; // separator byte

        // Parser should handle this without error and return empty (not implemented)
        let result = parser.parse(&test_data, Endian::Little, 0).unwrap();
        assert!(result.is_empty()); // STMN format not implemented yet
    }

    #[test]
    fn test_samsung_detection_pattern() {
        // Test the detection function directly with correct "Samsung" signature (7 bytes)
        // EXIFTOOL-SOURCE: auto-generated detection pattern
        let test_data = b"Samsung_test_data";
        let detection = detect_samsung_maker_note(test_data);

        assert!(detection.is_some());
        let detection = detection.unwrap();
        assert_eq!(detection.ifd_offset, 0);
        assert_eq!(detection.description, "Samsung maker note");
    }

    #[test]
    fn test_samsung_type2_pattern_detection() {
        let parser = SamsungMakerNoteParser;

        // Create test data matching Type2 pattern (EXIFTOOL-SOURCE: MakerNotes.pm:945)
        // Pattern: /^(\0.\0\x01\0\x07\0{3}\x04|.\0\x01\0\x07\0\x04\0{3})0100/s
        let mut test_data = vec![0u8; 20];
        // First pattern: \0.\0\x01\0\x07\0{3}\x04
        test_data[0] = 0x00;
        test_data[1] = 0xFF; // any byte
        test_data[2] = 0x00;
        test_data[3] = 0x01;
        test_data[4] = 0x00;
        test_data[5] = 0x07;
        ...
        test_data[8] = 0x00;
        test_data[9] = 0x04;
        test_data[10..12].copy_from_slice(b"01");
        test_data[12..14].copy_from_slice(&[0x30, 0x30]); // "00" part of "0100"

        // Parser should handle this without error
        let _result = parser.parse(&test_data, Endian::Little, 0).unwrap();
        // Result may be empty due to invalid IFD after pattern, but should not panic
    }
}
