...
fn process_olympus_ifd_for_equipment(
    ...
    if data.len() < 14 {
        debug!("Equipment data too small for IFD structure (need at least 14 bytes for IFD header)");
        return Ok(vec![]);
    }
    ...
    for i in 0..entry_count {
        ...
        if let Some(tag_value) = extract_equipment_tag_value(tag_id, format_id, count, value_offset, data, byte_order) {
...
fn extract_equipment_tag_value(
    ...
    match tag_id {
        ...
        0x100 => {
            if format_id == 2 && count <= 32 { // ASCII format
                let value_data = if count <= 4 {
                    // Value fits in the value field itself
                    &data[value_offset.saturating_sub(8)..value_offset.saturating_sub(8) + count as usize]
        ...
        0x101 => {
            if format_id == 2 && count <= 32 { // ASCII format
                let value_data = if value_offset < data.len() && value_offset + count as usize <= data.len() {
                    &data[value_offset..value_offset + count as usize]
                } else {
                    return None;
                };
        ...
        0x201 => {
            if format_id == 1 && count == 6 { // BYTE format, 6 bytes
                ...
                let hex_key = format!("{:x} {:02x} {:02x}", lens_data[0], lens_data[2], lens_data[3]);
                debug!("LensType raw bytes: {:?}, hex key: '{}'", lens_data, hex_key);
...
/// Check for other manufacturer signature patterns (stub)
