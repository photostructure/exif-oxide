    ...
    for (&(tag_id, ref namespace), tag_value) in &exif_reader.extracted_tags {
        ...
        if let Some(source_info) = exif_reader.tag_sources.get(&(tag_id, namespace.clone())) {
            if source_info.namespace == "Canon" && namespace == "Canon" {
                match tag_id {
                    // NOTE: Canon Main tag 0x1 is the CanonCameraSettings
                    // SubDirectory (ExifTool: Canon.pm:1226-1232) and holds NO
                    // FileNumber. A previous version fabricated a FileNumber
                    // from its entry 8 and stored it under tag 0x8, racing with
                    // the real FileNumber (Canon.pm:1261-1267) depending on
