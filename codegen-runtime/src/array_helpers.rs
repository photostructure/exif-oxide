...
pub fn get_array_element(val: &TagValue, index: usize) -> TagValue {
    match val {
        ...
        TagValue::Array(vec) => {
            vec.get(index).cloned().unwrap_or(TagValue::Empty)
        }
        // Typed arrays - convert element to appropriate TagValue
        TagValue::U8Array(vec) => {
            vec.get(index).map(|&v| TagValue::U8(v)).unwrap_or(TagValue::Empty)
        }
        TagValue::U16Array(vec) => {
            vec.get(index).map(|&v| TagValue::U16(v)).unwrap_or(TagValue::Empty)
        }
        TagValue::U32Array(vec) => {
            vec.get(index).map(|&v| TagValue::U32(v)).unwrap_or(TagValue::Empty)
        }
        TagValue::F64Array(vec) => {
            vec.get(index).map(|&v| TagValue::F64(v)).unwrap_or(TagValue::Empty)
        }
        TagValue::RationalArray(vec) => {
            vec.get(index).map(|&(n, d)| TagValue::Rational(n, d)).unwrap_or(TagValue::Empty)
        }
        TagValue::SRationalArray(vec) => {
            vec.get(index).map(|&(n, d)| TagValue::SRational(n, d)).unwrap_or(TagValue::Empty)
        }
...
mod tests {
    ...
    fn test_get_array_element_generic() {
        ...
        assert_eq!(get_array_element(&arr, 0), TagValue::String("first".to_string()));
