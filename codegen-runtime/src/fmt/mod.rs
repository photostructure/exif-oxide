...
pub fn safe_binary_operation(left: &TagValue, operator: &str, right: &TagValue) -> TagValue {
    match operator {
        ...
        "eq" | "==" => TagValue::U8(if left.to_string() == right.to_string() {
            1
        } else {
            0
        }),
        "ne" | "!=" => TagValue::U8(if left.to_string() != right.to_string() {
            1
        } else {
            0
        }),
...
mod basic_tests {
    ...
    fn test_safe_binary_operation() {
        ...
        assert_eq!(
            safe_binary_operation(
                &TagValue::String("left".to_string()),
                "???",
                &TagValue::String("right".to_string())
            ),
