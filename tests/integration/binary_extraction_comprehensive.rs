...
use exif_oxide::compat::{run_exiftool, run_exif_oxide};
...
impl ComprehensiveResults {
    ...
    fn add_failure(&mut self, result: BinaryExtractionResult) {
    ...
    fn update_statistics(&mut self, result: &BinaryExtractionResult, success: bool) {
        ...
        let entry = self.results_by_manufacturer.entry(manufacturer).or_insert((0, 0));
        entry.1 += 1; // total
        if success {
            entry.0 += 1; // success
        }
    ...
    fn print_comprehensive_report(&self) {
        ...
        println!("  Total extractions attempted: {}", self.total_extractions_attempted);
        println!("  Successful extractions: {}", self.successful_extractions.len());
        ...
        if self.total_extractions_attempted > 0 {
            let success_rate = (self.successful_extractions.len() as f64 / self.total_extractions_attempted as f64) * 100.0;
...
fn test_comprehensive_binary_extraction() {
    ...
    println!("📁 Found {} supported image files to test", test_files.len());
    ...
    if results.total_extractions_attempted > 0 {
        let success_rate = results.successful_extractions.len() as f64 / results.total_extractions_attempted as f64;
        ...
        if success_rate < 0.5 {
            ...
            println!("\n🎉 Excellent binary extraction compatibility: {:.1}% success rate!", success_rate * 100.0);
        }
...
fn test_binary_extraction_for_file(file_path: &Path) -> Result<Vec<BinaryExtractionResult>, Box<dyn std::error::Error>> {
...
fn test_single_tag_extraction(file_path: &Path, tag_name: &str) -> Result<BinaryExtractionResult, Box<dyn std::error::Error>> {
    ...
    if !exiftool_output.status.success() {
        return Err(format!("ExifTool extraction failed: {}", 
            String::from_utf8_lossy(&exiftool_output.stderr)).into());
    }
    ...
    let exif_oxide_output = Command::new("cargo")
        .args(["run", "--bin", "exif-oxide", "--", "-b", &format!("-{}", tag_name), &file_path_str])
        .output()?;
    
    if !exif_oxide_output.status.success() {
        return Err(format!("exif-oxide extraction failed: {}", 
            String::from_utf8_lossy(&exif_oxide_output.stderr)).into());
    }
    ...
    if matches {
        println!("    ✅ {} extraction: SUCCESS ({} bytes)", tag_name, exiftool_size);
    } else {
        println!("    ❌ {} extraction: FAILED ({} vs {} bytes)", tag_name, exiftool_size, exif_oxide_size);
    }
...
fn compute_sha256(data: &[u8]) -> String {
    ...
    let output = child.wait_with_output().expect("Failed to read sha256sum output");
    let hash_line = String::from_utf8_lossy(&output.stdout);
    
    // Extract just the hash part (before the first space)
    hash_line.split_whitespace().next().unwrap_or("").to_string()
}
