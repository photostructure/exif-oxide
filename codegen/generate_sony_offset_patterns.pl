...
unless (-f $json_file) {
    die "Error: $json_file not found. Run the offset pattern extractor first.\n";
}
...
sub generate_rust_code {
    ...
    if (@{$patterns->{model_conditions}}) {
        ...
        $code .= "pub static SONY_MODEL_CONDITIONS: LazyLock<Vec<(&str, &str, &str)>> = LazyLock::new(|| {\n";
        $code .= "    vec![\n";
    ...
    if (@{$patterns->{offset_calculations}}) {
        ...
        $code .= "pub static OFFSET_CALCULATION_TYPES: LazyLock<Vec<&str>> = LazyLock::new(|| {\n";
        $code .= "    vec![\n";
    ...
    $code .= "pub static OFFSET_EXAMPLES: LazyLock<HashMap<&str, &str>> = LazyLock::new(|| {\n";
