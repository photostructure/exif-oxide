...
pub struct TagKitStrategy {
    ...
    /// Track imports needed for generated functions (per-file state)
...
impl TagKitStrategy {
    ...
    fn generate_tag_table_code(
        &mut self,
        symbol: &ProcessedTagTable,
        ...
        // Now we can start building the output with the correct imports
            ...
            if let Some(value_conv_str) = value_conv_value.as_str() {
                ...
                match classify_valueconv_expression(value_conv_str, module) {
                    ...
                    | ValueConvType::PpiGeneratedComposite(_) => {
                        // PPI will generate this when we have the AST
                        // For now, fall back to Expression variant
                        // This should rarely happen since PPI AST should be available
                        return Ok(format!(
...
impl ExtractionStrategy for TagKitStrategy {
    ...
    fn extract(
        ...
        // Store the symbol data directly for processing during finish_extraction
    ...
    fn finish_extraction(&mut self, context: &mut ExtractionContext) -> Result<Vec<GeneratedFile>> {
        ...
        for symbol in &symbols {
            match self.generate_tag_table_code(symbol, context) {
                Ok(code) => {
