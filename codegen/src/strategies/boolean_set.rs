...
impl BooleanSetStrategy {
    ...
    fn generate_boolean_set_code(boolean_set: &BooleanSet) -> String {
        ...
        let const_name = boolean_set.name.chars().fold(String::new(), |mut acc, c| {
        ...
        code.push_str(&format!(
            "/// Boolean set for fast membership testing: {}\n",
            boolean_set.name
        ));
        code.push_str(&format!(
        ...
        let fn_name = boolean_set.name.chars().fold(String::new(), |mut acc, c| {
            if c.is_uppercase() && !acc.is_empty() {
                acc.push('_');
            }
...
impl ExtractionStrategy for BooleanSetStrategy {
    ...
    fn extract(
        ...
        debug!(
            "Extracted boolean set: {}::{} ({} keys)",
            symbol_data.module,
            symbol_data.name,
            boolean_set.keys.len()
        );
