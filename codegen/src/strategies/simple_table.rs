...
impl SimpleTableStrategy {
    ...
    pub fn new() -> Self {
        Self {
            tables: HashMap::new(),
            output_dir: String::new(), // Will be set from context during extraction
        }
    }
    ...
    fn generate_table_code(&self, table: &SimpleTable) -> String {
        ...
        code.push_str("//!\n");
        code.push_str("//! This file is auto-generated by codegen/src/strategies/simple_table.rs. Do not edit manually.\n\n");
        ...
        code.push_str(&format!(
            "static {const_name}_DATA: &[({key_type}, &'static str)] = &[\n"
        ));
        ...
        for (key, value) in entries {
            ...
            code.push_str(&format!("    ({formatted_key}, \"{escaped_value}\"),\n"));
        }
        ...
        code.push_str(&format!(
            "pub static {const_name}: LazyLock<HashMap<{key_type}, &'static str>> = LazyLock::new(|| {{\n"
        ));
        code.push_str(&format!(
            "    {const_name}_DATA.iter().copied().collect()\n"
        ));
        code.push_str("});\n\n");
    ...
    fn infer_key_type(&self, data: &HashMap<String, String>) -> &'static str {
        ...
        if all_numeric {
            ...
            let values: Vec<i64> = data.keys().filter_map(|k| k.parse::<i64>().ok()).collect();

            let max_value = values.iter().max().unwrap_or(&0);
            let min_value = values.iter().min().unwrap_or(&0);

            // If any value is negative, use signed types
            if *min_value < 0 {
...
impl ExtractionStrategy for SimpleTableStrategy {
    ...
    fn extract(&mut self, symbol: &FieldSymbol, context: &mut ExtractionContext) -> Result<()> {
        // Store output_dir from context on first use
...
// SimpleTableStrategy no longer implements Default since it requires output_dir
