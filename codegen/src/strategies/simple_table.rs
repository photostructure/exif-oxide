...
impl ExtractionStrategy for SimpleTableStrategy {
    ...
    fn extract(&mut self, symbol: &FieldSymbol, _context: &mut ExtractionContext) -> Result<()> {
        ...
        if let JsonValue::Object(data_map) = &symbol.data {
            ...
            if !table_data.is_empty() {
                ...
                debug!(
                    "📊 Extracted simple table: {} ({} entries)",
                    table.name,
                    table.data.len()
                );
    ...
    fn finish_extraction(&mut self) -> Result<Vec<GeneratedFile>> {
        ...
        info!(
            "🔧 Generating simple table code for {} modules",
            self.tables.len()
        );
        ...
        info!(
            "✅ SimpleTableStrategy generated {} files",
            generated_files.len()
        );
        Ok(generated_files)
    }
...
mod tests {
    ...
    fn test_naming_conventions() {
        ...
        assert_eq!(
            output_locations::to_snake_case("WhiteBalance"),
            "white_balance"
        );
        assert_eq!(
            output_locations::to_snake_case("canonWhiteBalance"),
            "canon_white_balance"
        );
