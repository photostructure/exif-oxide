...
impl MagicNumberStrategy {
    ...
    fn generate_magic_number_code(&self, data: &MagicNumberData) -> String {
        ...
        // Some ExifTool regex patterns have Perl-specific escaping that doesn't translate
        // perfectly to Rust regex. The code handles this gracefully with if let Ok(regex).
        code.push_str("#![allow(clippy::invalid_regex)]\n\n");
        ...
        for (file_type, pattern_str) in &regex_patterns {
            // Convert Perl regex escapes to Rust regex escapes
            // \0 in Perl means null byte, but in Rust regex it's a backreference
            // We need to use \x00 instead
            let rust_pattern_str = pattern_str.replace("\\0", "\\x00");
            ...
            let raw_pattern = if rust_pattern_str.contains('"') {
                // If the pattern contains quotes, we need to escape them properly
                format!("r#\"{}\"#", rust_pattern_str.replace('\\', "\\\\"))
            } else {
                // Simple raw string works fine
                format!("r\"{rust_pattern_str}\"")
            };
...
impl ExtractionStrategy for MagicNumberStrategy {
    ...
    fn extract(&mut self, symbol: &FieldSymbol, context: &mut ExtractionContext) -> Result<()> {
        info!("🔧 Extracting magicNumber raw byte patterns");
        ...
        if let JsonValue::Object(map) = &symbol.data {
            ...
            info!(
                "    ✓ Parsed {} magic number patterns ({} literal, {} regex)",
                data.patterns.len(),
    ...
    fn finish_extraction(
        ...
        for data in self.magic_data.values() {
            ...
            info!(
                "📁 Generated magic_numbers.rs with {} magic number patterns",
                data.patterns.len()
            );
        }
