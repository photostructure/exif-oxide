...
impl BinaryDataStrategy {
    ...
    fn extract_binary_data(&self, extraction: &BinaryDataExtraction) -> Result<String> {
        ...
        let output = Command::new("perl")
            ...
            .with_context(|| {
                format!(
                    "Failed to run process_binary_data.pl for {}::{}",
                    extraction.module_name, extraction.table_name
                )
            })?;
    ...
    fn generate_binary_data_code(
        &self,
        json_data: &str,
        extraction: &BinaryDataExtraction,
    ) -> Result<String> {
        let binary_data: JsonValue = serde_json::from_str(json_data)?;

        let entries = binary_data["entries"]
            .as_array()
        ...
        code.push_str(&format!(
            "/// Binary data table: {} (format: {}, first_entry: {})\n",
        ...
        for entry in entries {
            ...
            code.push_str(&format!(
                "    entries.insert({}, BinaryDataEntry {{\n",
                offset
            ));
        ...
        code.push_str(&format!(
            "/// Look up binary data entry for {} table\n",
            extraction.table_name
        ));
        code.push_str(&format!(
            "pub fn lookup_{}_entry(offset: u16) -> Option<&'static BinaryDataEntry> {{\n",
            extraction.table_name.to_lowercase()
        ));
        code.push_str(&format!(
            "    {}_BINARY_DATA.get(&offset)\n",
            extraction.table_name.to_uppercase()
        ));
        code.push_str("}\n");
        ...
        code.push_str("    \n");
        code.push_str(&format!(
            "    for (offset, entry) in {}_BINARY_DATA.iter() {{\n",
            extraction.table_name.to_uppercase()
        ));
