...
pub fn normalize_expression(expr: &str) -> String {
    ...
    if let Ok(cache) = NORMALIZATION_CACHE.lock() {
        if let Some(normalized) = cache.get(expr) {
            let cache_time = normalize_start.elapsed();
            if cache_time.as_millis() > 0 {
                trace!(
                    "✅ Cached normalization for '{}' in {:.2}ms",
    ...
    trace!(
        "🔄 Starting Perl normalization for: '{}'",
        expr.chars().take(50).collect::<String>()
    );
    ...
    let normalized = match normalize_with_perl(expr) {
        Ok(result) => result,
        Err(e) => {
            warn!("⚠️  Failed to normalize expression '{}': {}", expr, e);
            warn!("Using original expression");
            expr.to_string()
        }
    };
    let perl_time = perl_start.elapsed();

    if perl_time.as_millis() > 50 {
        warn!(
            "🐌 SLOW Perl normalization for '{}' took {:.2}ms",
            expr.chars().take(50).collect::<String>(),
            perl_time.as_millis()
        );
    } else if perl_time.as_millis() > 10 {
        debug!(
            "⚡ Perl normalization for '{}' took {:.2}ms",
            expr.chars().take(50).collect::<String>(),
            perl_time.as_millis()
        );
    }
    ...
    let total_time = normalize_start.elapsed();
    if total_time.as_millis() > 50 {
        warn!(
            "🔥 TOTAL normalization time for '{}': {:.2}ms",
            expr.chars().take(50).collect::<String>(),
...
pub fn batch_normalize_expressions(
    ...
    if uncached.is_empty() {
        ...
        return Ok(expressions
            ...
            .filter_map(|expr| {
                cache
                    .get(expr)
                    .map(|normalized| (expr.clone(), normalized.clone()))
            })
            .collect());
    }
    ...
    {
        let mut cache = NORMALIZATION_CACHE
            .lock()
            .map_err(|_| "Cache lock failed")?;
...
fn find_normalize_script() -> Option<std::path::PathBuf> {
    ...
    for _ in 0..5 {
        ...
        let script_path = current.join("scripts").join("normalize_expression.pl");
        if script_path.exists() {
            return Some(script_path);
        }

        // Check if we're in the project root
        let codegen_script = current
            .join("codegen")
            .join("scripts")
            .join("normalize_expression.pl");
