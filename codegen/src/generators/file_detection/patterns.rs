...
/// Generate regex patterns for file type detection from ExifTool regex patterns
pub fn generate_regex_patterns(json_dir: &Path, output_dir: &str) -> Result<()> {
    // Look for exiftool__regex_patterns.json in the file_types subdirectory
    let regex_patterns_path = json_dir.join("file_types").join("exiftool__regex_patterns.json");

    if !regex_patterns_path.exists() {
        warn!("    ⚠️  exiftool__regex_patterns.json not found, skipping regex patterns");
        return Ok(());
    }
    ...
    let json_data = match String::from_utf8(json_bytes.clone()) {
        ...
        Err(_) => {
            ...
            debug!("    ⚠️  exiftool__regex_patterns.json contains non-UTF-8 bytes, cleaning...");
    ...
    // Generate regex_patterns.rs directly in output_dir
    generate_regex_patterns_from_data(&data, Path::new(output_dir))?;

    debug!(
        "    ✓ Generated regex patterns with {} patterns",
        data.magic_patterns.len()
    );
...
fn generate_regex_patterns_from_data(
    ...
    // Note: Regex patterns are generated by the lookup_tables system in ExifTool_pm/regex_patterns.rs
    // This function now only processes the extracted data but doesn't generate a separate file
    // to avoid conflicts with the existing regex pattern generation system
