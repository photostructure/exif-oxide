...
fn generate_table_code(hash_name: &str, table_data: &ExtractedTable) -> Result<String> {
    ...
    match extraction_type {
        ...
        "file_type_lookup" => {
            // TODO: Implement file_type_lookup generation
            anyhow::bail!("file_type_lookup extraction not yet implemented")
        },
...
fn escape_regex_for_rust(pattern: &str) -> String {
    // For most patterns, raw strings work well
    // But if pattern contains quotes, we need to handle them
    if pattern.contains('"') {
        // If pattern contains quotes, use regular string with escaping
        pattern.replace('\\', "\\\\")
            .replace('"', "\\\"")
            .replace('\n', "\\n")
            .replace('\r', "\\r")
