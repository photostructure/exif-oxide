...
print STDERR
"Extracting conditional tag definitions from $table_name table in $module_display_name...\n";
...
my $conditional_data =
  extract_conditional_definitions( $table_ref, $manufacturer, $table_name );

print STDERR "  Found "
  . scalar( @{ $conditional_data->{conditional_arrays} } )
  . " conditional tag arrays\n";
print STDERR "  Found "
  . scalar( @{ $conditional_data->{count_conditions} } )
  . " count-based conditions\n";
print STDERR "  Found "
  . scalar( @{ $conditional_data->{binary_patterns} } )
  . " binary pattern conditions\n";
...
sub extract_conditional_definitions {
    ...
    foreach my $key ( sort keys %$table_ref ) {
        ...
        if ( ref $tag_info eq 'ARRAY' ) {
            my $conditional_array =
              extract_conditional_array( $key, $tag_info );
            if ( $conditional_array && @{ $conditional_array->{conditions} } ) {
                ...
                {
                    categorize_condition( $condition, \@count_conditions,
                        \@binary_patterns, \@format_conditions,
                        \@cross_tag_dependencies );
                }
            }
        }
        ...
        # Handle single conditional tag
            ...
            if ($condition_entry) {
                categorize_condition(
                    $condition_entry,  \@count_conditions,
                    \@binary_patterns, \@format_conditions,
                    \@cross_tag_dependencies
                );
            }
...
sub extract_conditional_array {
    ...
    foreach my $tag_def (@$array_ref) {
        ...
        $condition_entry->{description} = $tag_def->{Description}
        ...
        $condition_entry->{subdirectory} = JSON::true
          if $tag_def->{SubDirectory};
        $condition_entry->{data_member} = $tag_def->{DataMember}
          if $tag_def->{DataMember};
        $condition_entry->{raw_conv} = $tag_def->{RawConv}
          if $tag_def->{RawConv};
        $condition_entry->{value_conv} = $tag_def->{ValueConv}
          if $tag_def->{ValueConv};
        $condition_entry->{print_conv} = $tag_def->{PrintConv}
          if $tag_def->{PrintConv};
...
sub has_complex_condition {
    ...
    return 1
      if $condition =~ /DataMember|RawConv|ValueConv/;  # Cross-tag dependencies
...
sub categorize_condition {
    my (
        $condition_entry,   $count_conditions, $binary_patterns,
    ...
    if (   $condition_entry->{data_member}
        || $condition_entry->{raw_conv}
        || $condition =~ /\$\$self\{[^}]+\}/ )
    {
