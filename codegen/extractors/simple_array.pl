...
for my $array_expr (@array_expressions) {
    ...
    unless (ref($array_ref) eq 'ARRAY') {
        warn "Warning: $array_expr is not an array reference in $module_display_name\n";
        next;
    }
    ...
    for my $i (0 .. $#{$array_ref}) {
        ...
        if (ref($element)) {
            warn "Warning: Non-primitive element at index $i in $array_expr (skipping)\n";
            next;
        }
    ...
    if (@elements) {
        ...
        print STDERR "Extracted " . scalar(@elements) . " elements from $array_expr\n";
    } else {
...
if (@all_extractions) {
    for my $extraction (@all_extractions) {
        ...
        $filename = lc($filename) . ".json";   # Ensure lowercase and add extension
