...
my $exiftool_lib =
  "$FindBin::Bin/../../third-party/exiftool/lib/Image/ExifTool";
...
sub find_exiftool_modules {
    ...
    find(
        sub {
            ...
            return
              if
/^(BuildTagLookup|WriteCanonRaw|WritePostScript|WritePDF|Validate)\.pm$/;
...
sub process_table {
    ...
    foreach my $tag_id ( keys %$table_ref ) {
        ...
        next
          if $tag_id =~
/^(PROCESS_PROC|WRITE_PROC|CHECK_PROC|GROUPS|NOTES|FORMAT|FIRST_ENTRY|VARS|DATAMEMBER|NAMESPACE|PREFERRED|TABLE_NAME|SHORT_NAME|DID_TAG_ID|AVOID|PRIORITY)$/;

        my $tag_def = $table_ref->{$tag_id};

        # Extract SubDirectory info
        my @subdirs =
          extract_subdirectory_info( $tag_id, $tag_def, $module_name,
            $table_name );
...
sub get_module_tables {
    ...
    foreach my $symbol ( keys %$symtab ) {
        next
          unless $symbol =~ /^[A-Z]/;  # Tag tables usually start with uppercase
        ...
        foreach my $key ( keys %$hash_ref ) {
            if ( $key =~
                /^(0x[0-9a-fA-F]+|\d+|GROUPS|PROCESS_PROC|NOTES|FORMAT)$/ )
            {
    ...
    if ( -d "$generated_src/${module}_pm" ) {
        ...
        foreach my $gen_file (@gen_files) {
            ...
            if ( $subdir_info->{tag_id} =~ /^\d+$/ ) {
                ...
                if (   $content =~ /$tag_hex/i
                    || $content =~ /$subdir_info->{tag_name}/i )
                {
...
foreach my $module_file (@modules) {
    ...
    foreach my $table_name ( keys %tables ) {
        ...
        foreach my $subdir (@subdirs) {
            ...
            $module_stats{$module_short}{implemented}++
              if $impl_status->{is_implemented};
            ...
            push @{ $subdirectory_data{$module_short} },
              {
                ...
                implementation_status => $impl_status->{is_implemented}
                ? 'implemented'
                : 'missing',
...
if ($output_json) {
    my $coverage_pct =
      $total_subdirs > 0
      ? sprintf( "%.2f", ( $implemented_subdirs / $total_subdirs ) * 100 )
      : 0;
    ...
    foreach my $module (
        sort { $module_stats{$b}{total} <=> $module_stats{$a}{total} }
        keys %module_stats
      )
    {
    ...
    foreach my $module (
        sort { $module_stats{$b}{total} <=> $module_stats{$a}{total} }
        keys %subdirectory_data
      )
    {
        my @missing = grep { $_->{implementation_status} eq 'missing' }
          @{ $subdirectory_data{$module} };
        next unless @missing;
