...
my %modules;           # module_name -> { tables => {}, subdirs => {}, conditions => {}, stats => {} }
my %implementation_status;  # module_name -> { implemented => bool, coverage => float }
my %condition_complexity;  # condition_string -> complexity_score
my %tag_frequency;     # tag_name -> frequency_score (from TagMetadata.json if available)
...
sub analyze_module {
    ...
    while ($content =~ /^\s*%(\w+)\s*=\s*\(/gm) {
        my $table_name = $1;
        $module_data{tables}{$table_name} = analyze_table($content, $table_name);
    ...
    if ($table_content) {
        ...
        while ($table_content =~ /SubDirectory\s*=>\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/gs) {
            ...
            if ($subdir_info{condition}) {
                ...
                $table_data{complexity_score} += calculate_condition_complexity($subdir_info{condition});
            }
        }
    }
...
sub generate_summary_stats {
    ...
    my $implemented_modules = scalar grep { $implementation_status{$_}{implemented} } keys %modules;
    my $implementation_percent = $total_modules > 0 ? sprintf("%.1f", 100 * $implemented_modules / $total_modules) : 0;
    ...
    print "- **Implemented**: $implemented_modules ($implementation_percent%)\n";
    print "- **Total Subdirectories**: $total_subdirs\n";
    print "- **Simple Conditions**: $simple_subdirs (" . sprintf("%.1f", 100 * $simple_subdirs / ($total_subdirs || 1)) . "%)\n";
    print "- **Complex Conditions**: $complex_subdirs (" . sprintf("%.1f", 100 * $complex_subdirs / ($total_subdirs || 1)) . "%)\n";
    print "\n";
}

sub generate_implementation_matrix {
    print "## Implementation Status Matrix\n\n";
    print "| Module | Status | Tables | Subdirs | Direct | Simple | Complex | Priority |\n";
    print "|--------|--------|---------|---------|--------|--------|---------|----------|\n";
    ...
    foreach my $module_name (@sorted_modules) {
        my $module = $modules{$module_name};
        my $status = $implementation_status{$module_name}{implemented} ? "✅" : "❌";
        ...
        print "| $module_name | $status | $total_tables | $subdir_tables | $direct_subdirs | $simple_count | $complex_count | $priority_label |\n";
    }
    print "\n";
}
...
sub generate_priority_recommendations {
    ...
    foreach my $module_name (@high_priority) {
        ...
        print "- **$module_name**: $simple_count simple, $complex_count complex conditions. $reason\n";
    }
...
sub get_modules_by_priority {
    ...
    return sort {
        calculate_module_priority($b) <=> calculate_module_priority($a)
    } @modules;
}
...
sub generate_json_output {
    my %output = (
        summary => {
            ...
            implemented_modules => scalar grep { $implementation_status{$_}{implemented} } keys %modules,
            generated_at => time(),
        },
        modules => \%modules,
