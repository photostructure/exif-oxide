...
print STDERR
"Extracting tag table structure from $module_display_name table $table_name...\n";
        ...
        if ( ref $tag_info eq 'ARRAY' ) {
            ...
            for my $variant (@$tag_info) {
                if ( ref $variant eq 'HASH' && exists $variant->{Name} ) {
                    my $variant_data =
                      extract_single_tag( $variant, $manufacturer );
...
sub extract_single_tag {
    ...
    # Format information
    $data->{format} = $tag_info->{Format} || $tag_info->{Writable}
      if $tag_info->{Format} || $tag_info->{Writable};
