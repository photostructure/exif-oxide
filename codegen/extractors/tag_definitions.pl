...
if ( @ARGV < 2 ) {
    die
"Usage: $0 <module_path> <table_name> [--frequency-threshold <value>] [--include-mainstream] [--groups <group1,group2>]\n"
...
print STDERR
  "Extracting tag definitions from $module_display_name table $table_name...\n";
print STDERR "  Frequency threshold: $frequency_threshold\n"
  if $frequency_threshold > 0;
print STDERR "  Include mainstream: "
  . ( $include_mainstream ? "yes" : "no" ) . "\n";
print STDERR "  Groups filter: "
  . ( @groups_filter ? join( ", ", @groups_filter ) : "none" ) . "\n";
...
my @tags = extract_tags_from_table( $table_ref, $metadata, \%print_conv_refs,
    \%value_conv_refs );
...
sub extract_tags_from_table {
    ...
    foreach my $tag_id ( sort keys %$table_ref ) {
        ...
        if ( $tag_info->{PrintConv} ) {
            my $ref = generate_conv_ref( $tag_name, 'print_conv',
                $tag_info->{PrintConv} );
        ...
        if ( $tag_info->{ValueConv} ) {
            my $ref = generate_conv_ref( $tag_name, 'value_conv',
                $tag_info->{ValueConv} );
...
sub passes_filters {
    ...
    return 1
      if $frequency_threshold == 0 && !$include_mainstream && !@groups_filter;
