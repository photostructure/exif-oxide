...
print STDERR "Extracting runtime table $table_name from $module_display_name...\n";
...
print STDERR "  Model conditions: " . ($runtime_table_data->{metadata}->{has_model_conditions} ? "Yes" : "No") . "\n";
print STDERR "  Complex PrintConv: " . ($runtime_table_data->{metadata}->{has_complex_printconv} ? "Yes" : "No") . "\n";
...
sub extract_table_structure {
    ...
    $structure->{first_entry} = $table_ref->{FIRST_ENTRY} if defined $table_ref->{FIRST_ENTRY};
...
sub extract_tag_definition {
    ...
    if (ref $tag_info eq 'HASH') {
        ...
        if ($tag_info->{Condition}) {
            $tag_def->{condition} = extract_condition_spec($tag_info->{Condition});
        }
        
        # Extract PrintConv specification
        if ($tag_info->{PrintConv}) {
            $tag_def->{print_conv} = extract_print_conv_spec($tag_info->{PrintConv});
        }
        
        # Extract ValueConv specification
        if ($tag_info->{ValueConv}) {
            $tag_def->{value_conv} = extract_value_conv_spec($tag_info->{ValueConv});
        }
...
sub analyze_runtime_requirements {
    ...
    foreach my $tag_def (values %$tag_definitions) {
        if ($tag_def->{condition} && $tag_def->{condition}->{condition_type} =~ /model/) {
