...
sub extract_table {
    ...
    foreach my $tag ( sort keys %$table_ref ) {
        next
          if $tag =~
/^(GROUPS|TABLE_NAME|SHORT_NAME|PROCESS_PROC|WRITE_PROC|CHECK_PROC|VARS|DATAMEMBER|IS_OFFSET|IS_SUBDIR|FIRST_ENTRY|TAG_PREFIX|PRINT_CONV)$/;
        ...
        if ( exists $tag_info->{SubDirectory} ) {
            ...
            if ( ref $subdir eq 'HASH' ) {
                ...
                if ( exists $subdir->{ProcessProc} ) {
                    ...
                    $tag_data{SubDirectory}{ProcessProc} =
                      ref( $subdir->{ProcessProc} ) eq 'CODE'
                      ? 'CODE'
                      : $subdir->{ProcessProc};
                }
    ...
    if ( exists $table_ref->{PROCESS_PROC} ) {
        $table_data{process_proc} =
          ref( $table_ref->{PROCESS_PROC} )
          ? 'CODE'
          : $table_ref->{PROCESS_PROC};
    }
...
sub detect_table_type {
    ...
    foreach my $tag ( keys %$table_ref ) {
        ...
        my $tag_info = $table_ref->{$tag};
        $subdir_count++
          if ref $tag_info eq 'HASH' && exists $tag_info->{SubDirectory};
    }

    return 'subdirectory_table'
      if $total_tags > 0 && $subdir_count / $total_tags > 0.5;
