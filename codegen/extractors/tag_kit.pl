...
for my $tag_id ( sort keys %$table_ref ) {
    ...
    if ( ref $tag_value eq 'ARRAY' ) {
        ...
        for my $i ( 0 .. $#$tag_value ) {
            ...
            process_tag_info( $tag_info, $variant_id, $tag_id, \@tag_kits,
                \$extracted_count, \$skipped_complex );
        }
        next;
    }
    ...
    process_tag_info( $tag_value, $tag_id, $tag_id, \@tag_kits,
        \$extracted_count, \$skipped_complex );
}
...
print STDERR
"Skipped $skipped_complex complex PrintConvs requiring manual implementation\n"
  if $skipped_complex;
...
sub extract_print_conv {
    ...
    if ( ref $print_conv eq 'HASH' ) {
        ...
        elsif (exists $print_conv->{Mask}
            || exists $print_conv->{PrintHex}
            || exists $print_conv->{PrintString} )
        {
            my @types;
    ...
    # Code reference
    if ( ref $print_conv eq 'CODE'
        || ( !ref $print_conv && $print_conv =~ /^\\&/ ) )
    {
...
sub is_simple_expression {
    ...
    return 1
      if $expr =~
      /^Image::ExifTool::\w+::\w+\(\$val\)$/;          # ExifTool function calls
...
sub process_tag_info {
    my ( $tag_info, $variant_id, $original_tag_id, $tag_kits_ref,
        $extracted_count_ref, $skipped_complex_ref )
      = @_;
    ...
    # Extract PrintConv
    my ( $print_conv_type, $print_conv_data ) =
      extract_print_conv( $tag_info->{PrintConv} );
...
sub extract_subdirectory {
    ...
    my $info = { tag_table => $subdir->{TagTable} || 'Unknown', };
    ...
    if ( defined $subdir->{TagTable} && $subdir->{TagTable} ne 'Unknown' ) {
        ...
        if ($table_info) {
            ...
            $info->{is_binary_data} =
              $table_info->{is_binary_data} || JSON::false;
        }
    }
