...
sub scan_module_for_refs {
      ...
      )
    {
        ...
        if ( $subdir_block =~
            /TagTable\s*=>\s*['"]Image::ExifTool::(\w+)::(\w+)['"]/ )
        {
            my $target_module = $1;
            my $table_name    = $2;
            ...
            if ( $target_module ne $source_module ) {
                ...
                unless (
                    exists $table_definitions{$target_module}{$table_name} )
                {
                    $missing_tables{
                        "Image::ExifTool::${target_module}::${table_name}"} = 1;
                }
            }
        }
    }
