...
if ($target_module) {
    ...
    @target_modules = get_priority_modules($priority_filter, \%coverage_data, \%required_tags);
}

print STDERR "Target modules for config generation: " . join(", ", @target_modules) . "\n";
...
foreach my $module_name (@target_modules) {
    ...
    if (-f $config_file && !$force) {
        print STDERR "Config already exists for $module_name, skipping (use --force to overwrite)\n";
        next;
    }
...
sub load_coverage_data {
    ...
    my $dashboard_output = `perl ./coverage_dashboard.pl --output=json 2>/dev/null`;
    ...
    if ($dashboard_output) {
        ...
        if ($@) {
            print STDERR "Warning: Could not parse coverage dashboard output: $@\n";
        }
    }
...
sub get_priority_modules {
    ...
    foreach my $module_name (keys %$coverage_ref) {
        ...
        my $module_priority = calculate_module_priority($module_name, $module, $required_tags_ref);
    ...
    @modules = sort {
        my $a_priority = calculate_module_priority($a, $coverage_ref->{$a}, $required_tags_ref);
        my $b_priority = calculate_module_priority($b, $coverage_ref->{$b}, $required_tags_ref);
...
sub calculate_module_priority {
    ...
    if ($required_tags_ref && $required_tags_ref->{$module_name}) {
        ...
        print STDERR "Module $module_name has $required_tag_count required tags (+". ($required_tag_count * 50) ." points)\n";
    }
...
sub get_priority_label {
    ...
    return "⚡ Medium" if $priority >= 15;  # Modules with good subdirectory coverage
    return "💤 Low";
}
...
sub analyze_module_for_config {
    ...
    my $table_content = extract_table_content($content, $analysis{main_table});
    if ($table_content) {
        $analysis{tables}{$analysis{main_table}} = analyze_table_content($table_content);
        ...
        $analysis{has_binary_data} = ($table_content =~ /PROCESS_PROC.*ProcessBinaryData/i);
    }
...
sub extract_subdirectory_info {
    ...
    while ($table_content =~ /SubDirectory\s*=>\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/gs) {
        my $subdir_def = $1;
        ...
        tables => [
            {
                ...
                description => "Main $module_name tag table with " . scalar @{$analysis->{subdirectories}} . " subdirectories"
            }
        ],
        _auto_generated => {
            ...
            config => {
                include_subdirectories => (scalar @{$analysis->{subdirectories}} > 0) ? "true" : "false",
                include_printconv => $analysis->{tables}{$analysis->{main_table}}{has_printconv} ? "true" : "false",
                subdirectory_strategy => determine_subdirectory_strategy($analysis),
            ...
            notes => [
                ...
                "Subdirectories found: " . scalar @{$analysis->{subdirectories}},
...
sub determine_condition_handling {
    ...
    my $complex_conditions = grep { $_->{complexity} eq 'complex' } @{$analysis->{subdirectories}};
