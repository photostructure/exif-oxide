...
$function_pattern ||=
  'ProcessSony|Process.*Binary|SetARW';    # Default patterns for Sony
...
my $offset_patterns =
  extract_offset_patterns_from_source( $module_path, $manufacturer );

print STDERR "  Found "
  . scalar( @{ $offset_patterns->{model_conditions} } )
  . " model-specific conditions\n";
print STDERR "  Found "
  . scalar( @{ $offset_patterns->{offset_calculations} } )
  . " offset calculation patterns\n";
print STDERR "  Found "
  . scalar( @{ $offset_patterns->{base_types} } )
  . " base offset types\n";
...
sub extract_offset_patterns_from_source {
    ...
    while (
        $source_code =~ /(Set(?:32u|16u)\s*\([^)]+,\s*[^)]+,\s*([^)]+)\))/g )
    {
        my $full_expr   = $1;
        my $offset_expr = $2;
        $offset_expr =~ s/^\s+|\s+$//g;
...
sub parse_offset_calculation {
    ...
    if ( $calculation =~
/Get(32u|16u|8u)\s*\(\s*\$dataPt\s*,\s*\$entry\s*\+\s*(\d+)\s*\)\s*\+\s*(\$\w+)/
      )
    {
    ...
    if ( $calculation =~
        /Get(32u|16u|8u)\s*\(\s*\$dataPt\s*,\s*\$valuePtr\s*\)\s*\+\s*(\$\w+)/ )
    {
    ...
    if ( $calculation =~
/Get(32u|16u|8u)\s*\(\s*\$dataPt\s*,\s*(\d+)\s*\)\s*\+\s*(0x[0-9a-fA-F]+|\d+)/
      )
    {
    ...
    if ( $calculation =~
        /Get(32u|16u|8u)\s*\(\s*\$dataPt\s*,\s*\$entry\s*\+\s*(\d+)\s*\)/ )
    {
