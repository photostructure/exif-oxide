...
print STDERR "Extracting model detection patterns from $table_name table in $module_display_name...\n";
...
my $patterns_data = extract_model_detection_patterns($table_ref, $manufacturer, $table_name);

print STDERR "  Found " . scalar(@{$patterns_data->{patterns}}) . " model detection patterns\n";
if ($patterns_data->{conditional_tags}) {
    print STDERR "  Found " . scalar(@{$patterns_data->{conditional_tags}}) . " conditional tag arrays\n";
}
...
sub extract_model_detection_patterns {
    ...
    foreach my $key (sort keys %$table_ref) {
            ...
            if (@conditions) {
                ...
                foreach my $condition (@conditions) {
                    if ($condition->{condition}) {
                        my @model_refs = extract_model_patterns_from_condition($condition->{condition});
        ...
        # Handle single tag with condition
            ...
            my @model_refs = extract_model_patterns_from_condition($tag_info->{Condition});
            foreach my $pattern (@model_refs) {
                my $pattern_key = $pattern->{pattern};
                unless ($model_patterns_seen{$pattern_key}) {
                    push @patterns, $pattern;
...
sub extract_single_condition {
    ...
    $condition_data->{description} = $tag_def->{Description} if $tag_def->{Description};
