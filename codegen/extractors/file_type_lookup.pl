...
print STDERR "  Extensions: "
  . scalar( @{ $lookups_by_type{extensions} } ) . "\n";
print STDERR "  MIME types: "
  . scalar( @{ $lookups_by_type{mime_types} } ) . "\n";
print STDERR "  Descriptions: "
  . scalar( @{ $lookups_by_type{descriptions} } ) . "\n";
...
sub determine_lookup_type {
    ...
    if ( !ref $value ) {
        ...
        if ( $value =~ m{/} ) {
            ...
            return
              'description';    # Short uppercase string (alias to another type)
        }
        else {
