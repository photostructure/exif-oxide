...
echo "Checking which of ${#ALL_MODULES[@]} modules need patching..."
...
if [ ${#MODULES_TO_PATCH[@]} -eq 0 ]; then
  echo "✅ All modules already patched and formatted"
  exit 0
fi

echo "📝 Processing ${#MODULES_TO_PATCH[@]} modules that need conversion..."
