...
my $config_file = $ARGV[0] or die "Usage: $0 [--verbose] [--array ARRAY_NAME] CONFIG_FILE\n";
...
my $exiftool_lib_dir = File::Spec->catdir(dirname(__FILE__), "..", "..", "third-party", "exiftool", "lib");
unshift @INC, $exiftool_lib_dir;

# Load ExifTool and patch the Nikon module to access the arrays
my $exiftool_path = File::Spec->catfile($exiftool_lib_dir, "Image", "ExifTool", "Nikon.pm");
...
foreach my $array_config (@$arrays) {
    ...
    if (scalar(@perl_array) != scalar(@rust_array)) {
        print "❌ Size mismatch: perl=" . scalar(@perl_array) . " rust=" . scalar(@rust_array) . "\n";
        $failed_arrays++;
        next;
    }
    
    if ($expected_size && scalar(@perl_array) != $expected_size) {
        print "⚠️  Size mismatch with config: expected=$expected_size actual=" . scalar(@perl_array) . "\n";
    }
    ...
    for my $i (0 .. $#perl_array) {
        ...
        if ($perl_val != $rust_val) {
            print "❌ Element mismatch at index $i: perl=$perl_val (0x" . sprintf("%02x", $perl_val) . ") rust=$rust_val\n";
            $element_errors++;
    ...
    if ($element_errors > 0) {
        print "❌ $array_name failed validation ($element_errors element errors)\n";
        $failed_arrays++;
    } else {
        print "✅ $array_name passed validation (all " . scalar(@perl_array) . " elements match)\n";
        $passed_arrays++;
    }
...
sub find_rust_file {
    ...
    my $rust_file = File::Spec->catfile(dirname(__FILE__), "..", "..", "src", "generated", "Nikon_pm", $filename);
...
sub parse_rust_array {
    ...
    my $pattern = qr/pub\s+static\s+\Q$constant_name\E:\s*\[[^;]+;\s*\d+\]\s*=\s*\[(.*?)\];/s;
