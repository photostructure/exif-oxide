# ExifTool SubDirectory Coverage Report

Generated: Sat Jul 26 19:30:50 2025 UTC

## Summary

- **Total SubDirectories**: 1872
- **Implemented**: 222
- **Missing**: 1650
- **Coverage**: 11.86%

## Pattern Distribution

| Pattern Type | Count | Percentage |
|--------------|-------|------------|
| Binary_data  |   719 |      38.4% |
| Conditional  |     0 |       0.0% |
| Process_proc |     0 |       0.0% |
| Simple       |  1153 |      61.6% |
| Validated    |     0 |       0.0% |

## Module Breakdown

| Module | Total | Implemented | Coverage |
|--------|-------|-------------|----------|
| Nikon                |   218 |           1 |     0.5% |
| QuickTime            |   183 |          28 |    15.3% |
| Canon                |   147 |          73 |    49.7% |
| Exif                 |   122 |           0 |     0.0% |
| Sony                 |    95 |           4 |     4.2% |
| DNG                  |    94 |           0 |     0.0% |
| XMP                  |    80 |          78 |    97.5% |
| MXF                  |    71 |           0 |     0.0% |
| JPEG                 |    64 |           0 |     0.0% |
| RIFF                 |    56 |          20 |    35.7% |
| Olympus              |    52 |          17 |    32.7% |
| Pentax               |    51 |           0 |     0.0% |
| Matroska             |    48 |           0 |     0.0% |
| MIE                  |    34 |           0 |     0.0% |
| Jpeg2000             |    32 |           0 |     0.0% |
| PNG                  |    28 |           1 |     3.6% |
| CanonRaw             |    27 |           0 |     0.0% |
| Kodak                |    24 |           0 |     0.0% |
| FLIR                 |    23 |           0 |     0.0% |
| PDF                  |    20 |           0 |     0.0% |
| Panasonic            |    19 |           0 |     0.0% |
| DJI                  |    19 |           0 |     0.0% |
| FlashPix             |    19 |           0 |     0.0% |
| MNG                  |    18 |           0 |     0.0% |
| CanonVRD             |    16 |           0 |     0.0% |
| Leaf                 |    15 |           0 |     0.0% |
| LNK                  |    15 |           0 |     0.0% |
| NikonCapture         |    14 |           0 |     0.0% |
| MISB                 |    13 |           0 |     0.0% |
| Photoshop            |    13 |           0 |     0.0% |
| ASF                  |    12 |           0 |     0.0% |
| Ricoh                |    10 |           0 |     0.0% |
| Minolta              |     9 |           0 |     0.0% |
| GIMP                 |     9 |           0 |     0.0% |
| Samsung              |     9 |           0 |     0.0% |
| PanasonicRaw         |     9 |           0 |     0.0% |
| Real                 |     8 |           0 |     0.0% |
| H264                 |     7 |           0 |     0.0% |
| GIF                  |     7 |           0 |     0.0% |
| Flash                |     7 |           0 |     0.0% |
| ID3                  |     7 |           0 |     0.0% |
| IPTC                 |     7 |           0 |     0.0% |
| FujiFilm             |     7 |           0 |     0.0% |
| GoPro                |     7 |           0 |     0.0% |
| ICC_Profile          |     7 |           0 |     0.0% |
| MIFF                 |     6 |           0 |     0.0% |
| HTML                 |     6 |           0 |     0.0% |
| DjVu                 |     6 |           0 |     0.0% |
| Font                 |     5 |           0 |     0.0% |
| PSP                  |     5 |           0 |     0.0% |
| Ogg                  |     5 |           0 |     0.0% |
| Sanyo                |     4 |           0 |     0.0% |
| FLAC                 |     4 |           0 |     0.0% |
| AIFF                 |     4 |           0 |     0.0% |
| SigmaRaw             |     4 |           0 |     0.0% |
| Casio                |     4 |           0 |     0.0% |
| MinoltaRaw           |     4 |           0 |     0.0% |
| PostScript           |     4 |           0 |     0.0% |
| Vorbis               |     3 |           0 |     0.0% |
| FLIF                 |     3 |           0 |     0.0% |
| M2TS                 |     3 |           0 |     0.0% |
| RSRC                 |     3 |           0 |     0.0% |
| BPG                  |     3 |           0 |     0.0% |
| HP                   |     3 |           0 |     0.0% |
| Torrent              |     3 |           0 |     0.0% |
| TNEF                 |     2 |           0 |     0.0% |
| Sigma                |     2 |           0 |     0.0% |
| Stim                 |     2 |           0 |     0.0% |
| OpenEXR              |     2 |           0 |     0.0% |
| ISO                  |     2 |           0 |     0.0% |
| PICT                 |     2 |           0 |     0.0% |
| ITC                  |     2 |           0 |     0.0% |
| Opus                 |     2 |           0 |     0.0% |
| FotoStation          |     2 |           0 |     0.0% |
| MacOS                |     2 |           0 |     0.0% |
| Red                  |     2 |           0 |     0.0% |
| Theora               |     2 |           0 |     0.0% |
| MPF                  |     1 |           0 |     0.0% |
| AFCP                 |     1 |           0 |     0.0% |
| MPEG                 |     1 |           0 |     0.0% |
| PhotoMechanic        |     1 |           0 |     0.0% |
| ICO                  |     1 |           0 |     0.0% |
| WTV                  |     1 |           0 |     0.0% |
| BMP                  |     1 |           0 |     0.0% |
| SonyIDC              |     1 |           0 |     0.0% |
| PLIST                |     1 |           0 |     0.0% |
| PhaseOne             |     1 |           0 |     0.0% |
| Nintendo             |     1 |           0 |     0.0% |
| Unknown              |     1 |           0 |     0.0% |
| Apple                |     1 |           0 |     0.0% |
| JSON                 |     1 |           0 |     0.0% |

## Missing High-Priority SubDirectories

Top 20 unimplemented SubDirectories by module:

### Nikon

- **CustomSettingsD5000** (888): binary_data pattern, references `Image::ExifTool::NikonCustom::SettingsD5000`
- **OrientOffset** (160): binary_data pattern, references `Image::ExifTool::Nikon::OrientationInfo`
- **CustomSettingsD3** (769): binary_data pattern, references `Image::ExifTool::NikonCustom::SettingsD3`
- **CustomSettingsZ9** (847): binary_data pattern, references `Image::ExifTool::NikonCustom::SettingsZ9`
- **FlashInfo0100** (33554600): binary_data pattern, references `Image::ExifTool::Nikon::FlashInfo0100`

### QuickTime

- **HEVCConfiguration** (hvcC): binary_data pattern, references `Image::ExifTool::QuickTime::HEVCConfig`
- **ContentLightLevel** (clli): binary_data pattern, references `Image::ExifTool::QuickTime::ContentLightLevel`
- **AV1Configuration** (av1C): binary_data pattern, references `Image::ExifTool::QuickTime::AV1Config`
- **ICC_Profile** (colr): simple pattern, references `Image::ExifTool::ICC_Profile::Main`
- **ColorRepresentation** (colr): binary_data pattern, references `Image::ExifTool::QuickTime::ColorRep`

### Canon

- **ColorCalib** (320): binary_data pattern, references `Image::ExifTool::Canon::ColorCalib`
- **ColorCalib2** (255): binary_data pattern, references `Image::ExifTool::Canon::ColorCalib2`
- **ColorCoefs** (71): binary_data pattern, references `Image::ExifTool::Canon::ColorCoefs`
- **ColorCoefs2** (71): binary_data pattern, references `Image::ExifTool::Canon::ColorCoefs2`
- **ColorCalib2** (186): binary_data pattern, references `Image::ExifTool::Canon::ColorCalib2`

### Exif

- **IPTC-NAA** (33723): simple pattern, references `Image::ExifTool::IPTC::Main`
- **LeafSubIFD** (34954): simple pattern, references `Image::ExifTool::Leaf::SubIFD`
- **SubIFD** (330): simple pattern, references `Unknown`
- **JUMBF** (52545): simple pattern, references `Image::ExifTool::Jpeg2000::Main`
- **XiaomiSettings** (39321): simple pattern, references `Image::ExifTool::JSON::Main`

